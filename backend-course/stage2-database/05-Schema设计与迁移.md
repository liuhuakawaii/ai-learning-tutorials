...
你的博客系统需要存储用户信息、文章内容、分类标签、评论数据。如果直接开始写代码建表，你可能会发现：文章和标签是多对多关系需要中间表，评论的自引用回复需要特殊处理，删除用户时其文章应该怎么处理。这些决策都需要在写代码之前想清楚。Schema 设计就是把业务需求转化为数据库表结构的过程，而迁移则是将这些结构变更安全地同步到数据库并记录版本历史。
...
1. **在代码里直接修改数据库结构而不使用迁移**：手动执行 `ALTER TABLE` 改表结构会导致不同环境的数据库不一致，且无法回滚。始终使用 `prisma migrate` 管理结构变更。
2. **过度设计表结构**：初期设计时不需要考虑所有可能的扩展。先满足当前需求，需要时再通过迁移添加字段。过度设计会增加复杂度，拖慢开发进度。
3. **忘记设置 onDelete 行为**：外键关系必须考虑关联记录删除时的行为——Cascade（级联删除）、SetNull（设为空）、Restrict（阻止删除）。不设置会导致删除失败或产生孤儿数据。
4. **多对多关系手动创建中间表**：Prisma 支持隐式多对多关系（只需在两个模型中声明数组字段），会自动创建中间表。只有需要在中间表中存储额外字段时才需要显式定义。

## 工程建议

1. **迁移文件要提交到 Git**：`prisma/migrations/` 目录包含每次结构变更的 SQL 文件，是数据库结构的版本历史，团队成员可以通过它同步数据库状态。
2. **使用 prisma migrate reset 在开发环境重置数据库**：当迁移历史混乱时，`npx prisma migrate reset` 会删除数据库并重新执行所有迁移，快速恢复到一致状态。
3. **为外键字段创建索引**：`@@index([authorId])` 可以大幅加速关联查询。Prisma 不会自动为外键创建索引，需要手动在 Schema 中声明。
4. **用种子脚本填充测试数据**：创建 `prisma/seed.ts` 文件，用 `npx prisma db seed` 执行。这保证了每个开发者都有相同的测试数据，也方便演示和调试。
