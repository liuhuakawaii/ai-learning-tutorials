...
**思路**：为 PostgreSQL 和应用分别配置健康检查，使用 `depends_on` + `condition: service_healthy` 确保应用在数据库真正可用后才启动。
  ...
  app:
    ...
    command: >
      sh -c "
        ...
        echo 'Connecting to postgres:5432...' &&
        node -e \"
          const net = require('net');
          const client = net.createConnection(5432, 'postgres', () => {
...
docker compose ps
# 预期：
# postgres  Up (starting) 或 Up (healthy)
# app       Up (starting) — 等 postgres 变为 healthy 后才真正启动
...
**要点**：
- `healthcheck` 的 `test` 命令返回 0 表示健康，非 0 表示不健康
- `depends_on` + `condition: service_healthy` 是关键，单纯 `depends_on` 只等容器启动
- `start_period` 给服务初始启动时间，期间健康检查失败不算入 `retries`
- `pg_isready` 是 PostgreSQL 自带的轻量检查工具，比 `psql -c "SELECT 1"` 更合适
- 生产环境中，应用的 `/health` 端点也应该检查数据库连接

### 练习二

**思路**：故意配置错误的数据库密码，观察应用因连接失败而启动失败的行为，然后修复并重启。

**答案**：

```yaml
# compose.yml — 故意使用错误密码
services:
  postgres:
    image: postgres:14-alpine
    environment:
  ...
  app:
    image: postgres:14-alpine
    command: psql -h postgres -U postgres -d mydb -c "SELECT 1;"
    environment:
...
sleep 10
docker compose ps
# 预期：postgres Up (healthy)，app Exited

# 3. 查看 app 的错误日志
docker compose logs app
# 预期看到：FATAL: password authentication failed for user "postgres"

# 4. 修复密码——将 wrong_password 改为 secret
# 编辑 compose.yml，将 PGPASSWORD 改为 secret
...
**要点**：
- 错误密码导致 PostgreSQL 拒绝连接，应用退出码非 0
- `restart: "no"` 让容器失败后不自动重启，方便观察错误
- 生产环境用 `restart: unless-stopped` 配合健康检查，短暂故障可以自动恢复
- `docker compose up -d --force-recreate` 强制重建容器，应用新配置
- `docker compose logs` 是排查启动失败的首选工具
- 真实项目中，应用应该有连接重试机制（如等待数据库就绪后重试 3 次）
