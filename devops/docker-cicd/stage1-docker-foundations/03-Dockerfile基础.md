...
**思路**：这是一个最简单的 Node.js HTTP 服务，没有外部依赖，只需要基础镜像 + 复制文件 + 暴露端口 + 启动命令。
...
docker run -d -p 3000:3000 simple-http
curl http://localhost:3000
# 输出：Hello from Docker!
```

**要点**：
- 单文件项目不需要 `npm install`，直接 COPY 即可
- `WORKDIR /app` 设置工作目录，避免文件散落在根目录
- `EXPOSE 3000` 只是声明，实际映射需要 `-p 3000:3000`
- `CMD` 用 Exec 形式 `["node", "app.js"]` 比 Shell 形式更好，信号处理更正确

### 练习二

**思路**：有依赖的项目需要先复制 `package.json` 安装依赖，再复制源代码。同时创建 `.dockerignore` 排除不需要的文件。
...
docker run -d -p 3000:3000 express-demo
curl http://localhost:3000
# 输出：{"message":"Hello Express!"}
```

**要点**：
- 先复制 `package.json` + `package-lock.json`，再 `npm ci`，利用构建缓存
- 用 `npm ci --only=production` 只安装生产依赖，减小镜像体积
- `.dockerignore` 排除 `node_modules`、`.env` 等不需要进入镜像的文件
- 源代码放在 `src/` 目录下，结构更清晰

### 练习三

**思路**：故意制造构建错误，通过观察错误信息和使用中间镜像调试来理解构建过程。

**答案**：

```bash
# 1. 故意写一个错误的 Dockerfile（假设 package.json 中没有 install-express 脚本）
...
# 2. 运行构建，观察错误
docker build -t debug-app .
# 错误信息：npm ERR! missing script: install-express
# 注意错误发生在哪一步，以及之前哪些步骤成功了

# 3. 使用最后一个成功步骤的镜像 ID 调试
# 假设 Step 1 (FROM) 和 Step 2 (WORKDIR) 成功了
# 在构建输出中找到成功步骤的镜像 ID
docker run -it <last-successful-image-id> /bin/sh
# 进入容器后可以检查文件系统状态
ls -la /app/
...
**要点**：
- 构建失败时，Docker 会保留最后一个成功步骤的镜像，可以用它来调试
- 看错误信息时注意是哪一步失败的，检查该步骤的输入是否正确
- 用 `docker run -it <image-id> /bin/sh` 进入中间镜像，手动检查文件和目录
- 常见错误：文件名拼写错误、命令不存在、依赖未安装
