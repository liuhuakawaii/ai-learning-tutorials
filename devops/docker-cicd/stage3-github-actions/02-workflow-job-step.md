...
**思路**：使用矩阵策略在多个 Node.js 版本上并行运行测试，确保代码的版本兼容性。通过 `strategy.matrix` 定义版本数组，GitHub Actions 会自动为每个版本创建一个独立的 Job。
...
**要点**：
- `strategy.matrix.node-version` 定义要测试的版本数组，GitHub Actions 会为每个版本创建一个并行 Job
- `${{ matrix.node-version }}` 在 step 中引用当前矩阵值
- `fail-fast: false` 让一个版本失败时其他版本继续运行，方便看到所有版本的兼容性结果
- 如果不设置 `fail-fast`，默认为 `true`，一个失败会取消其他所有

### 练习二

**思路**：使用 `needs` 关键字定义 Job 之间的依赖关系，让 lint 和 test 并行执行，两者都通过后才执行 build。这是典型的 CI 流水线模式。
...
**要点**：
- `needs: [lint, test]` 表示 build 依赖 lint 和 test，两者都成功后才执行
- 没有 `needs` 的 Job 默认并行执行，所以 lint 和 test 会同时运行
- 每个 Job 运行在独立的虚拟机上，不共享文件系统，所以每个 Job 都需要 `actions/checkout`
- 如果 lint 或 test 任一失败，build 不会执行，整个 workflow 标记为失败
