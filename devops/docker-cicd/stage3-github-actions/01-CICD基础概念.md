...
**思路**：创建一个最简单的 GitHub Actions workflow，验证 CI 流水线能否正常触发和运行。关键是在项目根目录下创建 `.github/workflows/` 目录，并在里面放一个 YAML 文件。
...
2. 将上述内容保存为 `.github/workflows/ci.yml`
3. `git add .github/workflows/ci.yml && git commit -m "Add CI workflow" && git push`
4. 在 GitHub 仓库的 Actions 标签页查看运行结果

**要点**：
- workflow 文件必须放在 `.github/workflows/` 目录下，文件名任意（`.yml` 或 `.yaml`）
- `on` 字段定义触发条件，`push` 和 `pull_request` 是最常用的两种事件
- `runs-on: ubuntu-latest` 指定运行环境为最新的 Ubuntu 虚拟机
- 每个 `step` 的 `run` 字段执行 shell 命令

### 练习二

**思路**：在练习一的基础上，添加实际的代码检查步骤。需要先 checkout 代码，再安装 Node.js 环境，最后运行 lint 和测试命令。使用 `actions/setup-node` 的 `cache: npm` 选项可以自动缓存依赖。
...
**要点**：
- `actions/checkout@v4` 是必须的第一步，否则 CI 环境没有你的代码
- `actions/setup-node@v4` 安装指定版本的 Node.js，`cache: npm` 自动缓存依赖
- `npm ci` 比 `npm install` 更快、更可靠，专门用于 CI 环境，严格按照 `package-lock.json` 安装
- 如果项目没有 `npm run lint` 或 `npm test` 命令，需要先在 `package.json` 中配置好这些脚本
