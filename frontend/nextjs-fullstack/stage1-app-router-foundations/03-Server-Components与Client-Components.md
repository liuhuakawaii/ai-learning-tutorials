...
**思路**：Server Component 可以直接使用 `async/await` 获取数据，不需要 `useEffect` 或客户端状态管理。数据在服务端获取，渲染好的 HTML 发送给浏览器。
...
**要点**：
- Server Component 中直接用 `async/await`，不需要 `useEffect` 或 `useState`
- 数据在服务端获取，不会暴露给客户端（安全）
- `date-fns`、`lodash` 等工具库不会被打包进客户端 JS bundle
- 模拟的 500ms 延迟只影响服务端渲染时间，不会导致客户端白屏

### 练习 2：Client Component 交互

**思路**：搜索和过滤是用户交互行为，需要响应键盘输入，所以必须用 Client Component。通过 `'use client'` 指令标记，使用 `useState` 管理搜索关键词。
...
**要点**：
- 搜索输入需要响应 `onChange` 事件，这是客户端交互，必须用 `'use client'`
- `useState` 只能在 Client Component 中使用
- `users` 作为 props 从 Server Component 传入，数据获取仍在服务端

### 练习 3：组合 Server 和 Client Component

**思路**：Server Component 负责数据获取（可以访问数据库），Client Component 负责用户交互（排序、筛选）。这是 Next.js 推荐的组合模式——把 `'use client'` 边界推向叶子节点，最小化客户端 JS 体积。
...
async function fetchProducts(): Promise<Product[]> {
  ...
  return [
    { id: 1, name: '机械键盘', price: 599 },
    { id: 2, name: '无线鼠标', price: 199 },
    { id: 3, name: '显示器', price: 2499 },
    { id: 4, name: '耳机', price: 899 },
  ]
}
...
**要点**：
- Server Component（`ProductsPage`）负责数据获取，不会把 `fetchProducts` 打包到客户端
- Client Component（`ProductList`）负责排序交互，使用 `useState` 管理排序方式
- `'use client'` 边界在 `ProductList` 组件，而不是整个页面，这样数据获取逻辑留在了服务端
- 产品数据作为 props 从 Server Component 传递到 Client Component，这是两者协作的标准模式
