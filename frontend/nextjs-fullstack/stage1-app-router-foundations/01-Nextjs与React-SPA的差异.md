...
你用 Create React App 做了一个产品展示网站，部署上线后发现两个问题：一是搜索引擎完全搜不到你的页面，因为爬虫看到的是一个空的 `<div id="root"></div>`；二是用户反馈"打开页面要白屏好几秒"，尤其是手机端网络慢的时候。你尝试了各种优化手段——代码分割、预加载、CDN——但白屏问题始终无法根本解决，因为问题出在架构上：浏览器必须先下载并执行完所有 JavaScript，才能渲染出第一行内容。这时候你开始思考：有没有一种方案，能让服务器直接返回渲染好的 HTML？
...
**思路**：使用 `create-next-app` 脚手架创建项目，重点关注项目启动后页面是否一加载就有内容，以及页面源代码中是否包含完整的 HTML。
...
创建完成后，打开 http://localhost:3000，你会观察到：

1. 页面一加载就有完整内容，不会出现白屏（这是 SSR/SSG 的功劳）
2. 右键"查看页面源代码"，HTML 中已经包含完整的页面内容（标题、描述等），而不是只有一个空的 `<div id="root">`
3. 对比 Create React App 项目：CRA 的页面源代码通常只有 `<div id="root"></div>`，需要等 JS 下载执行后才能看到内容

**要点**：
- Next.js 默认在服务端渲染页面，用户看到的是已经包含内容的 HTML
- 这与 CRA 的纯客户端渲染形成鲜明对比
- 项目结构中 `app/` 目录是 App Router 的核心，`page.tsx` 文件定义了路由对应的页面

### 练习 2：体验服务端渲染

**思路**：创建一个包含异步数据获取的 Server Component，观察每次刷新页面时数据是否变化，以此验证服务端渲染的特性。
...
1. 每次刷新页面，`time` 都会变化——说明服务器在每次请求时都重新执行了渲染
2. `randomNumber` 每次都不同——进一步证明这是服务端实时生成的
3. 查看页面源代码，HTML 中已经包含了 `message`、`time` 和 `randomNumber` 的值，不需要等 JS 加载

**要点**：
- Server Component 中可以直接使用 `async/await`，这是客户端组件做不到的
- 服务端渲染意味着 HTML 在服务器上生成，浏览器拿到的就是完整内容
- 每次请求都会重新执行渲染逻辑，所以时间戳和随机数会变化

### 练习 3：对比 CSR 和 SSR

**思路**：创建两个功能相同但渲染策略不同的页面，通过查看页面源代码对比两者的 HTML 输出差异，直观理解 CSR 和 SSR 的区别。
...
1. CSR 页面的源代码中，`生成时间` 显示的是 `加载中...`，需要等 JS 执行后才会更新为真实时间
2. SSR 页面的源代码中，`生成时间` 已经是具体的 ISO 时间字符串，浏览器直接展示
3. CSR 页面首次加载会有短暂的"加载中..."闪烁，SSR 页面直接显示最终内容

**要点**：
- CSR 的 HTML 只是一个空壳，内容由 JavaScript 在浏览器中动态生成
- SSR 的 HTML 已经包含了完整内容，浏览器可以立即渲染
- 使用 `'use client'` 标记的组件会在客户端执行，数据获取和渲染都在浏览器中完成
- 没有 `'use client'` 的组件默认是 Server Component，在服务端执行

---

## 常见误区

1. **"SSR 一定比 SSR 快"**：SSR 需要服务器每次请求都执行渲染，如果服务器离用户远或数据库查询慢，首屏时间可能反而更长。SSG 和 ISR 才是静态内容的最佳选择。
2. **"用了 Next.js 就不需要 API 了"**：Server Component 可以直接查数据库，但这不意味着所有数据都应该在组件里查。跨页面共享的数据、需要被客户端轮询的数据，仍然适合用 API Route。
3. **"所有页面都应该用 SSR"**：纯交互的页面（如聊天窗口、实时协作编辑器）用 CSR 更合适。选择渲染策略应该基于数据的变化频率和页面的交互需求。
4. **"App Router 完全取代了 Pages Router"**：虽然 App Router 是官方推荐，但 Pages Router 仍然被支持且稳定。迁移项目不必急于一时，新项目优先用 App Router 即可。

## 工程建议

1. **新项目直接用 App Router**：不要再用 Pages Router 启动新项目，App Router 的 Server Components、流式渲染和 Server Actions 是未来方向。
2. **静态内容优先用 SSG/ISR**：博客、文档、营销页面这类不常变化的内容，用 SSG 构建时生成，配合 ISR 定期更新，既快又省服务器资源。
3. **查看页面源代码验证渲染策略**：开发时右键"查看页面源代码"，如果 HTML 里已经有完整内容，说明是 SSR/SSG；如果只有空壳 div，说明是 CSR。
4. **善用 Next.js 内置优化**：`next/image` 自动优化图片、`next/font` 预加载字体、`next/link` 预取路由，这些开箱即用的优化不要忽略。
