...
**思路**：使用 Server Actions 实现待办事项的增删改查。`addTodo` 和 `toggleTodo` 作为 Server Actions，在服务端操作数据后调用 `revalidatePath` 刷新页面。页面本身是 Server Component，直接获取数据渲染。
...
export default async function TodosPage() {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '500px' }}>
      <h1>待办事项</h1>
      <form action={addTodo} style={{ marginBottom: '20px', display: 'flex', gap: '8px' }}>
        <input
          name="text"
          placeholder="添加新任务..."
          required
...
**要点**：
- Server Actions 用 `'use server'` 标记，可以在 `<form action={...}>` 中直接调用
- `revalidatePath('/todos')` 在数据变更后清除缓存，触发页面重新渲染
- 使用 `bind(null, id)` 可以在表单 action 中传递参数
- 不需要手动管理 loading 状态，表单提交期间 Next.js 会自动处理

### 练习 2：实现删除确认

**思路**：删除是不可逆操作，需要在执行前弹出确认对话框。使用 `useState` 控制确认弹窗的显示/隐藏，确认后再调用 Server Action 执行删除。
...
export function DeleteButton({ postId, postTitle }: { postId: number; postTitle: string }) {
  const [showConfirm, setShowConfirm] = useState(false)
  ...
  return (
    <>
      <button
        ...
        style={{ padding: '4px 12px', background: '#fee', color: '#c00', border: '1px solid #c00', borderRadius: '4px', cursor: 'pointer' }}
      >
        删除
      </button>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '16px' }}>
              ...
              <button
                ...
                style={{ padding: '8px 16px', background: '#c00', color: 'white', border: 'none', borderRadius: '4px' }}
                disabled={isDeleting}
              >
                {isDeleting ? '删除中...' : '确认删除'}
              </button>
...
**要点**：
- `useState` 控制确认弹窗的显示状态，这是典型的 Client Component 交互
- 删除按钮点击后显示确认弹窗，而不是直接执行删除
- 添加 `isDeleting` 状态防止重复提交
- 确认弹窗用 `position: fixed` 覆盖整个页面，形成模态效果
- `deletePost` 是 Server Action，在服务端执行删除操作

### 练习 3：实现乐观更新的点赞功能

**思路**：使用 `useOptimistic` 实现乐观更新——点击按钮后立即更新 UI，不等待服务器响应。如果服务器操作失败，需要回滚到之前的状态。
...
export function LikeButton({ postId, initialLikes }: { postId: number; initialLikes: number }) {
  const [optimisticLikes, addOptimisticLike] = useOptimistic(
    initialLikes,
    (currentLikes, _: void) => currentLikes + 1
  )
    ...
    <button
      onClick={async () => {
        ...
        try {
          ...
          // useOptimistic 会在 revalidatePath 后自动同步服务端数据
        }
      }}
      style={{
        padding: '8px 16px',
...
```tsx
// app/posts/page.tsx（使用示例）
import { LikeButton } from './LikeButton'

// 模拟数据
const posts = [
  { id: 1, title: 'Next.js 入门', likes: 10 },
  { id: 2, title: 'React Server Components', likes: 25 },
]
...
export default function PostsPage() {
  return (
    <div style={{ padding: '20px' }}>
      <h1>文章列表</h1>
      {posts.map(post => (
        <div key={post.id} style={{ padding: '16px', border: '1px solid #eee', marginBottom: '12px', borderRadius: '8px' }}>
          <h2>{post.title}</h2>
...
**要点**：
- `useOptimistic` 在服务器响应前先更新 UI，用户感受到即时反馈
- 第一个参数是初始值（`initialLikes`），第二个参数是更新函数
- 点击后 UI 立即 +1，同时异步调用 Server Action
- 当 `revalidatePath` 触发重新渲染时，`useOptimistic` 会自动同步服务端返回的真实数据
- 如果服务器操作失败，下次渲染会回滚到服务端的实际值
- 适合点赞、收藏等高频交互场景，提升用户体验
