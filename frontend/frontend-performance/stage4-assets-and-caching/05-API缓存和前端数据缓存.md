...
**思路**：实现一个带 TTL 的内存缓存，添加 LRU 淘汰策略，测试缓存命中率。
...
// 测试
await fetchWithCache('/api/products');  // MISS → 请求 API
await fetchWithCache('/api/products');  // HIT → 直接返回缓存
```
...
**思路**：用 React Query 替代手动 fetch，配置合理的 staleTime，实现 mutation 后缓存失效。
...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      staleTime: 5 * 60 * 1000,      // 5 分钟后数据标记为 stale
      gcTime: 10 * 60 * 1000,        // 10 分钟后清除缓存
...
**要点**：
- staleTime 控制数据多久后标记为 stale（过期但不删除）
- gcTime 控制未使用的缓存多久后清除
- invalidateQueries 使缓存失效，触发重新请求

### 练习三：乐观更新

**思路**：实现一个待办事项列表，勾选时使用乐观更新，处理失败回滚。
...
**要点**：
- 乐观更新让用户立即看到操作结果，提升交互体验
- onMutate 中保存当前数据，onError 中回滚
- onSettled 无论成功或失败都重新验证，确保数据一致性
