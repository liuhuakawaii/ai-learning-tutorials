...
**思路**：用 web-vitals 库采集真实页面的三个 Core Web Vitals 指标，重点关注哪个指标最差以及对应的元素。
...
function sendToAnalytics(metric) {
  console.group(`📊 ${metric.name}: ${metric.value.toFixed(1)}`);
  console.log(`评级: ${metric.rating}`); // good / needs-improvement / poor
  console.log(`数值: ${metric.value}`);
  if (metric.entries.length > 0) {
    console.log('关联元素:', metric.entries[0].target || metric.entries[0].element);
  }
  console.groupEnd();
}
...
→ 原因: 首屏 banner 图 600KB JPEG，未 preload
...
→ 偏移元素: <div class="ad-slot" />
→ 原因: 广告位加载后才撑开空间

哪个指标最差？→ CLS 0.22，因为广告位没有预留空间
```

**要点**：
- web-vitals 的 `metric.attribution` 可以提供更详细的归因信息（如 LCP 的资源 URL、INP 的交互类型）
- INP 只有在用户实际交互后才会输出，开发时需要主动点击页面
- LCP 元素通常是首屏最大的图片或标题文本块

### 练习二：诊断 LCP 问题

**思路**：用 DevTools Performance 面板录制页面加载，找到 LCP 元素，分析它为什么加载慢。
...
1. 打开 DevTools → Performance → 开启 CPU 4x 节流
2. 点击刷新按钮录制页面加载
3. 找到 LCP 标记（蓝色竖线），点击查看对应元素

分析结果：
- LCP 元素: <img src="/images/hero.webp" />
- LCP 时间: 3.2s

在 Network 面板中找到 hero.webp：
- 开始加载时间: 1.8s（很晚！因为前面有 JS 阻塞）
- 加载完成时间: 2.9s
- 文件大小: 450KB（WebP 格式）

根因分析：
1. hero.webp 没有 preload → 浏览器解析到 <img> 才发现
2. 头部有两个同步 <script> 阻塞了 HTML 解析 → 延迟了资源发现
3. 图片虽然用了 WebP 但尺寸仍然偏大（450KB）

优化方案：
1. 添加 <link rel="preload" href="/images/hero.webp" as="image">
2. 同步 script 改为 defer
3. 用 srcset 提供 800w 和 1200w 两种尺寸
4. 预期 LCP 从 3.2s 降到 1.5s
```

**要点**：
- LCP 时间 = 资源发现时间 + 资源加载时间 + 渲染时间
- 很多时候瓶颈在"资源发现时间"——浏览器太晚才知道需要加载这个资源
- preload 可以让浏览器在解析 HTML 之前就开始下载 LCP 资源

### 练习三：制造并修复 CLS

**思路**：故意创建一个有 CLS 问题的页面，量化问题，然后逐个修复并对比效果。
...
<body>
  ...
  <script>
    setTimeout(() => {
      ...
      banner.style.cssText = 'background:#ff0;padding:20px;text-align:center;';
      banner.textContent = '限时优惠！';
...
<html>
<head>
  <!-- 修复 3：预加载字体 + font-display: optional -->
  ...
  <style>
    ...
    /* 修复 2：为横幅预留空间 */
    .banner-placeholder { min-height: 60px; }
    /* 修复 1：图片使用 aspect-ratio */
    .news-photo { aspect-ratio: 16/9; width: 100%; object-fit: cover; }
  </style>
</head>
<body>
  <div class="banner-placeholder"></div>
  <h1>新闻标题</h1>
  <!-- 修复 1：添加 width/height -->
  <img class="news-photo" src="/photo.jpg" alt="新闻图片" width="800" height="450">
  <p>正文内容正在阅读中...</p>
</body>
</html>
```
...
**要点**：
- 图片尺寸问题对 CLS 影响最大（修复 1 可减少约 60% 的 CLS）
- font-display: optional 虽然可能永远不显示 Web 字体，但完全消除了字体 CLS
- 动态内容应该用 `position: fixed` 或预留空间，而不是在文档流中插入
