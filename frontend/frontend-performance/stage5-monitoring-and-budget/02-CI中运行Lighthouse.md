...
**思路**：为项目创建 lighthouserc.js，配置性能断言，本地运行 lhci autorun 测试。
...
# ✅ CLS: 0.05 (≤ 0.1)
# ✅ TBT: 150ms (≤ 200ms)
# ⚠️ JS size: 220KB (≤ 200KB) - WARN
```

**要点**：
- numberOfRuns: 3 取中位数，减少 Lighthouse 分数波动
- 先用 warn 收集数据，稳定后再提升为 error
- 临时存储可以查看报告链接

### 练习二：集成 GitHub Actions

**思路**：创建 lighthouse.yml 工作流，配置 PR 触发，测试断言失败时的行为。
...
断言失败时的处理：
1. 点击 "Details" 查看 GitHub Actions 日志
2. 下载 Lighthouse Report artifact
3. 打开 HTML 报告查看详细分析
4. 优化后重新提交 PR
```

**要点**：
- Lighthouse CI GitHub App 可以自动生成 PR 评论
- uploadArtifacts: true 保存 HTML 报告
- 断言失败时阻止 PR 合并，防止性能退化

### 练习三：自定义断言

**思路**：为不同页面设置不同的性能预算，添加资源大小断言。
