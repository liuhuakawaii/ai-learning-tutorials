...
你已经设计好了数据库表结构和索引，接下来需要把数据暴露给前端。前端同事问你："接口文档在哪？参数类型是什么？错误码怎么定义？"你发现手写接口文档容易过时，参数校验散落在各个函数里，错误响应格式不统一。你需要一个框架，能自动生成文档、统一参数校验、规范响应格式——FastAPI 正是为此而生。
...
- **路由函数里直接写业务逻辑**：路由层应该只负责参数接收和响应返回，业务逻辑放在 Service 层，保持路由函数简洁
- **不定义响应模型，直接返回字典**：没有响应模型，前端无法知道接口返回什么字段、什么类型，也无法利用 FastAPI 的自动文档功能
- **错误时返回 200 状态码加错误信息**：应该使用标准 HTTP 状态码（404、422、500），让客户端能正确处理不同类型的错误
- **把所有路由写在一个文件里**：随着接口增多，单文件会变得难以维护，应该按功能模块拆分路由

---

## 工程建议

- 使用 Pydantic 模型定义请求参数和响应结构，FastAPI 会自动生成 OpenAPI 文档和参数校验
- 为所有接口定义统一的错误响应格式（包含 error 和 detail 字段），前端只需一套错误处理逻辑
- 路由按资源拆分（jobs.py、metrics.py），每个路由文件使用 APIRouter，通过 prefix 注册到主应用
- 使用 Depends 注入数据库连接和配置，便于测试时替换为 mock 对象
...
**思路**：按照 RESTful 规范设计路由，资源名用复数，HTTP 方法对应 CRUD 操作。
...
**要点**：
- 路由按资源拆分，products 和 prices 分属不同模块
- 路径参数用 `{product_id}`，查询参数用 `Query`
- 搜索接口独立于列表接口，语义更清晰
...
**思路**：使用 Pydantic 定义响应模型，明确字段类型和描述，FastAPI 自动生成文档。
...
class ProductResponse(BaseModel):
    ...
    price: Optional[float] = Field(None, description="当前价格", unit="元")
    price_min: Optional[float] = Field(None, description="历史最低价")
    price_max: Optional[float] = Field(None, description="历史最高价")
    ...
    source: str = Field(..., description="数据来源")
    source_url: Optional[str] = Field(None, description="商品链接")
...
class PriceTrendResponse(BaseModel):
    ...
    lowest_price: float = Field(..., description="历史最低价")
    highest_price: float = Field(..., description="历史最高价")
    trend: List[PricePoint] = Field(..., description="价格趋势数据")
    update_time: datetime = Field(..., description="数据更新时间")
...
class ProductListResponse(BaseModel):
    """商品列表响应"""
    data: List[ProductResponse] = Field(..., description="商品列表")
    pagination: PaginationInfo = Field(..., description="分页信息")
    update_time: datetime = Field(..., description="数据更新时间")
...
**要点**：
- 每个字段都加 `Field(description=...)`，自动生成 OpenAPI 文档
- 嵌套模型（PricePoint）用于复杂响应结构
- 统一的 PaginationInfo 和 ErrorResponse 可在多处复用

---

### 练习三

**思路**：实现 Service 层封装查询逻辑，路由层只负责参数接收和响应组装。
...
class ProductService:
    ...
    async def list_products(
        ...
        # 白名单校验排序字段，防止 SQL 注入
        allowed_sort_fields = {"price", "sales_count", "shop_rating", "created_at"}
        if sort_by not in allowed_sort_fields:
            sort_by = "created_at"
        sort_order = "DESC" if sort_order == "desc" else "ASC"

        count_sql = f"SELECT COUNT(*) FROM clean_products WHERE {where_clause}"
        data_sql = f"""
            SELECT product_id, name, category, brand, price,
                   shop_name, shop_rating, sales_count, source
            FROM clean_products
            WHERE {where_clause}
            ORDER BY {sort_by} {sort_order}
            LIMIT :limit OFFSET :offset
    ...
    async def get_product(self, product_id: str) -> Optional[Dict[str, Any]]:
        """获取单个商品"""
        sql = "SELECT * FROM clean_products WHERE product_id = :product_id"
        with self.engine.connect() as conn:
            row = conn.execute(text(sql), {"product_id": product_id}).fetchone()
            return dict(row._mapping) if row else None
```

**要点**：
- 排序字段用白名单校验，防止用户传入恶意字段名
- 路由层只做参数接收和响应组装，业务逻辑全在 Service 层
- 使用参数化查询（`:param`）防止 SQL 注入
