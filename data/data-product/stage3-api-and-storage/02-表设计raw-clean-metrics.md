...
**思路**：按照 raw → clean → metrics 三层架构设计，每层职责明确，字段根据业务需求选取。
...
CREATE TABLE raw_products (
    id              BIGSERIAL PRIMARY KEY,
    raw_data        JSONB NOT NULL,              -- 原始数据（JSON）
    source          VARCHAR(50) NOT NULL,         -- 数据来源（如 jd, taobao）
...
CREATE TABLE clean_products (
    product_id      VARCHAR(50) PRIMARY KEY,      -- 业务唯一键
    name            VARCHAR(200) NOT NULL,        -- 商品名称
    category        VARCHAR(100),                 -- 分类
    brand           VARCHAR(100),                 -- 品牌
    price           DECIMAL(10,2),                -- 当前价格
    price_min       DECIMAL(10,2),                -- 历史最低价
    price_max       DECIMAL(10,2),                -- 历史最高价
    ...
    crawl_time      TIMESTAMP NOT NULL,
    batch_id        VARCHAR(100) NOT NULL,
    raw_id          BIGINT REFERENCES raw_products(id),
    created_at      TIMESTAMP DEFAULT NOW(),
    updated_at      TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_clean_products_category ON clean_products(category);
CREATE INDEX idx_clean_products_price ON clean_products(price);
CREATE INDEX idx_clean_products_source ON clean_products(source);

-- 3. metrics_price_daily：按天聚合的价格指标
CREATE TABLE metrics_price_daily (
    id              BIGSERIAL PRIMARY KEY,
    product_id      VARCHAR(50) NOT NULL,         -- 商品 ID
    stat_date       DATE NOT NULL,                -- 统计日期
    avg_price       DECIMAL(10,2),                -- 当日平均价格
    min_price       DECIMAL(10,2),                -- 当日最低价格
    max_price       DECIMAL(10,2),                -- 当日最高价格
    price_change    DECIMAL(10,2),                -- 价格变动（相对前一天）
    record_count    INTEGER DEFAULT 0,            -- 采集次数
    UNIQUE(product_id, stat_date)
);

CREATE INDEX idx_metrics_price_daily_date ON metrics_price_daily(stat_date);
CREATE INDEX idx_metrics_price_daily_product ON metrics_price_daily(product_id);
```

**要点**：
- Raw 表用 JSONB 存原始数据，保留灵活性
- Clean 表用业务唯一键（product_id）做主键，支持 Upsert
- Metrics 表用维度+时间做唯一约束，保证幂等

---

### 练习二

**思路**：根据查询模式设计索引，等值查询用 B-tree，范围查询考虑组合索引。
...
-- 3. 按城市+薪资组合查询（组合索引，city 在前因为选择性更高）
CREATE INDEX idx_clean_jobs_city_salary ON clean_jobs(city, salary_avg);
...
-- 可选：覆盖索引，避免回表
CREATE INDEX idx_clean_jobs_city_salary_cover ON clean_jobs(city) INCLUDE (salary_avg, title, company);
```

**要点**：
- 组合索引 (city, salary_avg) 同时支持 `WHERE city = ?` 和 `WHERE city = ? AND salary_avg BETWEEN ? AND ?`
- 如果查询经常 `ORDER BY salary_avg DESC`，组合索引也能避免排序操作

---

### 练习三

**思路**：从 clean_jobs 按城市和日期分组聚合，使用 ON CONFLICT 实现 Upsert。

**答案**：

```sql
INSERT INTO metrics_city_daily (city, stat_date, job_count, avg_salary, max_salary, min_salary, median_salary)
SELECT
    city,
    publish_date AS stat_date,
    COUNT(*) AS job_count,
...
**要点**：
- 使用 `ON CONFLICT ... DO UPDATE` 实现幂等操作，重跑不会产生重复数据
- 过滤 `salary_avg IS NOT NULL` 避免空值影响聚合结果
- `PERCENTILE_CONT` 计算中位数，比单纯 AVG 更能反映真实薪资水平
