...
**思路**：打开目标网站的 DevTools，用 XHR 过滤器找到 API 请求，记录 URL、请求方法、请求头和参数，然后用 requests 复现。
...
def fetch_api_data():
    """
    复现通过 DevTools 找到的 API 请求
    以掘金首页文章列表为例（API 可能随时间变化，需自行确认）
    """
    # 从 DevTools Network 面板复制的信息
    url = "https://api.juejin.cn/recommend_api/v1/article/recommend_all_feed"

    # 请求头（从 DevTools 的 Headers 面板复制）
    headers = {
...
**要点**：
- DevTools 的 Network 面板是找 API 的核心工具，XHR 过滤器可以排除图片、CSS 等干扰
- 注意区分 GET 和 POST 请求，POST 请求的参数在 Payload 面板中查看
- 请求头中的 `Content-Type`、`Referer`、`Authorization` 等字段可能对请求成功至关重要
- API 结构可能随时变化，实际操作时以 DevTools 中看到的为准

### 练习二

**思路**：使用 GitHub 搜索 API，通过 page 参数翻页获取前 50 个仓库，提取关键字段后按 Star 数降序排列并保存为 CSV。
...
def scrape_top_repos(language, count=50):
    ...
    url = "https://api.github.com/search/repositories"
    all_repos = []
    per_page = min(count, 100)  # GitHub 搜索 API 每页最多 100 条
    ...
    for page in range(1, pages_needed + 1):
        ...
        response = requests.get(url, params=params, headers={
            "Accept": "application/vnd.github.v3+json",
            "User-Agent": "Python-GitHub-Scraper-Tutorial"
        })
        ...
        print(f"  获取到 {len(items)} 个仓库，累计 {len(all_repos)} 个")
    ...
    # 保存为 CSV
    filename = f"top_{language}_repos.csv"
    with open(filename, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["name", "stars", "forks", "description", "created_at"])
        writer.writeheader()
        writer.writerows(all_repos)

    print(f"\n已保存到 {filename}，共 {len(all_repos)} 个仓库")

    # 打印前 10 个
    print(f"\n{language} 热门仓库 Top 10:")
...
if __name__ == "__main__":
    repos = scrape_top_repos("python", count=50)
```

**要点**：
- GitHub 搜索 API 每页最多返回 100 条，需要通过 `page` 参数翻页
- `q=language:python` 是 GitHub 搜索语法，限定编程语言
- 每次请求间隔 2 秒，避免触发速率限制（无 token 时每小时 60 次）
- 保存为 CSV 时用 `utf-8-sig` 编码，确保 Excel 打开中文不乱码

### 练习三

**思路**：注册 OpenWeatherMap 获取免费 API Key，用 requests 调用天气 API 获取多个城市数据，处理错误并整理成表格。
...
def get_cities_weather(cities):
    ...
    for city in cities:
        ...
        if weather:
            ...
            print(f"  {weather['天气']}, {weather['温度']}°C")
        time.sleep(1)  # 礼貌等待

    # 保存为 CSV
    if results:
        filename = "cities_weather.csv"
        with open(filename, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.DictWriter(f, fieldnames=results[0].keys())
            writer.writeheader()
            writer.writerows(results)
        print(f"\n已保存到 {filename}，共 {len(results)} 个城市")
...
if __name__ == "__main__":
    ...
    # 打印汇总表格
    print(f"\n{'城市':<8} {'天气':<12} {'温度':>6} {'湿度':>6}")
    print("-" * 36)
    for w in weather_data:
        print(f"{w['城市']:<8} {w['天气']:<12} {w['温度']:>5}°C {w['湿度']:>5}%")
```

**要点**：
- API Key 应从环境变量读取（`os.environ.get("API_KEY")`），不要硬编码在代码中
- `units="metric"` 返回摄氏度，`lang="zh_cn"` 返回中文天气描述
- 需要处理多种错误：401（Key 无效）、404（城市不存在）、超时、连接失败
- 即使是免费 API 也应该控制请求频率，每次请求间隔 1 秒
