...
**思路**：使用 `csv.DictWriter` 写入数据，设置 `delimiter=';'` 指定分号分隔符，`quoting=csv.QUOTE_NONNUMERIC` 让非数字字段自动加引号，编码用 `utf-8-sig` 确保 Excel 兼容。
...
def write_csv_advanced(data, filename):
    """将数据写入 CSV 文件：分号分隔、utf-8-sig 编码、非数字字段加引号"""
    fieldnames = ['标题', '价格', '分类']
    with open(filename, 'w', newline='', encoding='utf-8-sig') as f:
...
write_csv_advanced(data, 'advanced.csv')

# 验证：读回数据
with open('advanced.csv', 'r', encoding='utf-8-sig') as f:
    print(f.read())
```

**要点**：
- `delimiter=';'` 让分号成为分隔符，避免数据中的逗号干扰解析
- `QUOTE_NONNUMERIC` 会自动给非数字字段（如标题、分类）加双引号
- 数据中包含逗号和分号的字段会被正确包裹在引号内，不会破坏格式

### 练习二

**思路**：先用 `json.dump` 写入嵌套数据，再用 `json.load` 读回，遍历每本书的评论列表计算平均评分，筛选出平均评分大于 4.7 的书名。
...
# 写入 JSON 文件
with open('books_reviews.json', 'w', encoding='utf-8') as f:
    json.dump(books, f, ensure_ascii=False, indent=2)

# 读取并筛选
with open('books_reviews.json', 'r', encoding='utf-8') as f:
...
for book in loaded_books:
    ...
    if not reviews:
        continue
    avg_rating = sum(r['评分'] for r in reviews) / len(reviews)
    if avg_rating > 4.7:
        print(f"  {book['书名']}: 平均评分 {avg_rating:.1f}")
```

**要点**：
- 嵌套 JSON 的读写和平坦数据一样用 `json.dump` / `json.load`
- 访问嵌套数据时用 `book['评论']` 获取评论列表，再遍历取评分
- 用 `sum() / len()` 计算平均值，注意处理评论列表为空的情况

### 练习三

**思路**：用 `openpyxl` 创建工作簿，第一个 Sheet 写入数据明细并设置表头样式，第二个 Sheet 写入统计数据。通过 `Font`、`PatternFill` 设置表头格式。
...
# 模拟爬取数据
products = [
    {'名称': 'Python教程', '价格': 59.9, '评分': 4.8, '销量': 1200},
    {'名称': 'JS高级指南', '价格': 79.0, '评分': 4.6, '销量': 800},
    {'名称': 'CSS秘籍', '价格': 45.0, '评分': 4.9, '销量': 600},
    {'名称': 'Go语言实战', '价格': 69.0, '评分': 4.5, '销量': 400},
    {'名称': 'Rust编程', '价格': 89.0, '评分': 4.7, '销量': 300},
]
...
# 写入数据
for p in products:
    ws1.append([p['名称'], p['价格'], p['评分'], p['销量']])
...
avg_price = sum(p['价格'] for p in products) / total_count
max_rating = max(p['评分'] for p in products)

ws2.append(['指标', '数值'])
ws2.append(['总条数', total_count])
ws2.append(['平均价格', round(avg_price, 2)])
ws2.append(['最高评分', max_rating])
...
wb.save('爬虫采集报告.xlsx')
print('报告已生成: 爬虫采集报告.xlsx')
```

**要点**：
- `Workbook()` 创建工作簿，`wb.active` 获取默认 Sheet，`wb.create_sheet()` 创建新 Sheet
- 表头样式通过 `Font`（字体）、`PatternFill`（背景色）、`Alignment`（对齐）组合设置
- 统计数据用 Python 内置的 `sum()`、`max()`、`len()` 计算后再写入 Excel
- `column_dimensions['A'].width` 设置列宽，让报表更美观
