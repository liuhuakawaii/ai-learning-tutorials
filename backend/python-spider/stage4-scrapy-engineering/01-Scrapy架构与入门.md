...
**思路**：按照 Scrapy 项目标准流程操作：创建项目 → 编写 Spider → 运行爬虫。Spider 只需定义 `name`、`start_urls` 和 `parse()` 方法，在 `parse()` 中用 CSS 选择器提取每条名言的文本和作者，使用 `yield` 逐条返回数据。
...
```python
# myquotes/spiders/quotes_spider.py
import scrapy
...
**要点**：
- `name` 是爬虫的唯一标识，运行时通过 `scrapy crawl {name}` 调用
- `start_urls` 中的 URL 会自动创建 GET 请求，响应传给 `parse()` 方法
- `yield` 逐条返回数据，Scrapy 引擎自动收集并交给 Pipeline
- `-o quotes.json` 参数让 Scrapy 自动导出为 JSON 文件，无需手动写文件逻辑

### 练习 2

**思路**：Scrapy Shell 是交互式调试工具，启动时自动请求指定 URL 并加载响应对象。在 Shell 中可以直接测试 CSS 选择器，确认能正确提取数据后再搬到 Spider 代码中。
...
**要点**：
- Scrapy Shell 启动后自动加载 `response` 对象，可以直接调用 `.css()` 方法
- `::text` 提取文本内容，`::attr(href)` 提取属性值
- `.get()` 返回第一个匹配结果，`.getall()` 返回所有匹配结果的列表
- 先在 Shell 中调试选择器，确认无误后再写到 Spider 文件中，效率远高于反复运行爬虫

### 练习 3

**思路**：在 `parse()` 方法中，提取完当前页数据后，查找"下一页"按钮的链接。如果存在，用 `yield response.follow()` 请求下一页，`callback` 指向 `self.parse` 实现循环调用。Scrapy 引擎会自动管理请求队列和去重。
...
# 预期结果：共 100 条名言（10 页 × 每页 10 条）
```

**要点**：
- `response.follow()` 自动处理相对路径（如 `/page/2/`），无需手动拼接完整 URL
- `callback=self.parse` 让下一页的响应仍然走 `parse()` 方法，形成循环
- Scrapy 内置去重机制，同一个 URL 不会被重复请求
- 翻页逻辑和数据提取在同一个 `parse()` 方法中，简洁清晰
