...
**思路**：使用 `scrapy startproject` 创建项目，在 `items.py` 中定义 `BookItem` 类规范数据结构，在 `spiders/books.py` 中编写三层回调（首页 → 分类 → 详情），通过 `meta` 传递分类信息，运行爬虫导出 JSON。
...
class BooksSpider(scrapy.Spider):
    ...
    def _cell(self, rows, idx):
        try:
            return rows[idx].css("td::text").get()
...
**要点**：
- `BookItem` 定义了数据结构的"契约"，访问未定义的字段会抛出 `KeyError`，比字典更安全
- 三层回调 `parse → parse_category → parse_detail` 实现首页→分类→详情的逐层深入
- `response.follow()` 自动处理相对路径，比 `scrapy.Request()` + `urljoin()` 更简洁
- `price_num` 存储清洗后的浮点数，方便后续排序和统计

### 练习 2

**思路**：在 `parse()` 方法中提取分类名称和链接，通过 `meta` 把分类名称传递给 `parse_category` 和 `parse_detail` 回调。`meta` 是在不同回调函数之间传递上下文数据的标准方式。
...
class BooksSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # 提取所有分类名称和链接
        for cat_link in response.css("div.side_categories ul.nav ul li a"):
            category_name = cat_link.css("::text").get("").strip()
            yield response.follow(
                cat_link,
                callback=self.parse_category,
                meta={"category": category_name},  # 传递分类名
            )

    def parse_category(self, response):
        category = response.meta["category"]  # 取出分类名
        for book in response.css("article.product_pod"):
            yield response.follow(
                book.css("h3 a::attr(href)").get(),
                callback=self.parse_detail,
                meta={"category": category},  # 继续传递到详情页
            )
        next_page = response.css("li.next a::attr(href)").get()
        if next_page:
            yield response.follow(
    ...
    def parse_detail(self, response):
        ...
        item["title"] = response.css("h1::text").get()
        item["price"] = response.css("p.price_color::text").get()
        item["category"] = response.meta["category"]  # 最终使用
        item["url"] = response.url
        yield item
```

**要点**：
- `meta` 是字典，可以传递任意键值对，在回调中通过 `response.meta["key"]` 取出
- 分类名只在列表页有，必须通过 `meta` 层层传递到详情页
- 翻页时也要带上 `meta`，否则翻到第二页时分类信息会丢失
- `meta` 类似 React 的 props 或 Vue 的路由参数，是跨回调传递数据的标准方式

### 练习 3

**思路**：在 `items.py` 中创建继承自 `ItemLoader` 的自定义类，配置 `default_output_processor` 和字段级的输入处理器。在 Spider 中用 `ItemLoader` 替代手动赋值，通过 `add_css()` 声明式提取数据，处理器链自动完成清洗。
...
class BookItemLoader(ItemLoader):
    ...
    price_num_in = MapCompose(extract_number)
...
class BooksSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        loader.add_css("price_num", "p.price_color::text")
        loader.add_css("description", "#product_description ~ p::text")
...
**要点**：
- `BookItemLoader` 继承 `ItemLoader`，通过 `default_output_processor = TakeFirst()` 确保每个字段取第一个值
- `title_in = MapCompose(str.strip)` 定义输入处理器，在数据进入时自动去空格
- `add_css()` 是声明式提取，比 `item["title"] = response.css(...).get()` 更简洁
- `add_value()` 用于添加非选择器来源的数据（如 `meta` 中的分类名）
- ItemLoader 特别适合提取逻辑包含多步清洗的场景（去空格→正则→类型转换）
