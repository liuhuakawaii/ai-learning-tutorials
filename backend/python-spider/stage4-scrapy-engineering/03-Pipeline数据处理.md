...
**思路**：在 `process_item()` 中用字符串的 `replace()` 方法依次去除常见货币符号（¥、$、€）和千分位逗号，然后用 `float()` 转换为浮点数。用 `try/except` 捕获转换失败的情况，设置默认值 `0.0`。
    ...
    def process_item(self, item, spider):
        if 'price' in item:
            ...
            try:
                ...
                spider.logger.warning(f"价格转换失败: {item.get('title')} "
                                      f"原始值: {item['price']}")
                item['price'] = 0.0
        return item
```
...
**要点**：
- 遍历货币符号列表比逐个 `replace()` 更易扩展
- 转换失败时记录日志，方便排查哪些数据有问题
- 必须 `return item`，否则后续 Pipeline 收不到数据
- 价格清洗是 Pipeline 最常见的用途之一，放在优先级最低（数字最小）的位置

### 练习 2

**思路**：在 `process_item()` 中逐项检查数据完整性：标题非空、价格大于 0、评分在 1-5 范围内。不满足条件的用 `raise DropItem()` 丢弃，同时通过 `spider.logger` 记录日志便于排查。
...
class BookValidatePipeline:
    """书本数据验证 Pipeline"""

    def process_item(self, item, spider):
        # 标题不能为空
        if not item.get('title') or not item['title'].strip():
            raise DropItem(f"缺少标题，丢弃: {item.get('url', 'unknown')}")
        ...
        if not isinstance(price, (int, float)):
            try:
                ...
                price = 0
        if price <= 0:
            raise DropItem(f"价格无效({price})，丢弃: {item['title']}")

        # 评分必须在 1-5 之间（0 表示无评分，也允许通过）
        rating = item.get('rating', 0)
        if rating != 0 and not (1 <= rating <= 5):
            raise DropItem(f"评分超出范围({rating})，丢弃: {item['title']}")

        spider.logger.debug(f"验证通过: {item['title']} "
                            f"价格={price} 评分={rating}")
        return item
```

**要点**：
- `raise DropItem()` 会阻止 Item 传递给后续 Pipeline，Scrapy 自动记录 `item_dropped_count`
- 评分允许为 0（表示无评分），只对有评分的数据做范围校验
- 价格可能已经被前面的 `CleanPipeline` 转为 `float`，也可能是原始字符串，需要兼容处理
- 验证 Pipeline 的优先级应高于存储 Pipeline，确保不合格数据不会被存入

### 练习 3

**思路**：创建三个 Pipeline 类分别负责清洗、验证、存储，在 `settings.py` 中通过 `ITEM_PIPELINES` 配置优先级顺序（300 → 400 → 500）。运行爬虫验证整条链是否正常工作。
...
class JsonSavePipeline:
    ...
    def close_spider(self, spider):
        ...
        spider.logger.info(f"已保存 {len(self.items)} 条数据到 output.json")
...
# 运行爬虫验证
scrapy crawl books
# 检查 output.json 中的数据是否已清洗、验证、正确保存
```

**要点**：
- 优先级数字越小越先执行：清洗（300）→ 验证（400）→ 存储（500）
- 每个 Pipeline 只做一件事，遵循单一职责原则
- `open_spider()` 初始化资源，`close_spider()` 释放资源并写入文件
- `process_item()` 必须返回 `item` 或抛出 `DropItem`，返回 `None` 会导致错误
- 验证 Pipeline 丢弃的 Item 不会到达存储 Pipeline，Scrapy 自动记录丢弃数量
