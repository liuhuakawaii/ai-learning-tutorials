...
你在用 BeautifulSoup 的 `find_all()` 提取数据时遇到了一个问题：目标元素没有唯一的 class 或 id，只能通过层级关系和文本内容来定位。比如你想找到"价格大于 100 的商品"，或者"某个分类下的第一个子元素"，用 `find()` 和 `find_all()` 组合起来写了一堆嵌套代码，既难读又容易出错。

CSS 选择器和 XPath 是两种更强大的元素定位方式。CSS 选择器你作为前端开发者已经很熟了，而 XPath 则提供了按文本匹配、向上查找父元素等 CSS 做不到的能力。掌握这两种选择器，你就能应对几乎所有的网页结构。
...
- **盲目使用 DevTools 复制的选择器。** DevTools 自动生成的选择器通常包含 `:nth-child()` 和深层级路径，网页结构稍有变化就会失效。复制后一定要手动简化，用 class 和语义化标签替代。
- **认为 CSS 选择器和 XPath 只能选一个用。** 两者可以混合使用——先用 CSS 选择器定位大容器，再用 XPath 在容器内做精细提取。根据场景选择最方便的那个。
- **在 XPath 循环中忘记加 `.` 前缀。** 循环体内用 `//` 会在整个文档中查找，而不是在当前元素内查找。必须用 `.//` 表示"从当前节点开始"，否则结果会出错。
- **过度依赖文本匹配。** XPath 的 `text()` 匹配对空格和换行敏感，网页改版时文案稍有变化就会失效。文本匹配适合做辅助条件，不适合作为唯一的定位依据。

---

## 工程建议

- **80% 的场景用 CSS 选择器就够了。** CSS 选择器语法简洁，前端开发者已经熟悉，大多数元素定位任务都能胜任。只有需要按文本查找或向上查找父元素时才切换到 XPath。
- **在 DevTools 控制台中先测试选择器再写代码。** 用 `document.querySelectorAll()` 测试 CSS 选择器，用 `$x()` 测试 XPath，确认能选到目标元素后再移植到 Python 代码中。
- **选择器写得越短越好，但要能唯一定位到目标。** 短选择器更稳定、更可读。优先用 class 和标签名组合，避免写满层级路径。`.product-card .price` 比 `#app > main > div:nth-child(2) > div > span.price` 好得多。
- **Scrapy 项目中两种都掌握。** Scrapy 的选择器同时支持 CSS 和 XPath，且 XPath 性能略优。如果你计划学习 Scrapy，建议提前熟悉 XPath 的常用语法。
...
**思路**：构造一个包含导航栏和商品列表的 HTML，然后用 CSS 选择器完成 5 个定位任务。
...
<nav class="main-nav">
    <a href="/" class="nav-link active">首页</a>
    <a href="/products" class="nav-link has-sub">商品</a>
    <a href="/about" class="nav-link">关于</a>
</nav>
<div class="product-list">
    <div class="product-card" data-category="electronics">
        <a href="/product/1" target="_blank">蓝牙耳机</a>
        <span class="price sale">¥ 89</span>
    </div>
    <div class="product-card" data-category="books">
        <a href="/product/2">Python教程</a>
        <span class="price">¥ 59</span>
    </div>
    <div class="product-card" data-category="electronics">
        <a href="/product/3" target="_blank">机械键盘</a>
...
# 1. 选中当前激活的导航项
active_nav = soup.select('.nav-link.active')
print("激活的导航项:", [a.get_text() for a in active_nav])

# 2. 选中所有有下级菜单的链接
has_sub = soup.select('.nav-link.has-sub')
print("有下级菜单:", [a.get_text() for a in has_sub])

# 3. 选中所有打折商品的价格
sale_prices = soup.select('.price.sale')
print("打折价格:", [s.get_text() for s in sale_prices])

# 4. 选中特定 data-category 下的商品链接
electronics_links = soup.select('[data-category="electronics"] a')
print("电子产品链接:", [a.get_text() for a in electronics_links])

# 5. 选中在新窗口打开的链接
new_tab_links = soup.select('a[target="_blank"]')
print("新窗口打开:", [a.get_text() for a in new_tab_links])
```

**要点**：
- `.class1.class2` 选择同时包含多个 class 的元素
- `[attr="val"]` 属性选择器可以精确匹配 data-* 属性
- 选择器可以组合使用，用空格表示后代关系，用逗号表示"或"关系

### 练习二

**思路**：构造一个 FAQ 页面 HTML，用 XPath 的 `contains(text(), "...")` 和位置索引来定位问题和答案。
...
<div class="faq-list">
    <div class="faq-item">
        <h3 class="question">如何安装 Python？</h3>
        <div class="answer">访问 python.org 下载安装包...</div>
    </div>
    <div class="faq-item">
        <h3 class="question">pip 是什么？</h3>
        <div class="answer">pip 是 Python 的包管理工具，用于安装第三方库...</div>
    </div>
    <div class="faq-item">
        <h3 class="question">虚拟环境有什么用？</h3>
        <div class="answer">虚拟环境可以隔离不同项目的依赖...</div>
    </div>
    <div class="faq-item">
        <h3 class="question">如何配置开发环境？</h3>
        <div class="answer">推荐使用 VS Code 或 PyCharm 作为开发工具...</div>
    </div>
</div>
"""

tree = etree.HTML(faq_html)

# 1. 找到包含特定关键词的问题标题
questions = tree.xpath('//h3[contains(text(), "Python")]')
print("包含 Python 的问题:")
for q in questions:
    print(f"  - {q.xpath('./text()')[0]}")

# 2. 找到答案中包含特定文本的问题
matched = tree.xpath('//div[@class="answer"][contains(text(), "包管理")]/preceding-sibling::h3')
print("\n答案含'包管理'的问题:")
for q in matched:
    print(f"  - {q.xpath('./text()')[0]}")

# 3. 用位置索引找到第 N 个问答 pair
n = 2  # 第 2 个
question_n = tree.xpath(f'(//div[@class="faq-item"])[{n}]/h3/text()')[0]
answer_n = tree.xpath(f'(//div[@class="faq-item"])[{n}]/div/text()')[0]
print(f"\n第{n}个问答: 问题={question_n}, 答案={answer_n}")
```

**要点**：
- `contains(text(), "关键词")` 是 XPath 独有的文本匹配能力，CSS 选择器做不到
- `preceding-sibling::` 可以向上查找同级兄弟节点
- XPath 位置索引从 1 开始（不是 0），用 `()` 包裹表达式后加 `[N]`

### 练习三

**思路**：构造商品评论区 HTML，混合使用 CSS 选择器和 XPath 提取评论数据。
...
<div class="review-section">
    <div class="review" data-rid="1001">
        <span class="username">小明</span>
        <span class="rating">★★★★★</span>
        <p class="content">非常好用，质量不错！</p>
    </div>
    <div class="review" data-rid="1002">
        <span class="username">小红</span>
        <span class="rating">★★★★☆</span>
        <p class="content">性价比很高，推荐购买。</p>
    </div>
    <div class="review" data-rid="1003">
        <span class="username">小刚</span>
        <span class="rating">★★★☆☆</span>
        <p class="content">一般般，有待改进。</p>
    </div>
</div>
"""

# 用 CSS 选择器定位评论列表容器和评论项
soup = BeautifulSoup(review_html, 'lxml')
reviews = soup.select('.review')

# 用 XPath 提取详细数据（使用 lxml）
tree = etree.HTML(review_html)
review_elements = tree.xpath('//div[@class="review"]')

print("=== 混合使用示例 ===")
for elem in review_elements:
    # XPath 提取用户名、评分、评论内容
    username = elem.xpath('.//span[@class="username"]/text()')[0]
    rating = elem.xpath('.//span[@class="rating"]/text()')[0]
    content = elem.xpath('.//p[@class="content"]/text()')[0]
    rid = elem.xpath('./@data-rid')[0]
    print(f"[{rid}] {username} | {rating} | {content}")

# 思考：在这个场景下 CSS 选择器更适合快速定位容器，
# 而 XPath 适合精确提取每个字段的文本和属性
```

**要点**：
- CSS 选择器和 XPath 可以混合使用，不必死守一种
- CSS 选择器语法简洁，适合快速筛选元素
- XPath 的 `.//` 在循环体内表示"从当前节点开始查找"，不要漏掉 `.`
- 选择哪种取决于场景：简单定位用 CSS，需要文本匹配或向上查找用 XPath
