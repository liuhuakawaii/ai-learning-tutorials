...
**思路**：分别用 `find_all()` 和 `select()` 两种方式遍历新闻列表，提取每条新闻的标题、链接和日期。
...
for i, article in enumerate(articles, 1):
    ...
    print(f"序号{i}: {title} | 链接: {href} | 日期: {date}")
...
for i, article in enumerate(articles, 1):
    ...
    href = article.select_one('h2 a').get('href')
    date = article.select_one('.date').get_text(strip=True)
    print(f"序号{i}: {title} | 链接: {href} | 日期: {date}")
```

**要点**：
- `find_all()` 用 `class_` 参数过滤（带下划线，因为 class 是 Python 关键字）
- `select()` 用 CSS 选择器语法，`.news-item` 匹配 class
- 先定位容器再提取子元素，比直接在整个页面搜索更精准

### 练习二

**思路**：解析表格 HTML，遍历每一行提取姓名和各科成绩，计算总分并找出最高分学生。
...
for row in rows:
    ...
    scores = [int(cell.get_text(strip=True)) for cell in cells[1:]]
    total = sum(scores)
    print(f"{name}: 语文{scores[0]} 数学{scores[1]} 英语{scores[2]} 总分{total}")
    if total > best_score:
        best_score = total
        best_student = name

print(f"\n总分最高: {best_student}，总分: {best_score}")
```

**要点**：
- `tbody tr` 选择器定位数据行，跳过表头行
- `find_all('td')` 获取一行中的所有单元格
- 文本提取后需要 `int()` 转换才能做数值计算
- 表格数据结构化程度高，是爬虫最容易处理的数据格式

### 练习三

**思路**：遍历所有 `<a>` 标签，用 `.get('href')` 安全获取链接，过滤掉没有 href、href 为空、以及非 http/相对路径的链接。
...
for link in all_links:
    href = link.get('href')  # 安全获取，不存在返回 None
    if not href:
        continue
...
**要点**：
- 使用 `.get('href')` 而非 `['href']`，属性不存在时返回 None 而非抛 KeyError
- `href` 为空字符串 `""` 时需要额外判断（空字符串不是 None，但也不有效）
- `startswith()` 判断链接是否为有效格式（绝对 URL 或相对路径）
