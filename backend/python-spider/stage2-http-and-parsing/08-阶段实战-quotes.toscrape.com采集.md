...
前面七课你学了 HTTP 协议、requests 请求、HTML 结构、BeautifulSoup 解析、CSS 选择器与 XPath、分页策略、数据导出。这些知识点就像一堆散落的乐高积木——每块你都会用，但还没拼成过一个完整的作品。

这一课就是你的"毕业设计"。我们要以 `quotes.toscrape.com` 这个专为爬虫学习者搭建的网站为靶场，从零开始完成一个真实的全站采集项目：分析页面结构、编写解析代码、实现自动翻页、导出 CSV 和 JSON 文件，还要加上错误处理和进度日志。走完这个流程，你就具备了独立开发爬虫的完整能力。
...
- **把所有代码写在一个函数里。** 请求、解析、导出、日志全部堆在 main() 里，代码超过 100 行就难以维护。按功能拆分成独立函数（fetch、parse、save、analyze），每个函数只做一件事。
- **不加错误处理就上线运行。** 网络请求随时可能超时、断连或返回异常状态码。不加 try/except 和重试机制，爬虫跑几分钟就崩是常态，不是意外。
- **忽略礼貌爬取规则。** 不设延迟连续请求、不带 User-Agent、不遵守 robots.txt，这些行为可能导致你的 IP 被封禁，甚至引发法律风险。
- **只做采集不做验证。** 爬完数据直接存文件，不检查采集数量是否正确、字段是否完整、数据是否有重复。等用数据时才发现问题，已经浪费了大量时间。

---

## 工程建议

- **先在浏览器中完成页面分析，再写代码。** 用 DevTools 确定目标元素的 CSS 选择器、翻页按钮的结构、数据字段的位置。把选择器记录下来再动手写代码，比边写边猜高效得多。
- **用 `collections.Counter` 做快速数据验证。** 采集完成后统计一下作者数量、标签分布、总条数，和网站上显示的数据做对比。数据量不对说明翻页逻辑有问题，需要排查。
- **从一开始就养成模块化编码习惯。** 即使是小项目，也按 fetch → parse → save → analyze 的模块拆分。这样后续扩展（比如加数据库导出、加命令行参数）时，只需要新增模块，不用重写整个代码。
- **保留完整的运行日志。** 打印每页的采集条数、累计条数、耗时信息。出问题时能快速定位是哪一页、哪个环节出了问题，而不是对着一堆数据发呆。
...
**思路**：将第八节的完整代码保存为 `quotes_spider.py`，直接运行即可。运行前确保已安装 `requests` 和 `beautifulsoup4` 依赖。
...
def save_to_csv(data, filepath):
    with open(filepath, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        writer.writeheader()
        for item in data:
            writer.writerow({'text': item['text'], 'author': item['author'], 'tags': ', '.join(item['tags'])})
    print(f'CSV 已保存: {filepath}（共 {len(data)} 条）')
...
def main():
    ...
    if all_quotes:
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        save_to_csv(all_quotes, os.path.join(OUTPUT_DIR, f'quotes_{ts}.csv'))
        save_to_json(all_quotes, os.path.join(OUTPUT_DIR, f'quotes_{ts}.json'))
    return all_quotes
...
**要点**：
- 运行前确认已执行 `pip install requests beautifulsoup4`
- 程序会自动创建 `output` 目录并保存 CSV 和 JSON 文件
- 预期采集 100 条名言（10 页 × 10 条/页）
- CSV 文件用 Excel 打开时中文正常显示（因为使用了 `utf-8-sig` 编码）

### 练习二

**思路**：每条名言下方有 `(about)` 链接指向作者详情页 `/author/Albert-Einstein`。需要提取该链接的 `href`，请求详情页后解析 `author-born-date`、`author-born-location` 和 `author-description` 三个元素。
...
def scrape_with_author_info():
    ...
    while url:
        ...
        for div in soup.select('div.quote'):
            text = div.select_one('span.text').get_text()
            author = div.select_one('small.author').get_text()
            tags = [t.get_text() for t in div.select('div.tags a.tag')]

            # 获取作者详情（带缓存，避免重复请求）
            if author not in author_cache:
                about_link = div.select_one('a[href*="/author/"]')
                if about_link:
                    author_url = BASE_URL + about_link['href']
                    author_soup = fetch_page(author_url)
                    if author_soup:
                        author_cache[author] = get_author_details(author_soup)
                    time.sleep(0.5)
                else:
                    author_cache[author] = {'born_date': '', 'born_location': '', 'description': ''}

            details = author_cache[author]
            all_quotes.append({
                'text': text,
                'author': author,
                'tags': tags,
    ...
    os.makedirs('output', exist_ok=True)
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    with open(f'output/quotes_with_author_{ts}.json', 'w', encoding='utf-8') as f:
        json.dump(all_quotes, f, ensure_ascii=False, indent=2)
    print(f'共采集 {len(all_quotes)} 条名言（含作者详情）')
    return all_quotes

if __name__ == '__main__':
    scrape_with_author_info()
```

**要点**：
- 作者详情页的 CSS 选择器：`.author-born-date`、`.author-born-location`、`.author-description`
- 使用 `author_cache` 字典缓存已查询的作者信息，避免同一个作者被请求多次
- 每次请求作者详情页后加 0.5 秒延迟，保持礼貌爬取
- `(about)` 链接的 `href` 格式为 `/author/Albert-Einstein`

### 练习三

**思路**：使用 `argparse` 模块定义命令行参数，支持 `--pages`（限制采集页数）、`--format`（导出格式）、`--output`（输出文件名前缀）。在 `main()` 函数中读取参数并控制采集逻辑。

**答案**：
```python
"""
quotes_spider.py -- 支持命令行参数的全站采集器
用法:
  python quotes_spider.py
  python quotes_spider.py --pages 3
  python quotes_spider.py --format csv
  python quotes_spider.py --output mydata
"""
import requests
from bs4 import BeautifulSoup
import csv, json, os, time
...
def main():
    ...
    parser.add_argument('--pages', type=int, default=0, help='采集页数，0 表示全部（默认 0）')
    parser.add_argument('--format', choices=['csv', 'json', 'both'], default='both', help='导出格式（默认 both）')
    parser.add_argument('--output', type=str, default='quotes', help='输出文件名前缀（默认 quotes）')
    ...
    while url:
        if args.pages > 0 and page_num > args.pages:
            print(f'已达到指定页数 {args.pages}，停止采集')
            break
    ...
    if all_quotes:
        ...
        if args.format in ('csv', 'both'):
            save_to_csv(all_quotes, f'output/{args.output}_{ts}.csv')
        if args.format in ('json', 'both'):
            save_to_json(all_quotes, f'output/{args.output}_{ts}.json')
        print(f'\n共采集 {len(all_quotes)} 条名言')
    else:
        print('未采集到任何数据')
...
**要点**：
- `argparse.ArgumentParser` 定义命令行参数，`--pages` 控制采集页数，`--format` 控制导出格式，`--output` 控制文件名前缀
- `args.pages` 默认为 0 表示采集全部页面，大于 0 时在循环中检查页数并提前退出
- `args.format` 使用 `choices` 限定可选值，避免用户输入无效格式
- 所有输出文件统一放在 `output/` 目录下，文件名包含时间戳避免覆盖
