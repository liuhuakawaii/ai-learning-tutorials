...
**思路**：使用封装好的 `cache` 工具类，对用户信息和文章列表分别设置缓存键和 TTL。更新用户信息时清除所有相关缓存，避免脏读。
...
const CACHE_KEYS = {
  ...
  USER_POSTS: (id: number) => `user:${id}:posts`,
} as const;

const CACHE_TTL = {
  USER_DETAIL: 600,   // 10 分钟
  USER_POSTS: 300,    // 5 分钟
};

/**
 * GET /api/users/:id
 * 获取用户信息（带缓存，TTL 10 分钟）
 */
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
    ...
    // 3. 写入缓存
    await cache.set(cacheKey, user, CACHE_TTL.USER_DETAIL);
...
router.put('/:id', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // 2. 清除该用户的所有相关缓存
    await Promise.all([
      cache.del(CACHE_KEYS.USER_DETAIL(id)),
      cache.delPattern(`user:${id}:posts:*`),
    ]);

    console.log(`🗑️ 已清除用户 ${id} 的相关缓存`);
...
router.get('/:id/posts', async (req: Request, res: Response) => {
  try {
    ...
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
    const cacheKey = `${CACHE_KEYS.USER_POSTS(id)}:${page}:${limit}`;
    ...
    // 3. 写入缓存
    await cache.set(cacheKey, result, CACHE_TTL.USER_POSTS);
...
**要点**：
- 更新用户信息后用 `Promise.all` 并行清除详情缓存和文章列表缓存，避免遗漏
- 文章列表缓存键包含分页参数 `page:limit`，不同分页结果互不干扰
- 缓存键命名统一用 `user:{id}` 前缀，方便批量清除（`delPattern`）

### 练习 2：API 限流

**思路**：使用 `createRateLimiter` 函数为不同接口配置不同的窗口时间和最大请求数。密码重置接口按邮箱限流而非按 IP，防止同一 IP 对多个邮箱发起攻击。

**答案**：

```typescript
// src/middleware/rate-limiter.ts（追加到 rateLimiters 对象中）

import { Request } from 'express';

export const rateLimiters = {
  // 文章列表：每分钟 100 次（读多写少，放宽限制）
  getPosts: createRateLimiter({
    windowMs: 60 * 1000,
    maxRequests: 100,
    message: '文章列表请求过于频繁，请稍后再试',
  }),
  ...
  // 密码重置：每小时 3 次（防滥用，按邮箱限流）
  forgotPassword: createRateLimiter({
    windowMs: 60 * 60 * 1000,
    maxRequests: 3,
    keyGenerator: (req: Request) => `forgot:${req.body.email || req.ip}`,
...
// 文章列表限流
app.get('/api/posts', rateLimiters.getPosts, postController.list);

// 发评论限流
app.post('/api/posts/:id/comments', rateLimiters.createComment, commentController.create);

// 密码重置限流
app.post('/api/auth/forgot-password', rateLimiters.forgotPassword, authController.forgotPassword);
```

**要点**：
- `keyGenerator` 决定限流维度：按 IP 限流防匿名攻击，按用户 ID 限流防注册用户滥用
- 密码重置按邮箱限流（而非 IP），因为攻击者可能用不同 IP 对同一邮箱暴力重试
- 全局限流放在最前面，特定路由限流叠加生效，双重保护

### 练习 3：阅读量统计

**思路**：用 Redis `INCR` 原子递增记录增量阅读量，用定时器每 5 分钟通过 `GETSET` 获取并重置增量，将增量累加到数据库中的总量上。读取时返回数据库总量 + Redis 增量。
...
/**
 * 增加文章阅读量（在 Redis 中原子递增）
 * 用户访问文章时调用
 */
export async function incrementView(postId: number): Promise<void> {
  const deltaKey = `post:${postId}:views:delta`;
  await redis.incr(deltaKey);
}

/**
 * 获取文章总阅读量（数据库总量 + Redis 增量）
 */
export async function getTotalViews(postId: number): Promise<number> {
  const deltaKey = `post:${postId}:views:delta`;

  // 并行获取数据库总量和 Redis 增量
  const [post, delta] = await Promise.all([
    prisma.post.findUnique({ where: { id: postId }, select: { viewCount: true } }),
    redis.get(deltaKey),
  ]);

  const dbViews = post?.viewCount || 0;
  const redisDelta = delta ? parseInt(delta) : 0;

  return dbViews + redisDelta;
}

/**
 * 将 Redis 中的阅读量增量同步到数据库
 */
async function syncViewCounts(): Promise<void> {
  console.log('📊 开始同步阅读量...');

  // 获取所有增量键
  const keys = await redis.keys('post:*:views:delta');
  let syncCount = 0;

  for (const key of keys) {
    // GETSET：获取当前值并重置为 0（原子操作）
    const delta = await redis.getset(key, '0');

    if (delta && parseInt(delta) > 0) {
      const postId = parseInt(key.split(':')[1]);

      // 将增量累加到数据库
      await prisma.post.update({
        where: { id: postId },
        data: { viewCount: { increment: parseInt(delta) } },
      });
  ...
  console.log(`📊 阅读量同步完成，更新了 ${syncCount} 篇文章`);
}
...
export function startViewSync(): void {
  ...
  syncTimer = setInterval(syncViewCounts, SYNC_INTERVAL);
  console.log(`📊 阅读量同步任务已启动（每 ${SYNC_INTERVAL / 1000 / 60} 分钟）`);
}

/**
 * 停止定时同步任务
 */
export function stopViewSync(): void {
  if (syncTimer) {
...
```typescript
// src/routes/post.routes.ts（在获取文章详情时调用）

import { incrementView, getTotalViews } from '../services/view-counter.service';

router.get('/:id', async (req: Request, res: Response) => {
  const id = parseInt(req.params.id);

  // 查缓存或数据库获取文章
  const post = await getPostFromCacheOrDB(id);
  if (!post) return res.status(404).json({ error: '文章不存在' });

  // 异步增加阅读量（不阻塞响应）
  incrementView(id).catch(err => console.error('增加阅读量失败:', err));

  // 获取实时总阅读量
  const views = await getTotalViews(id);

  res.json({ ...post, viewCount: views });
});
...
router.get('/:id/views', async (req: Request, res: Response) => {
  const id = parseInt(req.params.id);
  const views = await getTotalViews(id);
  res.json({ postId: id, viewCount: views });
});
```
...
import { startViewSync, stopViewSync } from './services/view-counter.service';
...
**要点**：
- `INCR` 是 Redis 原子操作，天然支持高并发，不会出现计数丢失
- `GETSET` 一次性获取并重置增量，避免并发下重复累加
- 定时同步而非每次阅读都写数据库，大幅减少数据库写入压力
- 读取时返回 `数据库总量 + Redis 增量`，保证数据实时性
