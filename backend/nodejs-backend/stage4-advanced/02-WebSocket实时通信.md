...
你的博客平台支持评论功能，用户 A 在一篇文章下发表了评论，但文章作者用户 B 并不知道——他只能每隔几分钟手动刷新页面查看是否有新评论。用户 B 抱怨："为什么不能像微信一样有实时通知？"问题的根源在于 HTTP 协议的单向性：只有客户端能主动请求服务器，服务器无法主动推送消息给客户端。传统的解决方案是轮询（每隔几秒请求一次），但这会浪费大量带宽和服务器资源。WebSocket 协议通过一次握手建立持久连接，之后双方可以随时互发消息，完美解决了实时通信的需求。本课将用 Socket.IO 为博客平台添加实时评论、实时通知和在线统计功能。
...
**思路**：为每个用户创建一个以 userId 命名的 Socket.IO 房间，发送私信时将消息推送到目标用户的房间。离线消息用 Redis 列表缓存，上线后自动补发。
...
const MESSAGE_TTL = 3600 * 24 * 7; // 离线消息保留 7 天
const MAX_OFFLINE_MESSAGES = 200;
...
export async function sendPrivateMessage(data: {
  ...
  if (receiverSockets.length > 0) {
    // 接收者在线，实时推送
    io.to(`user:${receiverId}`).emit('message:private', messagePayload);
    console.log(`📨 私信已推送给用户 ${receiverId}`);
  } else {
    // 接收者离线，缓存消息
    await cacheOfflineMessage(receiverId, messagePayload);
    console.log(`📦 用户 ${receiverId} 离线，私信已缓存`);
  }
 ...
 * 缓存离线私信
 */
async function cacheOfflineMessage(userId: number, message: any): Promise<void> {
  const key = `offline:messages:${userId}`;
  await redis.lpush(key, JSON.stringify(message));
 ...
 * 获取并清除离线消息（用户上线时调用）
 */
export async function getAndClearOfflineMessages(userId: number): Promise<any[]> {
  const key = `offline:messages:${userId}`;
  const messages = await redis.lrange(key, 0, -1);
...
io.on('connection', (socket) => {
  ...
  socket.on('message:private', async (data: { receiverId: number; content: string }) => {
    try {
      ...
      socket.emit('error', '发送私信失败');
    }
  });

  // 上线时补发离线消息
  getAndClearOfflineMessages(userId).then(messages => {
    for (const msg of messages) {
      socket.emit('message:private', msg);
    }
    if (messages.length > 0) {
      console.log(`📬 为 ${username} 补发了 ${messages.length} 条离线私信`);
    }
  });
});
```
...
**要点**：
- 每个用户有专属房间 `user:{userId}`，发送私信时用 `io.to()` 精确推送到目标用户
- 离线消息用 Redis List 缓存，`LPUSH` 保证最新消息在前，`LTRIM` 限制最大条数
- 历史消息通过 REST API 分页查询，实时消息通过 WebSocket 推送，两者互补

### 练习 2：文章点赞实时通知

**思路**：用 Redis Set 存储每篇文章的点赞用户集合，`SISMEMBER` 防止重复点赞，`SCARD` 获取点赞数。点赞时通过 Socket.IO 实时通知文章作者。
 ...
 * 点赞文章
 */
export async function likePost(userId: number, postId: number): Promise<{
  liked: boolean;
  likeCount: number;
}> {
  const likeKey = `post:${postId}:likes`;

  // 1. 检查是否已点赞（SISMEMBER 时间复杂度 O(1)）
  const alreadyLiked = await redis.sismember(likeKey, userId.toString());
  if (alreadyLiked) {
    // 已点赞则取消点赞
    await redis.srem(likeKey, userId.toString());
  ...
  // 2. 添加点赞记录到 Redis Set
  await redis.sadd(likeKey, userId.toString());
  const likeCount = await redis.scard(likeKey);

  // 3. 异步保存到数据库（可选，也可以定时批量同步）
  prisma.like.upsert({
    where: { userId_postId: { userId, postId } },
    create: { userId, postId },
    update: {},
  }).catch(err => console.error('保存点赞记录失败:', err));

  // 4. 通知文章作者（如果点赞者不是作者本人）
  ...
  if (post && post.authorId !== userId) {
    ...
    const io = getIO();
    const authorSockets = await io.in(`user:${post.authorId}`).fetchSockets();
    ...
    if (authorSockets.length > 0) {
      io.to(`user:${post.authorId}`).emit('notification:new', {
        ...
        type: 'like',
        title: '收到新点赞',
        message: `${liker?.name || '某用户'} 赞了你的文章《${post.title}》`,
        link: `/posts/${postId}`,
        createdAt: new Date().toISOString(),
      });
    }
  ...
  // 5. 广播最新的点赞数给文章房间内的用户
  const io = getIO();
  io.to(`post:${postId}`).emit('like:updated', { postId, likeCount, liked: true });
...
export async function getLikeCount(postId: number): Promise<number> {
  return redis.scard(`post:${postId}:likes`);
}

/**
 * 检查用户是否已点赞
 */
export async function hasLiked(userId: number, postId: number): Promise<boolean> {
  const result = await redis.sismember(`post:${postId}:likes`, userId.toString());
  return result === 1;
}
```
...
import { Router, Request, Response } from 'express';
import { likePost, getLikeCount, hasLiked } from '../services/like.service';
import { authMiddleware } from '../middleware/auth';
...
// 获取点赞状态和数量
router.get('/posts/:postId/like', async (req: Request, res: Response) => {
  const postId = parseInt(req.params.postId);
  const userId = (req as any).session?.userId;

  const [likeCount, liked] = await Promise.all([
    getLikeCount(postId),
    userId ? hasLiked(userId, postId) : Promise.resolve(false),
  ]);

  res.json({ postId, likeCount, liked });
});
...
**要点**：
- Redis Set 的 `SADD`/`SISMEMBER`/`SCARD` 都是 O(1) 操作，高并发下性能极佳
- 点赞和取消点赞用同一个接口，通过 `SISMEMBER` 判断当前状态后切换
- 点赞数变化实时广播给文章房间内所有用户，所有人都能看到实时更新

### 练习 3：在线状态管理

**思路**：用 Redis Hash 存储用户在线状态，Socket.IO 心跳机制检测断连。客户端每 10 秒发送 ping，服务端记录最后心跳时间，定时扫描超时用户标记为离线。
...
const STATUS_KEY = 'user:presence';
const HEARTBEAT_KEY = 'user:heartbeat';
const HEARTBEAT_TIMEOUT = 30; // 30 秒无响应标记为离线
 ...
 * 设置用户在线状态
 */
export async function setUserStatus(userId: number, status: UserStatus): Promise<void> {
  await redis.hset(STATUS_KEY, userId.toString(), status);
  await redis.hset(HEARTBEAT_KEY, userId.toString(), Date.now().toString());
}
...
export async function getUserStatus(userId: number): Promise<UserStatus> {
  const status = await redis.hget(STATUS_KEY, userId.toString());
 ...
 * 批量获取用户在线状态
 */
export async function getMultipleUserStatus(
  userIds: number[]
  ...
  for (const id of userIds) {
    pipeline.hget(STATUS_KEY, id.toString());
  }
  const results = await pipeline.exec();

  const statusMap: Record<number, UserStatus> = {};
  userIds.forEach((id, index) => {
    statusMap[id] = (results?.[index]?.[1] as UserStatus) || 'offline';
  });
 ...
 * 更新心跳时间
 */
export async function updateHeartbeat(userId: number): Promise<void> {
  await redis.hset(HEARTBEAT_KEY, userId.toString(), Date.now().toString());
}
 ...
 * 清除用户在线状态（离线时调用）
 */
export async function clearUserStatus(userId: number): Promise<void> {
  await redis.hdel(STATUS_KEY, userId.toString());
  await redis.hdel(HEARTBEAT_KEY, userId.toString());
}
...
export async function checkHeartbeats(): Promise<void> {
  ...
  for (const [userId, lastBeat] of Object.entries(allHeartbeats)) {
    const elapsed = (now - parseInt(lastBeat)) / 1000;

    if (elapsed > HEARTBEAT_TIMEOUT) {
      // 超时，标记为离线
      await clearUserStatus(parseInt(userId));

      // 广播离线事件
      const io = getIO();
      io.emit('user:offline', { userId: parseInt(userId) });

      console.log(`🔴 用户 ${userId} 心跳超时（${Math.floor(elapsed)}秒），已标记离线`);
    }
  }
}
 ...
 * 获取所有在线用户
 */
export async function getOnlineUsers(): Promise<Array<{ userId: number; status: UserStatus }>> {
  const allStatus = await redis.hgetall(STATUS_KEY);
...
// 每 15 秒检查一次心跳超时
setInterval(checkHeartbeats, 15 * 1000);
...
io.on('connection', (socket) => {
  ...
  // 用户主动设置状态（离开/忙碌）
  socket.on('status:set', (status: 'online' | 'away' | 'busy') => {
    setUserStatus(userId, status);
    io.emit('user:status_changed', { userId, username, status });
  });
...
import { Router, Request, Response } from 'express';
import { getUserStatus, getMultipleUserStatus, getOnlineUsers } from '../services/presence.service';
...
router.post('/status/batch', async (req: Request, res: Response) => {
  ...
  const statuses = await getMultipleUserStatus(userIds);
  res.json(statuses);
});
...
**要点**：
- Redis Hash 存储状态，`HSET`/`HGET` O(1) 操作，支持批量查询（Pipeline）
- 心跳检测用定时器扫描而非每个用户单独计时，实现简单且资源消耗可控
- 客户端每 10 秒发送心跳，服务端 30 秒超时判定离线，留有网络波动余量
- 状态变更通过 `io.emit` 广播，所有在线用户都能收到好友状态变化

---

## 常见误区

1. **所有功能都用 WebSocket**：文章列表、用户信息这些不需要实时的数据也用 WebSocket 推送，增加了复杂度却没带来价值。WebSocket 适合真正的实时场景（聊天、通知、在线状态），REST 接口适合 CRUD 操作。

2. **不做认证就允许 WebSocket 连接**：任何人都能连上 WebSocket 服务器，接收广播消息。必须在握手阶段验证 JWT Token，拒绝未认证的连接。

3. **离线消息不处理**：用户下线期间收到的通知直接丢失，上线后一无所知。应该用 Redis 缓存离线消息，用户上线后自动补发。

4. **房间机制不做清理**：用户离开页面后没有触发 `leave:room`，房间里的 Socket 越来越多，广播范围越来越大。要在 `disconnect` 事件中自动清理房间。

---

## 工程建议

1. **事件名称集中管理**：用常量对象（如 `SOCKET_EVENTS`）统一管理所有事件名称，避免字符串拼写错误。前后端共享同一份事件定义。

2. **消息格式要统一**：所有消息都包含 `type`、`data`、`timestamp` 三个字段。客户端只需要一个分发函数就能处理所有类型的消息。

3. **心跳检测要及时发现断开**：Socket.IO 默认 25 秒发一次心跳，20 秒无响应才判定断开。对于在线状态这种场景，可能需要更短的心跳间隔。

4. **用 `fetchSockets()` 获取在线用户详情**：不要自己维护在线用户列表，Socket.IO 的 `fetchSockets()` API 能直接获取所有连接的 Socket 信息，包括 `socket.data` 中的用户数据。
