...
**思路**：按照 NestJS 模块化规范，创建 Module、Controller、Service、DTO 四个文件。Service 处理嵌套评论逻辑（parentId），Controller 只做参数接收和 Service 调用，用 DTO 验证输入。
...
export class CommentsService {
  ...
  async create(postId: number, userId: number, dto: CreateCommentDto) {
    ...
    if (dto.parentId) {
      ...
      if (!parentComment) {
        throw new NotFoundException(`父评论 ID ${dto.parentId} 不存在`);
      }
   ...
   * 获取评论的子回复（分页加载）
   */
  async findReplies(commentId: number, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit;
  ...
  async remove(commentId: number, userId: number) {
    const comment = await this.prisma.comment.findUnique({
      where: { id: commentId },
      select: { id: true, userId: true, _count: { select: { replies: true } } },
    });
    ...
    // 如果有子回复，软删除（保留结构）；否则硬删除
    if (comment._count.replies > 0) {
      await this.prisma.comment.update({
        where: { id: commentId },
        data: { content: '[该评论已删除]', isDeleted: true },
      });
    } else {
...
export class CommentsController {
  ...
  findReplies(
    ...
    return this.commentsService.findReplies(commentId, query.page, query.limit);
  }
...
**要点**：
- Controller 只做三件事：解析参数、调用 Service、返回响应，不包含业务逻辑
- 创建回复时验证父评论是否存在且属于同一文章，防止数据不一致
- 有子回复的评论采用软删除（标记 `isDeleted`），保留回复树结构
- DTO 配合 `ValidationPipe` 自动验证输入，`@Type(() => Number)` 转换查询参数类型

### 练习 2：自定义装饰器

**思路**：使用 NestJS 的 `createParamDecorator` 创建参数装饰器，从 `ExecutionContext` 中提取所需信息。装饰器让代码更声明式、更简洁。
 ...
 * @CurrentUser('email') email: string → 只获取 email 字段
 */
export const CurrentUser = createParamDecorator(
...
export const Pagination = createParamDecorator(
  (defaults: { page?: number; limit?: number } | undefined, ctx: ExecutionContext) => {
    const request = ctx.switchToHttp().getRequest();
 ...
 *
 * 使用方式：
 * @ApiDoc({ summary: '获取文章列表', auth: true, tag: '文章' })
 */
export function ApiDoc(options: ApiDocOptions) {
  ...
  if (options.status) {
    decorators.push(ApiResponse({ status: options.status }));
  }

  if (options.type) {
    decorators.push(ApiResponse({ status: 200, type: options.type }));
  }
...
export class PostsController {
  ...
  @Get()
  @ApiDoc({ summary: '获取文章列表', tag: '文章' })
  findAll(@Pagination({ page: 1, limit: 10 }) pagination: { page: number; limit: number; skip: number }) {
    return this.postsService.findAll(pagination);
  }

  @Post()
  @UseGuards(JwtAuthGuard)
  @ApiDoc({ summary: '创建文章', auth: true, status: 201, tag: '文章' })
  create(@Body() dto: CreatePostDto, @CurrentUser('userId') authorId: number) {
    return this.postsService.create(dto, authorId);
  }

  @Get('me')
  @UseGuards(JwtAuthGuard)
  @ApiDoc({ summary: '获取当前用户信息', auth: true, tag: '用户' })
...
**要点**：
- `createParamDecorator` 的第一个参数是装饰器传入的数据，第二个是执行上下文
- `@CurrentUser('email')` 传入字段名时只返回该字段，不传时返回完整用户对象
- `@Pagination()` 封装了分页参数解析、默认值设置和边界检查，避免在每个 Controller 中重复
- `@ApiDoc()` 用 `applyDecorators` 聚合多个装饰器，减少路由方法上的装饰器堆叠

### 练习 3：全局异常过滤器

**思路**：用 `@Catch()` 不指定异常类型来捕获所有异常。区分 `HttpException`（已知业务错误）和未知错误，对 Prisma 错误做特殊处理，生产环境隐藏内部细节。
...
/**
 * Prisma 错误码映射
 * 参考：https://www.prisma.io/docs/reference/api-reference/error-reference
 */
  ...
  private readonly logger = new Logger('ExceptionFilter');
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof exceptionResponse === 'string') {
        ...
        code = resp.error || 'HTTP_EXCEPTION';

        // class-validator 的错误是数组，提取所有消息
    ...
    // 3. 处理 Prisma 错误
      ...
      message = PRISMA_ERROR_MESSAGES[prismaException.code] || '数据库操作失败';

      // 唯一约束冲突，提取冲突字段
      if (prismaException.code === 'P2002' && prismaException.meta?.target) {
        const fields = prismaException.meta.target.join(', ');
        message = `字段 [${fields}] 的值已存在`;
      }
    }
    // 4. 处理其他未知错误
    else {
...
async function bootstrap() {
  ...
  app.useGlobalPipes(new ValidationPipe({
    ...
    forbidNonWhitelisted: true, // 有未定义属性时抛出错误
    transform: true,           // 自动转换类型
  }));
...
**要点**：
- `@Catch()` 不指定类型时捕获所有异常，包括非 `HttpException` 的意外错误
- Prisma 错误通过检查 `code` 前缀 `P` 来识别，`P2002` 是唯一约束冲突，提取 `meta.target` 告知用户哪个字段重复
- 生产环境不返回 `stack`，只返回安全的错误消息；开发环境返回完整堆栈方便调试
- 5xx 错误记录完整堆栈用于排查，4xx 错误只记录消息避免日志噪声
