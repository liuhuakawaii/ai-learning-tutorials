...
**思路**：认证中间件的核心逻辑是"提取 Token → 验证 Token → 挂载用户信息 → 放行"。需要处理三种错误情况：无 Token、Token 过期、Token 无效，分别返回不同的错误码让前端可以针对性处理。
...
**要点**：
- `extractToken` 负责从 `Bearer <token>` 格式中提取 Token，返回 null 表示无 Token
- `verifyAccessToken` 验证失败时抛出自定义错误（`ACCESS_TOKEN_EXPIRED` / `ACCESS_TOKEN_INVALID`），中间件根据错误类型返回不同的 `code`
- 返回 `code` 字段让前端可以程序化判断错误类型（如 `TOKEN_EXPIRED` 时自动刷新 Token）

### 练习 2：实现 requireRole 中间件

**思路**：用工厂函数模式，接收允许的角色列表作为参数，返回一个 Express 中间件。中间件检查 `req.user.role` 是否在允许列表中，不在则返回 403。
...
**要点**：
- 工厂函数模式让中间件可以复用：`requireRole('ADMIN')` 和 `requireRole('ADMIN', 'MODERATOR')` 使用同一套逻辑
- `requireAdmin` 和 `requireUser` 是常用组合的快捷方式
- 必须先检查 `req.user` 是否存在，防止 `authenticate` 中间件未执行时直接崩溃
- 403 表示"已认证但无权限"，与 401（未认证）严格区分

### 练习 3：实现评论的权限系统

**思路**：评论模块的权限设计与文章类似：浏览公开，创建需登录，编辑和删除需要所有权验证。编辑只允许作者，删除允许作者或管理员。
...
// 需要登录 + 是评论作者或管理员（allowAdmin 默认为 true）
router.delete(
  '/:id',
  authenticate,
  requireOwnership({ model: 'comment' }),
...
// src/middleware/requireOwnership.ts（需要在 switch 中添加 comment 模型）
// case 'comment':
...
**要点**：
- 编辑（PUT）和删除（DELETE）都需要 `requireOwnership`，但删除额外允许管理员操作
- `requireOwnership` 的 `allowAdmin` 默认为 `true`，管理员可以直接删除任何评论
- `req.resource` 挂载了查询到的资源，Controller 可以直接使用，避免重复查询

### 练习 4：设计一个更复杂的权限系统

**思路**：增加 EDITOR 和 MODERATOR 两个角色后，需要重新设计权限矩阵，更新 Role 枚举，调整中间件配置。关键是理清每个角色的核心职责边界。

**答案**：
```typescript
// 1. 更新 Role 枚举（prisma/schema.prisma）
enum Role {
  ADMIN
  EDITOR
  MODERATOR
...
// 2. 新权限矩阵
// ┌──────────────────────┬──────┬──────┬────────┬───────────┬───────┐
// │ 操作                  │ USER │ EDITOR│MODERATOR│ ADMIN    │
// ├──────────────────────┼──────┼──────┼────────┼───────────┤
// │ 浏览已发布文章         │  ✅  │  ✅  │  ✅    │  ✅      │
// │ 发表评论              │  ✅  │  ✅  │  ✅    │  ✅      │
// │ 发布文章              │  ✅  │  ✅  │  ❌    │  ✅      │
...
export const requireUser = requireRole('USER', 'EDITOR', 'MODERATOR', 'ADMIN')

// 4. 路由配置更新
// 文章编辑：EDITOR 和 ADMIN 可以编辑任何文章
router.put(
  '/articles/:id',
  authenticate,
...
// 评论删除：MODERATOR 和 ADMIN 可以删除任何评论
router.delete(
  '/comments/:id',
  authenticate,
...
**要点**：
- EDITOR 负责内容管理（编辑/审核文章），MODERATOR 负责社区管理（审核评论），职责不重叠
- `requireOwnership` 对 EDITOR 和 MODERATOR 应该设置 `allowAdmin: false`，因为它们不是 ADMIN
- 新增角色时，先画权限矩阵再写代码，矩阵是设计文档也是验收标准
- 使用 `requireRole('EDITOR', 'ADMIN')` 而非单独的 `requireEditor`，保持灵活性
