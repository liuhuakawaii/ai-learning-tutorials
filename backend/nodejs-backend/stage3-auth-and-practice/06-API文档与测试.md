...
你的博客 API 已经开发了十几个接口，前端同事开始对接。第一天他就跑来问："这个接口的参数是什么格式？""返回的字段有哪些？""认证 Token 放在 Header 还是 Cookie？"你口头解释了一遍，第二天他又忘了。三个月后你自己回来看代码，也忘了某个接口的细节。更糟糕的是，你改了一个接口的返回格式，没有通知前端，导致线上出了 bug。这些问题的根源是：没有 API 文档。同时，每次手动测试所有接口耗时耗力，改一行代码就担心会不会破坏其他功能。本课将教你用 Swagger 自动生成 API 文档，用 Vitest + supertest 编写自动化测试，让项目真正做到"文档即代码、代码即文档"。
...
**思路**：参考课程中认证接口和文章接口的 Swagger 注释风格，为评论的四个接口（列表、创建、更新、删除）编写完整的 JSDoc 注释，包含请求参数、请求体、响应格式和安全要求。
 ...
 *   get:
 *     summary: 获取评论列表
 *     description: 获取指定文章的评论列表，支持分页，包含嵌套回复
 *     tags: [评论]
 *     parameters:
 *       - in: query
 *         name: articleId
 ...
 *       422:
 *         description: 缺少必填参数 articleId
 */
router.get('/', optionalAuth, commentController.list.bind(commentController))
 ...
 *   post:
 *     summary: 创建评论
 *     description: 为指定文章创建评论，支持嵌套回复（需要登录）
 *     tags: [评论]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *   delete:
 *     summary: 删除评论
 *     description: 删除评论及其所有回复（需要登录且是评论作者或管理员）
 *     tags: [评论]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
**要点**：
- 每个接口都要写清楚 `tags`（分组）、`security`（是否需要认证）、`responses`（各种状态码）
- 请求参数用 `parameters` 描述，请求体用 `requestBody` 描述，格式要与 OpenAPI 规范一致
- `description` 字段要写业务含义，不能只重复 `summary`
- 错误响应也要写明（401、403、404、422），前端需要根据错误码做不同处理

### 练习 2：编写认证接口的集成测试

**思路**：参考课程中已有的注册、登录、获取用户信息测试，补充 Token 刷新、修改密码、更新个人资料的测试，以及边界情况测试（空字符串、超长输入等）。每个测试用例需要覆盖正常流程和异常流程。

**答案**：
```typescript
// tests/auth-extended.test.ts
import { describe, it, expect } from 'vitest'
import request from 'supertest'
import app from '../src/app'
import { createTestUser } from './helpers'
...
describe('认证接口 - 扩展测试', () => {
  ...
  describe('POST /api/auth/refresh', () => {
    ...
    it('应该拒绝缺少 refreshToken 字段', async () => {
      const response = await request(app)
        .post('/api/auth/refresh')
        .send({})
  ...
  describe('边界情况测试', () => {
    ...
    it('注册时用户名超过 50 个字符应返回 422', async () => {
      const response = await request(app)
        .post('/api/auth/register')
        .send({
    ...
    it('登录时发送超大请求体应返回 413 或 400', async () => {
      const response = await request(app)
        .post('/api/auth/login')
        .send({
...
**要点**：
- 修改密码后要用新密码登录验证，确保密码真正被更新了
- 边界测试要覆盖空字符串、超长输入、密码长度临界值等场景
- Token 刷新要测试有效 Token、无效 Token、空 Token 三种情况
- 安全相关测试：错误的旧密码必须返回 401，不能返回具体的错误原因

### 练习 3：编写完整的评论测试

**思路**：按照 CRUD 操作编写评论模块的集成测试，重点测试权限控制（只能操作自己的评论）和嵌套评论逻辑。使用 `createTestUser` 和 `createTestArticle` 辅助函数搭建测试数据。
...
describe('评论接口', () => {
  ...
  describe('POST /api/comments', () => {
    it('应该成功创建评论', async () => {
      ...
      expect(response.status).toBe(201)
      expect(response.body.success).toBe(true)
      expect(response.body.data.comment.content).toBe('这篇文章写得很好！')
      expect(response.body.data.comment.author.username).toBe('testuser')
    })

    it('应该拒绝未认证的请求', async () => {
      const response = await request(app)
        .post('/api/comments')
        .send({ articleId: 1, content: 'test' })
    ...
    it('应该拒绝缺少必填字段', async () => {
      ...
      const response = await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: 1 }) // 缺少 content

      expect(response.status).toBe(422)
    })

    it('应该拒绝评论不存在的文章', async () => {
      const { accessToken } = await createTestUser()

      const response = await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: 99999, content: 'test' })
  ...
  describe('GET /api/comments', () => {
    it('应该返回文章的评论列表', async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        await request(app)
          .post('/api/comments')
          .set('Authorization', `Bearer ${accessToken}`)
          .send({ articleId: article.id, content: `评论 ${i}` })
      }

      const response = await request(app)
        .get(`/api/comments?articleId=${article.id}`)
    ...
    it('应该支持分页', async () => {
      ...
      const response = await request(app)
        .get(`/api/comments?articleId=${article.id}&page=2&pageSize=10`)
  ...
  describe('PUT /api/comments/:id', () => {
    it('作者应该能更新自己的评论', async () => {
      ...
      const createResponse = await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: article.id, content: '原始评论' })

      const commentId = createResponse.body.data.comment.id
      ...
      expect(response.status).toBe(200)
      expect(response.body.data.comment.content).toBe('更新后的评论')
    })

    it('不应该能更新别人的评论', async () => {
      const { user: author, accessToken: authorToken } = await createTestUser({
      ...
      const createResponse = await request(app)
        ...
        .send({ articleId: article.id, content: '作者的评论' })
  ...
  describe('DELETE /api/comments/:id', () => {
    it('作者应该能删除自己的评论', async () => {
      ...
      const createResponse = await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: article.id, content: '要删除的评论' })
    ...
    it('管理员应该能删除任何评论', async () => {
      ...
      const createResponse = await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${authorToken}`)
        .send({ articleId: article.id, content: '普通用户的评论' })
  ...
  describe('嵌套评论', () => {
    it('应该能回复评论', async () => {
      ...
      const parentResponse = await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: article.id, content: '父评论' })

      const parentId = parentResponse.body.data.comment.id
      ...
      expect(replyResponse.body.data.comment.parentId).toBe(parentId)
    })

    it('评论列表应包含嵌套回复', async () => {
      const { user, accessToken } = await createTestUser()
      const article = await createTestArticle(user.id)
      ...
      await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: article.id, content: '回复 1', parentId })

      await request(app)
        .post('/api/comments')
        .set('Authorization', `Bearer ${accessToken}`)
        .send({ articleId: article.id, content: '回复 2', parentId })
      ...
      expect(response.body.data.items).toHaveLength(1)
      // 该评论下应该有 2 条回复
      expect(response.body.data.items[0].replies).toHaveLength(2)
    })

    it('应该拒绝回复不存在的父评论', async () => {
      const { user, accessToken } = await createTestUser()
      const article = await createTestArticle(user.id)
...
**要点**：
- 更新和删除评论必须测试"非作者"场景，确保返回 403
- 管理员应该能删除任何人的评论，这是一个特殊的权限逻辑
- 嵌套评论测试要验证 `parentId` 关联和列表中的嵌套结构
- 获取评论列表时，顶级评论和回复的过滤逻辑（`parentId: null`）是关键

---

## 常见误区

1. **Swagger 文档和代码不同步**：写了 Swagger 注释后就再也不更新，接口改了参数但文档还是旧的。前端按照文档调用，结果线上出 bug。应该把 Swagger 文档作为代码的一部分，改接口必须改文档。

2. **只测试正常流程**：注册接口只测了"正常注册成功"，没测"重复邮箱""弱密码""缺少字段"等异常情况。真正容易出 bug 的恰恰是这些边界情况。

3. **测试依赖外部服务**：测试中直接调用真实的邮件服务、第三方 API，导致测试不稳定（网络波动就失败）、速度慢、有副作用。应该用 mock 或者测试专用的配置。

4. **跳过集成测试只写单元测试**：单元测试覆盖了工具函数，但整个注册流程从没跑过。结果上线后发现中间件顺序不对、数据库事务没提交，这些只有集成测试才能发现。

---

## 工程建议

1. **用 JSDoc 注释生成 Swagger 文档**：`swagger-jsdoc` 扫描路由文件中的 `@swagger` 注释自动生成文档，不需要维护单独的 YAML 文件。注释和代码在同一个文件里，更容易保持同步。

2. **测试文件放在 `tests/` 目录**：按模块组织测试文件（`auth.test.ts`、`article.test.ts`），每个测试文件对应一个功能模块。测试辅助函数（如 `createTestUser`）放在 `tests/helpers.ts` 中复用。

3. **测试前清理数据库**：用 `beforeEach` 清理测试数据库，确保每个测试用例的环境干净。但要注意清理顺序（先删外键关联的表），否则会报外键约束错误。

4. **API 文档要包含错误响应**：Swagger 文档中不仅写成功响应，还要写清楚各种错误状态码（401、403、404、422）及其含义。前端需要根据不同的错误码做不同的处理。
