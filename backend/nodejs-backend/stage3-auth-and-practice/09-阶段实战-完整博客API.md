...
**思路**：点赞和收藏功能本质上是用户与文章的多对多关系，需要创建中间表。关注系统是用户之间的多对多关系。所有功能都需要考虑幂等性（重复操作不产生副作用）。

**答案**：
```typescript
// 1. prisma/schema.prisma - 添加模型
model Like {
  id        String   @id @default(uuid())
  userId    String
  postId    String
...
// 2. src/services/like.service.ts
import prisma from '../config/database'
import { Errors } from '../utils/errors'
...
// 3. src/services/favorite.service.ts
import prisma from '../config/database'
...
export class FavoriteService {
  ...
  async listByUser(userId: string, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit
...
// 4. src/services/follow.service.ts
import prisma from '../config/database'
import { Errors } from '../utils/errors'
...
export class FollowService {
  async toggle(followerId: string, followingId: string) {
    if (followerId === followingId) {
      throw Errors.badRequest('不能关注自己')
    }
...
**要点**：
- 点赞和收藏使用 `@@unique([userId, postId])` 保证幂等性，同一个用户不能重复点赞
- toggle 模式（存在则删除，不存在则创建）是最常见的点赞/收藏交互方式
- 关注系统需要检查"不能关注自己"的边界情况
- Prisma 的 `include` 和 `_count` 可以在查询时同时获取关联数据，避免 N+1 问题

### 练习 2：性能优化

**思路**：使用 Redis 缓存热点数据（文章列表、文章详情），利用 Prisma 的 `include` 解决 N+1 查询问题，为常用查询字段添加数据库索引。

**答案**：
```typescript
// 1. Redis 缓存工具 src/utils/cache.ts
...
export async function setCache(key: string, value: any, ttl: number = DEFAULT_TTL) {
...
// 2. 在文章服务中使用缓存 src/services/post.service.ts
...
// 3. prisma/schema.prisma - 添加索引
// model Post {
//   @@index([status, createdAt])    // 按状态和时间排序的复合索引
//   @@index([authorId, status])     // 按作者查询已发布文章
//   @@index([categoryId, status])   // 按分类查询已发布文章
//   @@index([slug])                 // Slug 查询
// }

// model Comment {
//   @@index([postId, createdAt])    // 按文章查询评论
//   @@index([postId, parentId])     // 查询顶级评论
// }
```

**要点**：
- 缓存策略：读取时先查缓存，miss 再查数据库并写入缓存；写入时清除相关缓存
- N+1 问题：用 Prisma 的 `include` 在一次查询中获取关联数据，而不是循环查询
- 数据库索引：为 `WHERE` 和 `ORDER BY` 涉及的字段创建复合索引
- 缓存 TTL 不宜过长（5 分钟较合理），数据一致性要求高的场景用"写时清除"

### 练习 3：测试

**思路**：搭建 Vitest 测试环境，为 Service 层编写单元测试（mock 数据库），为 API 接口编写集成测试（使用 supertest + 测试数据库），通过覆盖率报告确保达到 80%。
...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      include: ['src/**/*.ts'],
      exclude: ['src/**/*.test.ts', 'src/types/**', 'src/config/**']
    }
  }
})
...
// 3. tests/services/auth.service.test.ts - 单元测试示例
...
**要点**：
- 单元测试 mock 掉数据库依赖，只测试业务逻辑，速度快
- 集成测试使用真实数据库（测试专用），测试完整的请求链路
- `beforeEach` 清理数据库确保每个测试的独立性，注意删除顺序（先删有外键的表）
- 覆盖率目标 80%，重点关注 Service 层和 Controller 层，工具函数应达到 100%

### 练习 4：部署

**思路**：按照 Railway 的标准流程部署：连接 GitHub → 添加数据库 → 配置环境变量 → 设置构建和启动命令 → 配置自定义域名 → 设置 GitHub Actions 自动部署。
...
# - New → Database → PostgreSQL（自动注入 DATABASE_URL）

# 2. 配置环境变量（Variables 标签页）
# NODE_ENV=production
...
# 3. 配置启动命令（Settings 标签页）
# Build Command: npm run build
# Start Command: npx prisma migrate deploy && node dist/index.js

# 4. 配置自定义域名
# 在 Railway Dashboard → Settings → Domains → Custom Domain
...
**要点**：
- Railway 会自动注入 `DATABASE_URL`，不需要在环境变量中手动配置
- 自定义域名需要在 DNS 服务商处添加 CNAME 记录，生效时间取决于 DNS TTL
- GitHub Actions 中 `RAILWAY_TOKEN` 需要在 GitHub 仓库的 Secrets 中配置
- 部署后必须测试健康检查端点和核心业务接口，确认一切正常
