...
**思路**：使用 bcryptjs 的 `hash` 和 `compare` 方法实现密码哈希和验证。`hash` 会自动生成随机盐值，因此同一密码每次哈希结果不同，但 `compare` 都能正确验证。
...
async function test() {
  ...
  console.log('正确密码:', await comparePassword(password, hash))  // true
  console.log('错误密码:', await comparePassword('wrong', hash))    // false

  // 验证同一密码产生不同哈希
  const hash2 = await hashPassword(password)
  console.log('两次哈希不同:', hash !== hash2)  // true（盐值不同）
  console.log('但都验证通过:', await comparePassword(password, hash2))  // true
}
...
**要点**：
- `bcrypt.hash` 内部会自动生成盐值并拼接到哈希结果中，无需手动管理盐
- `bcrypt.compare` 会从哈希值中提取盐值再重新计算，所以同一密码不同哈希都能验证通过
- salt rounds 设为 10 约 100ms，生产环境推荐 10-12

### 练习 2：JWT Token 生成与验证

**思路**：使用 `jwt.sign` 生成 Token 并设置过期时间，使用 `jwt.verify` 验证签名和有效期。验证失败时捕获异常返回 null，而不是抛出错误让调用方崩溃。
...
**要点**：
- `jwt.sign` 的第三个参数 `expiresIn` 控制过期时间，支持 `'1h'`、`'7d'`、`'15m'` 等格式
- `jwt.verify` 在 Token 过期或无效时会抛出异常，必须用 try/catch 捕获
- Payload 是 Base64 编码而非加密，不要在其中存储密码等敏感信息

### 练习 3：封装完整的认证工具

**思路**：将密码相关函数和 JWT 相关函数统一封装在一个模块中，使用环境变量管理密钥，定义清晰的 TypeScript 接口。密码强度校验用正则实现，Token 对包含 Access Token 和 Refresh Token。
...
async function test() {
  ...
  const hash = await hashPassword(password)
  console.log('哈希:', hash)
  console.log('验证:', await comparePassword(password, hash))
  console.log('强度:', validatePasswordStrength('abc'))
  console.log('强度:', validatePasswordStrength(password))

  const tokens = generateTokenPair({ userId: 1, email: 'test@example.com', role: 'USER' })
  console.log('AccessToken:', tokens.accessToken.substring(0, 50) + '...')
...
**要点**：
- Access Token 短有效期（15分钟），Refresh Token 长有效期（7天）
- 密钥从环境变量读取，开发环境用 fallback 值，生产环境必须配置
- `extractToken` 处理 `Bearer ` 前缀的提取，返回 null 表示无 Token
- 密码强度校验在注册时调用，防止用户设置弱密码
