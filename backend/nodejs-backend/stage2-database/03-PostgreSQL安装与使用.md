...
你在 SQL 入门课上学了 `CREATE TABLE`、`SELECT`、`INSERT` 等语法，但一直在纸上练习。现在你需要一个真实的数据库环境来运行这些 SQL。PostgreSQL 是目前功能最强大的开源关系型数据库，原生支持 JSON、全文搜索、地理数据，也是 Supabase、Neon 等新兴平台的底层引擎。这节课你将在本地安装 PostgreSQL，用 pgAdmin 图形界面和 psql 命令行工具操作数据库，并为博客项目创建真实的数据表和测试数据。
...
1. **使用 postgres 超级用户连接应用**：postgres 用户拥有所有权限，一旦应用被攻破，攻击者可以对数据库做任何事。生产环境应该创建权限最小化的应用用户。
2. **连接字符串写在代码里**：数据库密码等敏感信息应该放在 `.env` 文件中，通过环境变量读取。写在代码里的密码会被提交到 Git，造成安全隐患。
3. **不备份数据库**：硬盘损坏、误操作删除数据、迁移失败——这些情况都可能发生。定期使用 `pg_dump` 备份数据库，是运维的基本功。
4. **忘记设置正确的字符编码**：创建数据库时应该指定 UTF-8 编码，否则存储中文字符可能出现乱码。

## 工程建议

1. **为不同环境创建不同权限的用户**：开发环境可以用宽松权限方便调试，生产环境创建只读用户、读写用户、管理员用户，遵循最小权限原则。
2. **使用连接字符串格式管理数据库连接**：`postgresql://user:password@host:port/database` 格式是业界标准，Prisma、TypeORM 等 ORM 都支持。
3. **安装后立即修改默认密码**：postgres 用户的默认密码是你安装时设置的，不要用弱密码。如果密码中包含特殊字符，记得在连接字符串中进行 URL 编码。
4. **善用 psql 元命令提高效率**：`\dt` 列出表、`\d 表名` 查看结构、`\timing` 开启计时——这些快捷命令比打开 pgAdmin 更高效。
