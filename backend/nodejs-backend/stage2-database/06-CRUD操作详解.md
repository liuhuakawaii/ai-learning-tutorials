...
Schema 设计好了，数据库表也建好了，现在你需要用代码来操作数据库——创建用户、查询文章列表、更新评论内容、删除过期数据。在上一阶段你用内存数组做这些操作（`push`、`find`、`splice`），现在要把它们替换成 Prisma Client 的 API。虽然 API 不同，但核心思路是一样的：创建用 `create`，查询用 `findMany`，更新用 `update`，删除用 `delete`。掌握这些基本操作，你就能用 TypeScript 安全地操作数据库了。
...
1. **查询时返回所有字段包括密码**：默认 `findMany()` 会返回所有字段。用户表中的密码字段应该用 `select` 排除，避免泄露到 API 响应中。
2. **忽略 Prisma 的类型推导能力**：Prisma 会根据查询自动推导返回类型。使用 `select` 时返回类型会自动变窄，使用 `include` 时会自动包含关联类型。不需要手动定义 interface。
3. **不用事务处理多步操作**：创建文章并关联标签是两个操作，如果中间失败会导致数据不一致。用 `$transaction` 包裹多个操作，保证原子性。
4. **在循环中逐条查询关联数据（N+1 问题）**：查 10 篇文章然后循环查每篇的作者，会产生 11 次查询。用 `include: { author: true }` 一次搞定。

## 工程建议

1. **使用 select 精确控制返回字段**：只查询需要的字段可以减少数据传输量，提升查询性能，同时避免返回敏感信息。
2. **用 upsert 处理"不存在则创建"的场景**：`upsert` 比先 `findUnique` 再 `create` 更高效，且是原子操作，不会出现并发问题。
3. **用 Promise.all 并行执行独立查询**：查询用户列表和查询总数可以并行执行，总耗时等于较慢的那个查询，而不是两者之和。
4. **原始 SQL 作为 Prisma 的补充而非替代**：大多数 CRUD 用 Prisma API 完成，遇到 Prisma 不支持的高级功能（如窗口函数、CTE）时才用 `$queryRaw`。
