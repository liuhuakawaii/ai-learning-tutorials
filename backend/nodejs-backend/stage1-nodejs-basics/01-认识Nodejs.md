...
你每天都在用 JavaScript 写前端代码，但有没有想过：为什么 npm install 能在终端里运行？为什么 Vite 的开发服务器是用 JavaScript 写的？为什么你可以在命令行里执行 `node app.js` 启动一个后端服务？这些问题的答案都指向同一个技术——Node.js。它让 JavaScript 脱离了浏览器的"牢笼"，变成了可以操作文件、创建服务器、连接数据库的通用语言。理解 Node.js 的本质，是你从前端迈向全栈的第一步。
...
**思路**：直接在终端和 REPL 中执行命令，观察输出结果，验证 Node.js 是否正确安装。

**答案**：

```bash
# 1. 查看版本（应显示 v18.x.x 或 v20.x.x 等）
node -v
...
> console.log('Hello Node.js!')
# 输出: Hello Node.js!
# 输出: undefined（console.log 没有返回值）

> process.version
# 输出: 'v20.x.x'（取决于安装版本）

> process.platform
# 输出: 'win32'（Windows）/ 'darwin'（macOS）/ 'linux'（Linux）
...
**要点**：
- `process.version` 返回 Node.js 版本字符串，`process.platform` 返回操作系统平台标识
- REPL 中每条语句都会输出返回值，`console.log()` 的返回值是 `undefined`
- `.exit` 或按两次 `Ctrl+C` 可以退出 REPL

### 练习 2：体验异步

**思路**：对比同步和异步两种文件读取方式的执行顺序，理解非阻塞 I/O 的核心机制。
...
**要点**：
- `readFileSync` 会阻塞后续代码执行，直到文件读取完毕
- `readFile` 是异步的，回调函数会在文件读取完成后才执行，但主线程不会等待
- "这行在异步读取之前输出"证明了异步操作不会阻塞后续代码

### 练习 3：观察事件循环

**思路**：通过混合使用同步代码、`setTimeout`、`Promise` 和 `process.nextTick`，验证事件循环中微任务和宏任务的执行顺序。
...
3: 微任务 - Promise.then
4: 微任务 - Promise 链式调用
2: 宏任务 - setTimeout 0ms
```

**要点**：
- 同步代码最先执行（1 和 6），按书写顺序排列
- `process.nextTick` 的优先级高于 `Promise.then`（5 在 3 之前）
- `Promise` 链式调用按顺序执行（3 在 4 之前）
- `setTimeout` 即使延迟 0ms 也是宏任务，在所有微任务之后执行（2 最后）
- 执行顺序：同步代码 → nextTick → Promise 微任务 → setTimeout 宏任务

## 常见误区

1. **认为 Node.js 是一个前端框架**：Node.js 是一个运行时环境，不是框架。React、Vue 才是前端框架，Express、Koa 才是 Node.js 的 Web 框架。
2. **认为单线程等于不能处理并发**：Node.js 的单线程指的是 JavaScript 代码执行在单线程上，但底层的 I/O 操作由 libuv 线程池处理，因此能高效处理大量并发请求。
3. **认为 Node.js 适合所有后端场景**：Node.js 擅长 I/O 密集型任务（如 Web API、实时通信），但 CPU 密集型任务（如视频编码、大量计算）会阻塞主线程，需要使用 Worker Threads 或选择其他语言。
4. **混淆浏览器事件循环和 Node.js 事件循环**：浏览器有 requestAnimationFrame 和渲染步骤，Node.js 没有；Node.js 的事件循环有 6 个阶段，比浏览器更复杂。

## 工程建议

1. **选择 LTS 版本用于生产环境**：Node.js 的 LTS（长期支持）版本有 18 个月的维护期，稳定性有保障。学习阶段用最新 LTS 即可，生产环境不要追新。
2. **理解事件循环再写异步代码**：很多 bug 源于对事件循环执行顺序的误解。建议用 `process.nextTick`、`Promise`、`setTimeout` 的组合实验来加深理解。
3. **监控主线程阻塞**：在生产环境中，使用 `--inspect` 标志配合 Chrome DevTools 或 clinic.js 工具，检测是否有代码长时间阻塞主线程。
4. **善用 REPL 快速验证想法**：在终端输入 `node` 进入交互式环境，可以快速测试 JavaScript 语法、验证 API 行为，比创建文件再运行更高效。
