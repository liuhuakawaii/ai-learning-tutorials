...
**思路**：通过创建两个文件验证 CommonJS 模块的缓存机制——`require` 同一模块时返回的是同一个缓存对象。
...
console.log(c2.increment()); // 3
console.log(c2.getCount());  // 4

// 回答：c1 和 c2 是同一个对象
// 原因：Node.js 对 require 的结果做了缓存，同一个模块只会加载一次。
// 第二次 require 时直接返回缓存中的 exports 对象，所以 c1 和 c2 指向同一块内存。
// 验证：console.log(c1 === c2)  // true
```
...
**要点**：
- CommonJS 模块是单例的，`require` 同一路径返回缓存对象
- 模块内的变量（如 `count`）是模块私有的，通过 `exports` 暴露方法来间接访问
- `exports` 是 `module.exports` 的引用，给 `exports` 添加属性是安全的，但直接赋值（`exports = {}`）会断开引用

### 练习 2：使用 process 对象

**思路**：利用 `process` 的各种属性获取运行时信息，涵盖版本、路径、平台、内存等。
...
运行 `node info.js --verbose --port=3000` 后输出类似：
...
运行时间: 0.123 秒
命令行参数: [ '/usr/bin/node', '/Users/dev/project/info.js', '--verbose', '--port=3000' ]
```

**要点**：
- `process.argv` 是数组，前两个元素分别是 Node.js 可执行文件路径和当前脚本路径，后续才是用户传入的参数
- `process.memoryUsage()` 返回的对象中 `rss` 是进程总内存，`heapUsed` 是 V8 堆已用内存
- `process.cwd()` 返回当前工作目录，与脚本文件所在目录不同（脚本目录用 `__dirname`）

### 练习 3：Buffer 编码转换

**思路**：利用 `Buffer.from()` 和 `toString()` 实现不同编码之间的转换，理解字符串与字节的区别。
...
// 4. 字节长度 vs 字符长度
console.log('字符数:', str.length);             // 7（H,e,l,l,o,你,好）
console.log('字节数:', Buffer.byteLength(str, 'utf-8')); // 13（5 + 3*2 + 额外字节）
console.log('"你好"字节数:', Buffer.from('你好').length);  // 6
console.log('"hi"字节数:', Buffer.from('hi').length);      // 2
```

运行 `node encode.js` 后输出类似：

```
Base64: SGVsbG/kuIjloIc=
解码后: Hello你好
与原文一致: true
Hex: 48656c6c6fe4bda0e5a5bd
字符数: 7
字节数: 13
"你好"字节数: 6
"hi"字节数: 2
...
**要点**：
- `Buffer.from(str, encoding)` 将字符串按指定编码转为字节数组，`toString(encoding)` 将字节数组按指定编码转回字符串
- Base64 常用于数据传输（如图片嵌入 HTML），Hex 常用于调试和哈希展示
- UTF-8 编码中英文字符占 1 字节，中文字符占 3 字节，所以 `str.length`（字符数）和 `Buffer.byteLength`（字节数）不同

### 练习 4：路径处理

**思路**：使用 `path` 模块的 `__dirname`、`join`、`resolve`、`parse` 等方法处理路径，理解相对路径与绝对路径的区别。
...
// 2. 拼接 config/settings.json 路径
const configPath = path.join(__dirname, 'config', 'settings.json');
console.log('拼接路径:', configPath);
...
// 4. join vs resolve 的区别
console.log('join:', path.join('src', 'utils', 'helper.js'));
// 输出: src/utils/helper.js（简单拼接）

console.log('resolve:', path.resolve('src', 'utils', 'helper.js'));
// 输出: /当前工作目录/src/utils/helper.js（返回绝对路径）

console.log('join 含 ..:', path.join('src', '..', 'config', 'app.json'));
// 输出: config/app.json（.. 向上退一级）

console.log('resolve 含 ..:', path.resolve('src', '..', 'config', 'app.json'));
// 输出: /当前工作目录/config/app.json（.. 向上退一级，最终绝对路径）
```

**要点**：
- `path.join` 只做路径拼接和规范化，返回相对路径；`path.resolve` 会从右到左解析直到构造出绝对路径
- `__dirname` 是当前文件所在目录的绝对路径，`__filename` 是当前文件的绝对路径
- `path.parse()` 将路径字符串拆分为 `root`、`dir`、`base`、`name`、`ext` 五个部分
- 始终使用 `path.join` 或 `path.resolve` 拼接路径，不要用字符串 `+` 拼接，避免跨平台兼容问题
