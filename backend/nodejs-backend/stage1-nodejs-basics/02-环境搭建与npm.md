...
**思路**：通过 `npm init` 交互式创建 `package.json`，然后安装依赖观察文件变化，理解 npm 的依赖管理机制。
...
mkdir my-npm-practice
cd my-npm-practice
npm init -y  # -y 跳过交互问答，使用默认值

# 2. 查看生成的 package.json
cat package.json
# 输出类似：
# {
#   "name": "my-npm-practice",
#   "version": "1.0.0",
#   "description": "",
#   "main": "index.js",
...
# 5. 查看 node_modules 内容
ls node_modules/
# 包含 lodash 及其依赖

# 6. 查看 package-lock.json
cat package-lock.json
# 记录了完整的依赖树和每个包的精确版本
```

**要点**：
- `npm init -y` 可以跳过交互问答，直接生成默认 `package.json`
- `npm install <包名>` 会自动将包添加到 `dependencies`，同时生成 `package-lock.json`
- `node_modules` 目录存放实际下载的包，不应提交到 Git
- `package-lock.json` 锁定依赖的精确版本，确保团队安装结果一致

### 练习 2：npm scripts

**思路**：在 `package.json` 的 `scripts` 字段中定义命令，通过 `npm run` 执行，理解 npm scripts 的工作原理。
...
// package.json
{
  ...
  "scripts": {
    "start": "node index.js",
    "greet": "node -e \"console.log('你好，npm！')\"",
    "info": "node -e \"console.log(JSON.stringify(process.env, null, 2))\""
  }
}
```
...
# 执行脚本
npm run start
# 输出: Hello from npm scripts!
# 输出: 当前时间: 2024/1/1 12:00:00
# 输出: Node.js 版本: v20.x.x
...
# info 会输出大量环境变量，用 JSON.stringify 格式化
npm run info
```

**要点**：
- `npm run <script名>` 执行 `package.json` 中定义的脚本
- `npm start` 和 `npm test` 是特殊命令，可以省略 `run`
- npm scripts 可以访问 `node_modules/.bin` 中的可执行文件，无需全局安装
- `node -e` 可以直接执行一行 JavaScript 代码

### 练习 3：使用 npx

**思路**：通过 `npx` 运行 npm 包中的可执行文件，理解 npx 与 `npm install -g` 的区别。

**答案**：

```bash
# 1. 使用 npx 创建 React 项目
# npx 会临时下载 create-react-app 并执行，执行完自动清理
npx create-react-app my-react-app

# 2. 使用 npx 运行一次性命令
npx cowsay "Hello Node.js!"
# 如果本地没有 cowsay，npx 会自动下载并运行
...
# 4. 对比：如果用全局安装的方式
# npm install -g create-react-app  ← 不推荐，会污染全局
# create-react-app my-react-app    ← 调用全局命令
```

**要点**：
- `npx` 可以直接运行 `node_modules/.bin` 中的命令，无需配置 `npm run`
- `npx` 会临时下载包并执行，不会污染全局安装，用完即弃
- 对于一次性使用的 CLI 工具（如脚手架），优先用 `npx` 而非全局安装
- `npx` 还可以执行指定版本的包：`npx cowsay@1.0.0 hello`
