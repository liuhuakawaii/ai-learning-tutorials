...
线上告警突然响了：用户反馈"下单后余额没扣"。你看了错误日志，堆栈信息指向 `processOrder` 函数，但这个函数有 50 行，涉及数据库查询、余额计算、库存扣减，到底哪一步出了问题？你把代码翻来覆去看了半小时，发现是并发场景下两个请求同时读取了余额——但这种 Bug 在本地根本复现不了。这一课教你如何用 AI 从错误日志出发，系统性地缩小问题范围、构建根因假设、生成最小复现用例，快速定位那些"看不见"的 Bug。
...
1. **Bug 修复必须伴随回归测试**：找到 Bug 后先写一个能复现的测试用例，确认测试失败后再修复，修复后确认测试通过，这个测试就是防止 Bug 再现的保险。
2. **善用 git bisect 定位引入点**：当不确定哪个 commit 引入了 Bug 时，用 `git bisect` 配合自动化测试脚本，几分钟就能定位到具体 commit，比手动翻历史高效得多。
3. **建立团队的 Bug 模式库**：将高频出现的 Bug 模式（空值访问、竞态条件、时区问题等）整理为文档，新人入职时学习这些模式，可以大幅减少同类 Bug 的产生。
4. **生产环境保留足够的日志上下文**：Bug 定位的第一步是收集信息，确保错误日志包含请求 ID、用户 ID、关键变量值，否则 AI 也巧妇难为无米之炊。
...
**思路**：错误信息 `Converting circular structure to JSON` 表示对象存在循环引用，即对象的某个属性直接或间接地引用了自身。需要定位 `serialize` 函数中序列化的对象，找出循环引用的路径并断开循环。

**答案**：

```typescript
// src/utils/serializer.ts - 问题分析
// 原始代码可能存在如下结构：
interface TreeNode {
  id: string;
  name: string;
...
// 构建循环引用
const parent: TreeNode = { id: '1', name: 'parent' };
const child: TreeNode = { id: '2', name: 'child', parent };
parent.child = child; // parent.child.parent === parent，形成循环

// ❌ 直接序列化会报错
JSON.stringify(parent); // TypeError: Converting circular structure to JSON

// ✅ 修复方案 1：使用 replacer 函数过滤循环引用
...
// ✅ 修复方案 2：序列化时排除 parent 字段
function serializeNode(node: TreeNode) {
  return JSON.stringify(node, (key, value) => {
    if (key === 'parent') return undefined; // 断开循环
    return value;
  });
}

// ✅ 修复方案 3：DTO 模式 - 转换为纯数据对象
function toDTO(node: TreeNode): Record<string, unknown> {
  return {
    id: node.id,
    name: node.name,
    child: node.child ? toDTO(node.child) : null,
    // 不包含 parent 引用
  };
}
```
...
**要点**：
- 循环引用是 `JSON.stringify` 的常见错误，用 `WeakSet` 追踪已访问对象是最通用的解决方案
- 生产环境中序列化 API 响应时，应使用 DTO 模式将领域对象转换为纯数据对象，从根本上避免循环引用
- 错误堆栈指向 `serialize` → `sendResponse`，说明问题出在响应序列化阶段，应重点检查返回给客户端的数据结构

---

### 练习 2：修复异步 Bug

**思路**：`loadDashboard` 函数有三个严重的异步问题：1) 使用 `.then()` 但没有 `await`，导致赋值在函数返回后才执行；2) 三个请求是串行等待的，没有利用并发；3) 返回时 `user`、`orders`、`notifications` 仍是 `undefined`。

**答案**：

```typescript
// ❌ 原始代码的问题：
// 1. .then() 回调是异步的，return 时变量还没赋值
// 2. 没有 await，函数返回的是 Promise 但解构结果全是 undefined
// 3. 三个请求互相独立却串行执行，浪费时间

// ✅ 修复后：使用 Promise.all 并发执行 + await 等待结果
async function loadDashboard(userId: string) {
...
// ✅ 如果需要容错（某个查询失败不影响其他）：
async function loadDashboardWithFallback(userId: string) {
  ...
  return {
    user: getValue(results[0]),
    orders: getValue(results[1]) ?? [],
    notifications: getValue(results[2]) ?? [],
  };
}
```

**要点**：
- `async` 函数中使用 `.then()` 而不 `await` 是最常见的异步 Bug，变量赋值发生在微任务队列中，函数返回时还未执行
- 多个独立的异步操作应使用 `Promise.all` 并发执行，3 个各 100ms 的查询从 300ms 降到 100ms
- 生产环境中对并行查询建议使用 `Promise.allSettled` 替代 `Promise.all`，避免一个查询失败导致整体失败

---

### 练习 3：生成最小复现

**思路**：不稳定排序的 Bug 表现在相等元素的相对顺序被改变。需要构造一个包含重复值的数组，排序后验证元素的原始顺序是否被保持。核心是对比排序前后的索引。

**答案**：

```typescript
import { sortBy } from './sortBy'; // 假设的排序函数

describe('sortBy 稳定性测试', () => {
  // 辅助函数：检查排序是否稳定
  function isStableSort<T>(arr: T[], keyFn: (item: T) => number): boolean {
    // 给每个元素标记原始索引
    const indexed = arr.map((item, index) => ({ item, originalIndex: index }));
    const sorted = indexed.sort((a, b) => keyFn(a.item) - keyFn(b.item));

    // 对于值相同的元素，原始索引应该是递增的
    for (let i = 1; i < sorted.length; i++) {
      if (
        keyFn(sorted[i].item) === keyFn(sorted[i - 1].item) &&
        sorted[i].originalIndex < sorted[i - 1].originalIndex
      ) {
        return false; // 相同值的元素顺序被颠倒了
      }
    }
    return true;
  }
  ...
  it('混合值应正确排序且相同值保持稳定', () => {
    ...
    // 值应升序排列
    expect(sorted.map((s) => s.value)).toEqual([1, 1, 2, 2, 3]);
    // value=1 的元素应保持 b 在 d 前面
    expect(sorted[0].name).toBe('b');
    expect(sorted[1].name).toBe('d');
    // value=2 的元素应保持 c 在 e 前面
    expect(sorted[2].name).toBe('c');
    expect(sorted[3].name).toBe('e');
  });
  ...
  it('大量相同值的数据排序应保持稳定', () => {
    ...
    expect(isStableSort(items, (item) => item.value)).toBe(true);
  });

  it('空数组和单元素数组应正常处理', () => {
    expect(sortBy([], (item: { value: number }) => item.value)).toEqual([]);
    expect(sortBy([{ name: 'a', value: 1 }], (item) => item.value)).toEqual([
      { name: 'a', value: 1 },
    ]);
  });
});
...
**要点**：
- 不稳定排序 Bug 的复现关键是构造"值相同但需要保持顺序"的数据，用原始索引对比是最可靠的验证方式
- `Array.prototype.sort` 在不同引擎中的稳定性不同（V8 从 Node 12 起保证稳定），如果需要兼容旧环境应显式使用稳定排序算法
- 测试应覆盖纯相同值、混合值、大量相同值、边界情况（空数组、单元素）四种场景
