...
**思路**：两种方式的核心差异在于"人机分工比例"。补全模式下你是主驾驶，AI 是副驾驶；Chat 模式下你变成了产品经理，AI 是执行者。对比时重点关注：总耗时、代码一致性、需要手动修改的行数。

**答案**：

以 Todo List 增删改查为例，预期结果如下：

| 维度 | 方式 A（补全） | 方式 B（Chat） |
|------|--------------|--------------|
| 总耗时 | 25-35 分钟 | 8-15 分钟 |
| 代码行数 | 你写 60%，AI 补 40% | AI 写 90%，你改 10% |
| 需手动修改 | 较少（补全贴合上下文） | 较多（可能框架/风格不对） |
| 代码一致性 | 高（你控制风格） | 中（可能混用风格） |
| 学习效果 | 高（每行都理解） | 中（需要额外审查） |

关键发现：
- 补全适合你已经知道怎么写的场景，AI 加速输入
- Chat 适合你不确定实现方案的场景，AI 提供完整方案
- 最佳实践是两者结合：用 Chat 生成骨架，用补全填充细节

**要点**：
- 补全模式的优势是"实时、贴合上下文"，适合已有思路的编码
- Chat 模式的优势是"全局规划"，适合从零开始的复杂任务
- 实际工作中两者交替使用效率最高
- 记录耗时和修改量可以帮助你建立"何时用哪种模式"的直觉

---

### 练习 2：结构化提问练习

**思路**：差的提问缺少技术栈、功能范围、安全要求等关键信息，AI 只能猜测；好的提问把这些都明确下来，AI 的输出就能精准匹配需求。对比时重点关注完整性、安全性和代码风格三个维度。

**答案**：

差的提问"帮我写个登录功能"可能得到的代码：
```typescript
// AI 可能生成的"最低配"版本
...
**要点**：
- 好的提问明确了技术栈（Express + TypeScript + JWT），AI 不会猜成 Flask 或其他框架
- 好的提问指定了验证库（Zod）和加密方式（bcrypt），安全措施一步到位
- 差的提问没有错误处理、没有输入验证、密码明文比较——这些都是安全隐患
- 结构化提问的核心收益是"减少返工"，一次到位 vs 反复修改

---

### 练习 3：调试实战

**思路**：故意引入一个典型的异步 bug（比如忘记 await 导致的竞态条件），然后用结构化的方式向 AI 描述问题。关键是按照"错误现象 + 相关代码 + 已排查方向"的格式提问。

**答案**：

示例 bug：在 async 函数中忘记 await，导致数据库操作未完成就返回了结果。

```typescript
// 故意引入的 bug：缺少 await
async function updateUserProfile(userId: string, data: UpdateUserInput) {
  const user = prisma.user.update({  // ❌ 缺少 await
    where: { id: userId },
    data: { name: data.name, bio: data.bio },
  });
  return user; // 返回的是 Promise 对象，不是 user 数据
}
```

向 AI 提问的结构化方式：
```
我在调用 updateUserProfile 时返回值不对。

错误现象：
返回的是一个 Promise 对象而不是 user 数据，前端拿到的是 {}。

相关代码：
#file:src/services/user.ts 第 23 行的 updateUserProfile 函数

已排查：
- 数据库连接正常
- userId 是有效的 UUID
- Prisma schema 中 User model 有 name 和 bio 字段

请帮我定位问题并给出修复方案。
```

AI 的诊断过程：
1. 看到"返回 Promise 对象"→ 大概率是 async 函数中忘记 await
2. 定位到第 23 行 → `prisma.user.update()` 前面没有 await
3. 修复：加上 `await`
4. 附加建议：开启 ESLint 的 `no-floating-promises` 规则防止类似问题

**要点**：
- 调试提问的黄金公式：错误现象 + 相关代码 + 已排查方向
- 提供"已排查"信息能帮助 AI 缩小诊断范围，避免重复建议你已经试过的方向
- AI 调试能力很强，但前提是你给的信息足够——模糊的"有 bug"帮不了任何人
- 修复后追问"如何防止类似问题"能获得工程层面的建议（如 ESLint 规则）
