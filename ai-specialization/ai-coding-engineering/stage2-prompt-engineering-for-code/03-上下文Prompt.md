...
**思路**：CLAUDE.md 是项目级上下文的"一键配置"文件。好的 CLAUDE.md 应该覆盖四层上下文模型中的 Layer 1（技术栈）、Layer 2（项目结构）和 Layer 4（约定规范）。Layer 3（相关代码）不需要写在 CLAUDE.md 中，而是在每次对话时通过 @file 引用。

**答案**：

以一个 Next.js 14 电商项目为例：
...
## Coding Conventions
- API 响应统一用 { data?, error? } 包装
- 错误处理用 src/lib/errors.ts 中的 AppError 子类，不用 try-catch 吞错误
- 所有 API 路由必须用 Zod 验证输入
- 组件使用 React Server Components 优先，需要交互时用 'use client'
...
- pnpm prisma studio — 打开 Prisma 数据库浏览器
```

验证方式：开一个新对话，只说"帮我实现一个商品搜索 API"，不提供任何额外上下文。如果 AI 生成的代码：
- 使用 Next.js App Router 的 route.ts 格式 ✅
- 使用 Prisma 查询 ✅
- 使用 Zod 验证 ✅
- 使用 { data, pagination } 响应格式 ✅
- 使用 AppError 错误处理 ✅

说明 CLAUDE.md 生效了。

**要点**：
- CLAUDE.md 应该包含 AI 每次对话都需要的信息，不要写成完整项目文档
- Layer 1（技术栈）和 Layer 4（约定规范）放在 CLAUDE.md 中，每次对话自动加载
- Layer 2（项目结构）放在 CLAUDE.md 中，帮助 AI 理解文件组织方式
- Layer 3（相关代码）不需要放在 CLAUDE.md 中——每次对话时用 @file 引用即可
- 随项目演进定期更新 CLAUDE.md，过时的上下文比没有更危险

---

### 练习 2：上下文精简练习

**思路**：原始 Prompt 包含了 30+ 个依赖和 10+ 个目录，但"登录表单"只需要其中的一小部分。精简原则：只保留与当前任务直接相关的上下文，去除噪声。
...
```
React 18 + TypeScript + Tailwind CSS 项目，实现用户登录表单组件。

参考：@file: src/components/forms/register-form.tsx（现有注册表单的风格）
验证：使用 react-hook-form + zod
请求：用 Axios POST /api/auth/login
状态：登录成功后用 Zustand 存储用户信息，跳转到 /dashboard

约束：不使用 any，支持 loading 和 error 状态，支持 Enter 键提交。
```
...
|---------|---------|------|
| React 18 | ✅ | 框架版本 |
| TypeScript 5.3 | ❌ | "TypeScript" 已足够，版本号不关键 |
| Vite 5 | ❌ | 登录表单不涉及构建工具 |
| Zustand 4.5 | ✅ | 状态管理，登录后要存用户信息 |
| React Query 5.17 | ❌ | 登录是 mutation，用 Axios 直接请求即可 |
| Tailwind 3.4 | ✅ | 样式方案 |
| Framer Motion 10 | ❌ | 登录表单不需要动画 |
| Axios 1.6 | ✅ | HTTP 请求库 |
| react-hook-form 7 | ✅ | 表单管理 |
| zod 3.5 | ✅ | 表单验证 |
| react-router-dom 6 | ✅ | 登录后跳转 |
| lucide-react 0.300 | ❌ | 图标库，不影响登录逻辑 |
| 其他 20 个依赖 | ❌ | 与登录表单无关 |

**要点**：
- 精简原则：只保留与当前任务直接相关的依赖
- "版本号"通常不需要——"TypeScript" 比 "TypeScript 5.3" 更高效
- 构建工具（Vite）、动画库（Framer Motion）、图标库（lucide-react）对登录表单没有影响
- 精简后的 Prompt 从 500+ 字减少到 10 行，但信息密度反而更高
- 如果 AI 生成的代码缺少某个库（如忘记用 react-hook-form），在第二轮补充即可

---

### 练习 3：上下文对比实验

**思路**：用同一个需求（如"实现用户列表 API"），分别用"无上下文"和"有上下文"两种方式让 AI 生成代码。核心观察点：框架是否匹配、风格是否一致、返工次数差异。

**答案**：

| 维度 | 无上下文 | 有上下文 |
|------|---------|---------|
| 框架 | 可能是 Express（最常见） | 正确使用项目框架（如 Next.js App Router） |
| ORM | 可能是原生 SQL 或 Mongoose | 正确使用 Prisma |
| 错误处理 | try-catch + console.log | AppError + 统一错误格式 |
| 响应格式 | { users: [...] } 或直接返回数组 | { data: [...], pagination: {...} } |
| 类型安全 | any 或无类型 | 完整的 TypeScript 类型 |
| 输入验证 | 无 | Zod 验证查询参数 |
| 返工轮次 | 5-8 轮 | 0-2 轮 |

无上下文时 AI 的典型输出：
```javascript
// AI 可能生成 Express + 原生 SQL 的代码
...
**要点**：
- 无上下文时 AI 默认选择"最常见的方案"——通常是 Express + 原生 SQL，而不是你的项目技术栈
- 有上下文时 AI 能精确匹配框架、ORM、错误处理方式、响应格式——几乎零返工
- 上下文的核心价值不是"让 AI 写得更好"，而是"让 AI 写得跟你一样"
- CLAUDE.md 一次配置、永久生效——这是上下文管理的最高 ROI 投资
- 实验应该实际操作一次，亲身感受差异比看表格更有说服力
