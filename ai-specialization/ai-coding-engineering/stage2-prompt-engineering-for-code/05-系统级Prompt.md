...
你的团队有 5 个开发者，都在用 AI 辅助编码。开发者 A 让 AI 写的 API 用 Express 风格，B 用的是 Next.js App Router，C 用了 Koa。错误处理更是五花八门：有人用 try-catch，有人用全局中间件，有人直接返回 500。代码审查时，大家花大量时间争论风格而非业务逻辑。问题不在 AI，而在于没有统一的"AI 编程规范"——一份让所有人在所有对话中都遵循的系统级 Prompt。
...
1. **CLAUDE.md 控制在 50-100 行**：只写核心规范（技术栈、错误处理、响应格式、命名约定），细节放在子目录的 CLAUDE.md 中。过长的规范文件会消耗大量 token，反而降低 AI 对关键信息的关注度。

2. **规范与代码保持同步**：每次技术栈变更或架构调整后，立即更新 CLAUDE.md。可以在 PR 模板中加入"是否需要更新 CLAUDE.md"的检查项，避免规范过时。

3. **用具体规则替代模糊描述**：不要写"写高质量代码"，而要写"函数不超过 30 行"、"使用 early return 模式"、"禁止使用 any"。AI 需要可执行的具体指令，而非抽象原则。

4. **分层管理规范**：全局偏好放 `~/.claude/CLAUDE.md`，项目级规范放项目根目录，模块特定规范放子目录。这样既避免重复，又能精确控制不同模块的行为差异。
...
**思路**：选择一个实际项目（如任务管理应用），按照课程推荐的 CLAUDE.md 结构编写。核心是具体可执行——每条规则都要让 AI 能直接遵循，而非模糊描述。

**答案**：

```markdown
# CLAUDE.md — TaskFlow 任务管理平台

## 项目概述
团队任务管理平台，支持看板视图、任务分配、截止日期提醒和数据统计。
...
src/
  ...
  lib/errors.ts     # AppError 错误类层次
  lib/prisma.ts     # Prisma 单例
  hooks/            # 自定义 hooks
  types/            # 共享类型

## 编码约定
- 文件名 kebab-case，组件名 PascalCase，函数名 camelCase
- 禁止使用 any，用 unknown + 类型守卫
- 优先用 interface 定义对象形状，type 用于联合类型
- 导入顺序：React → 第三方 → @/lib → @/components → 相对路径

## 错误处理
- 业务层抛 AppError 子类（NotFoundError, ValidationError, UnauthorizedError）
- API 路由捕获 AppError 返回对应状态码
- 禁止空 catch 或吞掉错误
...
## 测试约定
- 测试文件放在 __tests__/ 目录，后缀 .test.ts
- 测试描述用中文
- Mock 外部依赖，不 Mock 内部模块
...
## 已知限制
- 不使用 next/router（用 next/navigation）
- 不使用 getServerSideProps（用 Server Components）
- 不在 Client Components 中调用 Prisma
```

**要点**：
- 控制在 50-100 行，只写核心规范
- 每条规则都是可执行的具体指令
- 技术栈版本明确，避免 AI 引入不兼容的库

---

### 练习 2：规范迁移练习

**思路**：分别编写 Express 版和 Next.js 版的 CLAUDE.md，重点对比两者的差异点——框架选择、路由方式、错误处理、中间件模式。然后用两份规范让 AI 生成同一个 API，验证风格一致性。

**答案**：

**Express 版 CLAUDE.md（关键差异部分）：**

```markdown
## 技术栈
- Framework: Express 4.18 + TypeScript 5.4
- 路由: Express Router
- 中间件: 自定义 error middleware 统一处理错误
- 响应: res.json({ code: 0, data: T, message: 'ok' })

## API 路由模式
src/routes/user.ts → Router + Controller + Service 三层
Controller 只做参数解析和响应，Service 做业务逻辑

## 错误处理
throw new AppError(404, '用户不存在')
→ 由全局 errorMiddleware 统一捕获并返回
```

**Next.js 版 CLAUDE.md（关键差异部分）：**

```markdown
## 技术栈
- Framework: Next.js 14 App Router + TypeScript 5.4
- 路由: app/api/ 下的 Route Handlers
- 响应: NextResponse.json({ data: T })

## API 路由模式
src/app/api/users/route.ts → export async function GET/POST
直接调用 Prisma，不需 Controller 层

## 错误处理
throw new AppError(404, '用户不存在')
→ 由 app/error.tsx 全局错误边界捕获
```

**对比验证结果：**

用 Express 规范生成的用户 API 使用 `Router()` + `res.json()` 模式；用 Next.js 规范生成的同一个 API 使用 `export async function GET()` + `NextResponse.json()` 模式。两者风格各自统一，验证了规范的有效性。

**要点**：
- 迁移时需要更新：框架名称、路由模式、错误处理方式、响应格式
- 不变的部分（命名规范、TypeScript 规则、测试约定）可以直接复用
- 规范迁移后要让 AI 生成 2-3 个 API 验证一致性

---

### 练习 3：团队规范协作

**思路**：模拟"规范不完善→AI 生成不符→补充规范→再次验证"的迭代过程。关键是识别哪些是 AI 能自动推断的（如基本语法），哪些必须显式写入规范（如业务约定）。

**答案**：

**第 1 步：初始 CLAUDE.md（前端负责人视角）**

写了基本的技术栈和编码约定，但遗漏了以下 3 个预期。

**第 2 步：AI 生成代码后发现的 3 处不符：**

1. **日期格式**：AI 用了 `Date.toLocaleString()`，但团队约定用 `dayjs` 库 + `YYYY-MM-DD HH:mm` 格式
2. **空状态组件**：AI 直接在页面中写 `<div>暂无数据</div>`，但团队有统一的 `<EmptyState>` 组件
3. **错误提示**：AI 用 `alert()` 提示错误，但团队用 `sonner` toast 库

**第 3 步：补充到 CLAUDE.md 的规则：**

```markdown
## 补充约定
- 日期处理统一用 dayjs，格式化模板：YYYY-MM-DD HH:mm
- 空状态使用 @/components/ui/empty-state 组件
- 错误提示使用 sonner toast（import { toast } from 'sonner'）
```

**第 4 步：再次生成验证，3 处全部符合预期。**

**AI 能自动推断的（不需要写入规范）：**
- TypeScript 基本语法和类型系统
- React 组件的基本结构
- Tailwind CSS 的常规用法

**必须显式写入规范的（业务/团队约定）：**
- 使用哪个第三方库（dayjs vs date-fns vs luxon）
- 使用哪个内部组件（EmptyState vs 自定义）
- 错误提示方式（toast vs alert vs modal）

**要点**：
- AI 无法推断团队的特定选择（用哪个库、哪个组件），必须显式声明
- 每次发现不符就补充规范，形成"生成→发现→补充→验证"的闭环
- 规范越具体，AI 输出越一致
