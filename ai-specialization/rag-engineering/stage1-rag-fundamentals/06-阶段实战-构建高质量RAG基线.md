...
经过前面五节课的学习，你已经了解了 RAG 架构、文档解析、分块策略、Embedding 模型和向量数据库。但现在面临一个现实问题：如何把这些零散的知识串成一个完整可运行的系统？你的项目经理要求你在一周内交付一个能回答公司内部知识库问题的 RAG 原型，并且要有量化的质量指标。你需要的不是更多的理论，而是一套可复制的工程模板——从文档摄入到回答生成，每一步都有明确的实现和评估方法。
...
1. **用配置类管理所有参数**：不要把 chunk_size、top_k、temperature 等参数散落在代码各处。用一个 dataclass 集中管理，方便不同环境（开发/测试/生产）切换配置，也方便实验记录和复现。
2. **增量更新而非全量重建**：当知识库有新文档时，只对新文档做 Embedding 并插入向量库，不要每次都全量重建索引。百万级文档的全量 Embedding 成本可能高达数百美元。
3. **评估数据集要覆盖边界情况**：除了常规问题，测试集必须包含"无法回答的问题"（知识库中没有相关内容）和"需要多文档综合的问题"。只测简单问题会高估系统质量。
4. **记录每次实验的完整配置**：每次优化后，把配置参数、评估指标和代码版本一起记录下来。RAG 系统的调优涉及大量参数交互，没有实验记录就无法回溯和复现。
...
**思路**：使用本课提供的完整代码模板，替换文档路径和配置参数即可。关键是准备好测试文档集和 5 个真实测试问题，然后记录 Recall@5 和延迟作为基线。
...
# 2. 构建系统
pipeline = build_rag_system("./my_docs", config)
...
test_cases = [
    EvalCase(
        question="公司的报销流程是什么？",
        expected_answer="报销需提交发票和审批单，经部门经理审批后财务处理",
        expected_sources=["员工手册.pdf", "财务制度.md"],
    ),
    # ... 更多测试用例
]

evaluator = RAGEvaluator(pipeline)
eval_result = evaluator.run_evaluation(test_cases)
print(f"\nRecall@5: {eval_result.recall_at_k:.2%}")
print(f"平均延迟: {eval_result.latency_ms:.0f}ms")
```

**要点**：
- 先用默认配置跑通全流程，再逐步调优参数
- 测试问题要覆盖简单查询、复杂查询和无法回答的查询
- 记录基线指标（Recall@5、延迟、成本），后续优化时作为对比基准

---

### 练习 2: 分块策略对比实验

**思路**：在同一文档集上分别用三种 chunk_size 和递归/语义分块，用 10 个测试问题评估 Recall@5。核心是控制变量——只改分块策略，其他参数（embedding 模型、top_k、LLM）保持一致。
...
# 测试用例
test_cases = [
    EvalCase("什么是RAG?", "检索增强生成技术", ["rag_intro.pdf"]),
    EvalCase("HNSW索引原理", "分层可导航小世界图", ["vector_db.md"]),
    # ... 至少 10 个
]

# 分块策略对比
strategies = {
    "fixed_256": {"chunk_size": 256, "chunk_overlap": 25},
    "fixed_512": {"chunk_size": 512, "chunk_overlap": 50},
    "fixed_1024": {"chunk_size": 1024, "chunk_overlap": 100},
    "recursive_512": {"chunk_size": 512, "chunk_overlap": 64},
}
...
for name, params in strategies.items():
    ...
    # 构建 RAG（复用同一 embedding 模型和 LLM）
    config = RAGConfig(**params)
    # ... 构建 pipeline（参考 main.py）
    ...
    results[name] = {
        ...
        "recall_at_5": round(eval_result.recall_at_k, 4),
        "latency_ms": round(eval_result.latency_ms, 0),
    }

    print(f"  块数: {results[name]['chunk_count']}")
    print(f"  Recall@5: {results[name]['recall_at_5']:.2%}")

# 输出对比表
print("\n" + "=" * 70)
print(f"{'策略':<15} {'块数':<8} {'平均大小':<10} {'Recall@5':<10} {'延迟ms':<10}")
for name, r in results.items():
    print(f"{name:<15} {r['chunk_count']:<8} {r['avg_chunk_size']:<10.0f} {r['recall_at_5']:<10.2%} {r['latency_ms']:<10.0f}")
```

**要点**：
- chunk_size=256 检索精度高但可能丢失上下文，chunk_size=1024 上下文完整但精度下降
- 递归分块在大多数场景下优于固定分块，因为它按语义边界（段落、句子）切分
- 语义分块效果最好但需要额外的 embedding 计算时间，适合对质量要求高的场景

---

### 练习 3: 构建评估数据集

**思路**：评估数据集要覆盖三类问题：简单问题（单文档可答）、复杂问题（需多文档综合）、无答案问题（知识库中无相关内容）。每类问题都要有期望答案和期望来源文档。
...
def build_eval_dataset() -> list[EvalCase]:
    ...
    simple_questions = [
        EvalCase(
            question="什么是RAG?",
            expected_answer="RAG是检索增强生成技术，在生成回答前先检索相关文档",
            expected_sources=["rag_intro.pdf"],
        ),
        EvalCase(
            question="HNSW索引的时间复杂度是多少?",
            expected_answer="HNSW的时间复杂度是O(log N)",
            expected_sources=["vector_db_guide.md"],
        ),
        EvalCase(
            question="chunk_overlap的作用是什么?",
            expected_answer="chunk_overlap防止相邻块之间的信息断裂，保持语义连续性",
            expected_sources=["chunking_strategy.md"],
        ),
        EvalCase(
            question="BGE模型需要为查询添加前缀吗?",
            expected_answer="是的，BGE建议为查询添加前缀以区分文档和查询的编码方式",
            expected_sources=["embedding_guide.md"],
        ),
        EvalCase(
            question="ChromaDB适合生产环境吗?",
            expected_answer="ChromaDB适合原型开发和本地测试，生产环境建议使用Milvus或Qdrant",
            expected_sources=["vector_db_comparison.md"],
        ),
        EvalCase(
            question="Embedding归一化的作用是什么?",
            expected_answer="归一化后余弦相似度和点积等价，可以使用更高效的点积检索",
            expected_sources=["embedding_best_practices.md"],
        ),
        EvalCase(
            question="什么是交叉编码器重排序?",
            expected_answer="交叉编码器对查询和文档联合编码，比双塔模型更精确但更慢",
            expected_sources=["reranking_guide.md"],
        ),
        EvalCase(
            question="PDF中的表格如何提取?",
            expected_answer="使用PyMuPDF或pdfplumber从PDF中提取表格，转换为Markdown格式保留结构",
            expected_sources=["document_parsing.md"],
        ),
        EvalCase(
            question="向量数据库的HNSW索引是什么?",
            expected_answer="HNSW是分层可导航小世界图索引，通过多层图结构实现O(log N)的近似最近邻搜索",
            expected_sources=["vector_indexing.md"],
        ),
        EvalCase(
            question="什么是Faithfulness评估指标?",
            expected_answer="Faithfulness衡量生成的回答是否忠于检索到的参考资料，不包含编造信息",
            expected_sources=["evaluation_guide.md"],
        ),
    ]
    cases.extend(simple_questions)

    # 复杂问题（10 个）：需要多文档综合
    complex_questions = [
        EvalCase(
            question="从文档摄入到生成回答的完整RAG流程是什么?",
            expected_answer="文档解析→分块→Embedding→向量存储→查询→检索→重排序→LLM生成",
            expected_sources=["rag_intro.pdf", "chunking_strategy.md", "embedding_guide.md"],
        ),
        EvalCase(
            question="如何选择合适的向量数据库?",
            expected_answer="根据数据规模、过滤需求、运维能力和预算选择：原型用ChromaDB，生产用Milvus/Qdrant",
            expected_sources=["vector_db_comparison.md", "vector_indexing.md"],
        ),
        EvalCase(
            question="RAG系统的常见失败模式有哪些?",
            expected_answer="检索失败（语义不匹配、分块切断上下文）、生成失败（幻觉、忽略上下文）、系统失败（延迟、成本）",
            expected_sources=["rag_intro.pdf", "evaluation_guide.md"],
        ),
        # ... 更多复杂问题
    ]
    cases.extend(complex_questions)

    # 无答案问题（5 个）：知识库中没有相关内容
    no_answer_questions = [
        EvalCase(
            question="如何训练一个GPT模型?",
            expected_answer="无法回答",
            expected_sources=[],
        ),
        EvalCase(
            question="RAG系统的部署需要多少服务器?",
            expected_answer="无法回答",
            expected_sources=[],
        ),
        EvalCase(
            question="如何用RAG系统做图像生成?",
            expected_answer="无法回答",
            expected_sources=[],
        ),
        EvalCase(
            question="RAG和区块链有什么关系?",
            expected_answer="无法回答",
            expected_sources=[],
        ),
        EvalCase(
            question="如何用RAG进行股票预测?",
            expected_answer="无法回答",
            expected_sources=[],
        ),
    ]
    cases.extend(no_answer_questions)

    return cases

# 构建并保存
dataset = build_eval_dataset()
print(f"评估数据集: {len(dataset)} 条")
print(f"  简单问题: 10 条")
print(f"  复杂问题: 10 条")
print(f"  无答案问题: 5 条")
...
with open("eval_dataset.json", "w", encoding="utf-8") as f:
    json.dump([asdict(c) for c in dataset], f, ensure_ascii=False, indent=2)

# 自动运行评估脚本
from run_eval import run_full_evaluation
report = run_full_evaluation("eval_dataset.json")
print(f"\n评估报告:")
print(f"  Recall@5: {report['recall_at_5']:.2%}")
print(f"  答案质量: {report['answer_quality']:.2%}")
print(f"  无答案识别率: {report['no_answer_detection']:.2%}")
```

**要点**：
- 评估数据集至少 25 条，覆盖简单、复杂、无答案三类问题，比例建议 10:10:5
- 无答案问题测试系统是否会产生幻觉——理想情况下应返回"无法回答"而非编造内容
- 每次优化后都要在完整数据集上跑评估，避免"优化了 A 但搞砸了 B"
