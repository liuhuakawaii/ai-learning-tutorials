...
你的 RAG 系统端到端延迟是 4 秒，用户点击提问后要盯着空白页面等 4 秒才能看到答案。产品团队收到大量反馈"系统太慢了"，但性能优化已经做得差不多了——Embedding、检索、LLM 生成每个环节都已经优化到接近极限。一个低成本的改善方案是流式输出：让用户从第一个字开始就看到内容逐步生成，而不是等全部生成完再一次性展示。TTFT（首字延迟）从 4 秒降到 300ms，感知体验完全不同。
...
1. **TTFT 是最应该优化的指标**：用户感知的"速度"主要取决于首字出现的时间。确保检索和 LLM 首次输出之间的链路尽可能短，必要时用异步预加载。
2. **流式输出要做好错误处理**：如果流式传输到一半 LLM 报错，前端需要有优雅的降级策略，而不是显示不完整的回答。建议在流式过程中维护一个缓冲区。
3. **背压控制是生产必备**：当 LLM 生成速度超过网络传输速度时，需要有背压机制防止内存溢出。SSE 和 WebSocket 都有标准的背压处理方式。
4. **流式场景下的引用标注要特殊处理**：传统 RAG 在生成完成后统一添加引用，但流式生成时引用信息可能在中途才确定。建议在引用确定后立即通过流式通道发送更新。
...
**思路**：用 FastAPI 的 `StreamingResponse` 包装异步生成器实现 SSE，通过 `request.is_disconnected()` 检测客户端断开，用 `asyncio.wait_for` 包装整体生成过程实现超时控制。
...
async def stream_chat_generator(query: str, rag_pipeline, request: Request, timeout: float = 30):
    """带超时和断连检测的 SSE 生成器"""
    try:
        async with asyncio.timeout(timeout):
            yield await sse_event("status", {"stage": "retrieving"})

            results = await rag_pipeline.retrieve(query)
            sources = [{"title": r.get("title", ""), "url": r.get("url")} for r in results]
            yield await sse_event("sources", {"sources": sources})

            if await request.is_disconnected():
                return

            yield await sse_event("status", {"stage": "generating"})
            context = "\n".join(r["text"] for r in results)

            async for token in rag_pipeline.generate_stream(query, context):
                if await request.is_disconnected():
                    return
                yield await sse_event("token", {"content": token})

            yield await sse_event("done", {"status": "complete"})

    except asyncio.TimeoutError:
        yield await sse_event("error", {"message": f"请求超时 ({timeout}s)"})
    except Exception as e:
...
async def stream_chat(request: Request):
    ...
    return StreamingResponse(
        stream_chat_generator(query, rag_pipeline, request, timeout=30),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "Connection": "keep-alive",
...
**要点**：
- `asyncio.timeout(30)` 是 Python 3.11+ 的超时上下文管理器，超时后自动抛出 `TimeoutError` 并通过 SSE 通知客户端
- 在生成循环中每次都检查 `request.is_disconnected()`，客户端断开后立即停止生成，避免浪费 LLM 调用
- `X-Accel-Buffering: no` 头部防止 Nginx 缓冲 SSE 响应，确保真正的流式传输

---

### 练习 2: 带来源标注的流式生成

**思路**：在检索完成后先发送 sources 事件，将每个 source 用编号标记后拼入上下文，让 LLM 在生成时引用 `[1]`、`[2]` 等标记，最后发送完整的引用列表元数据。
...
class StreamingRAGWithCitations:
    """带来源标注的流式 RAG"""
    ...
    async def stream_query(self, query: str, top_k: int = 5) -> AsyncGenerator[dict, None]:
        # 1. 检索
        embedding = await self.embedder.embed([query])
        raw_results = await self.retriever.search(embedding[0], top_k=top_k)
        ...
        context_parts = []
        for s in sources:
            context_parts.append(f"[{s.index}] {s.title}\n{raw_results[s.index - 1]['text']}")
        context = "\n\n".join(context_parts)

        system_prompt = (
            "你是一个 RAG 助手。根据以下来源回答问题。\n"
            "在回答中使用 [编号] 标注引用来源，例如 [1]、[2]。\n"
            "如果来源中没有相关信息，明确说明。"
        )
        messages = [
            {"role": "system", "content": system_prompt},
        ...
        yield {
            "type": "metadata",
            "citations": [vars(s) for s in sources],
...
**要点**：
- 先发送 `sources` 事件让前端立即展示引用来源卡片，用户在 LLM 生成完成前就知道答案来自哪里
- 上下文中显式标注 `[1]`、`[2]` 编号并配合 system prompt 指引，让 LLM 自然地在回答中引用来源
- 最后发送 `metadata` 事件携带完整引用列表，前端收到后可以将回答中的 `[1]` 替换为可点击的超链接

---

### 练习 3: 背压感知的流式管道

**思路**：用 `asyncio.Queue(maxsize)` 实现背压——生产者（LLM 生成）往队列写入时如果队列满会自动等待，消费者（网络发送）按可调速率从队列取数据并发送。
...
class BackpressureAwareStreamPipeline:
    """背压感知的流式管道"""

    def __init__(self, max_queue_size: int = 50, initial_rate: int = 50):
        self.max_queue_size = max_queue_size
        self.current_rate = initial_rate
        self.min_rate = 10
        self.max_rate = 200
    ...
    async def stream(
        ...
        async def producer():
            try:
                async for token in llm_generator:
                    ...
                    await queue.put(token)  # 队列满时自动等待 → 背压生效
        ...
        try:
            while True:
                ...
                batch_interval = 1000 / self.current_rate * len(batch)

                if len(batch) >= max(1, self.current_rate // 10) or elapsed_ms > 50:
                    for item in batch:
                        yield item
                    batch.clear()
    ...
    def _adapt_rate(self, latency_ms: float):
        """根据发送延迟自适应调整速率"""
        if latency_ms > 100:
            self.current_rate = max(self.min_rate, self.current_rate - 5)
        elif latency_ms < 20:
    ...
    def get_metrics(self) -> dict:
        ...
        return {
            "ttft_ms": round(self._ttft_ms, 1) if self._ttft_ms else None,
            "total_tokens": self._total_tokens,
            "elapsed_ms": round(elapsed, 1),
            "tokens_per_sec": round(self._total_tokens / (elapsed / 1000), 1) if elapsed > 0 else 0,
            "current_rate": self.current_rate,
        }
```

**要点**：
- `asyncio.Queue(maxsize=50)` 是背压的核心——队列满时生产者的 `put()` 会自动阻塞，防止内存无限增长
- `_adapt_rate()` 根据消费者实际发送延迟动态调整速率，延迟高就降速，延迟低就提速，避免网络拥塞
- `get_metrics()` 返回 TTFT 和吞吐量指标，用于监控流式管道的健康状态和性能调优
