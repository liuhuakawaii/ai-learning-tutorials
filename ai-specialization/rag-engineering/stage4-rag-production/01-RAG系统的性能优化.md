...
**思路**：使用装饰器模式自动为 Pipeline 各阶段插桩，记录每次调用的耗时到内部列表，然后汇总统计各阶段的 p50/p95/p99 延迟，最后根据阈值规则给出优化建议。
...
class RAGPerformanceProfiler:
    ...
    def _make_wrapper(self, stage_name: str, func: Callable) -> Callable:
        if asyncio.iscoroutinefunction(func):
            @functools.wraps(func)
            async def async_wrapper(*args, **kwargs) -> Any:
                start = time.perf_counter()
    ...
    def instrument(self, pipeline):
        """为 pipeline 添加性能监控，自动包装所有公开方法"""
        stage_names = ["embed_query", "search_vectors", "rerank", "generate_answer"]
        for name in stage_names:
            if hasattr(pipeline, name):
                original = getattr(pipeline, name)
                setattr(pipeline, name, self._make_wrapper(name, original))
        return pipeline

    def generate_report(self) -> dict:
        """生成性能报告，包含各阶段的 p50/p95/p99 延迟"""
        report = {}
        for stage, times in self._timings.items():
            ...
            report[stage] = {
                ...
                "p50_ms": round(sorted_times[int(n * 0.5)] if n > 0 else 0, 1),
                "p95_ms": round(sorted_times[int(n * 0.95)] if n > 1 else sorted_times[-1], 1),
                "p99_ms": round(sorted_times[int(n * 0.99)] if n > 1 else sorted_times[-1], 1),
    ...
    def get_optimization_suggestions(self) -> list[str]:
        ...
        avg_per_stage = {stage: info["avg_ms"] for stage, info in report.items()}
        slowest = sorted(avg_per_stage.items(), key=lambda x: x[1], reverse=True)[:3]

        suggestions = []
        for stage, avg_ms in slowest:
            threshold = self.SLOW_THRESHOLD_MS.get(stage, 1000)
            if avg_ms > threshold:
                if stage == "embed_query":
                    suggestions.append(
                        f"[{stage}] 平均 {avg_ms}ms，超过阈值 {threshold}ms。"
                        f"建议：使用批量 Embedding 或启用 Embedding 缓存。"
                    )
                elif stage == "search_vectors":
                    suggestions.append(
                        f"[{stage}] 平均 {avg_ms}ms，超过阈值 {threshold}ms。"
                        f"建议：优化向量索引参数（如 HNSW ef_search），或与 BM25 检索并行执行。"
                    )
                elif stage == "generate_answer":
                    suggestions.append(
                        f"[{stage}] 平均 {avg_ms}ms，超过阈值 {threshold}ms。"
                        f"建议：启用流式输出降低感知延迟，或使用更小的模型，或压缩 Prompt 减少 token 数。"
                    )
                elif stage == "rerank":
                    suggestions.append(
                        f"[{stage}] 平均 {avg_ms}ms，超过阈值 {threshold}ms。"
                        f"建议：减少 Rerank 输入数量，或使用更快的 Reranker 模型。"
                    )
                else:
                    suggestions.append(
                        f"[{stage}] 平均 {avg_ms}ms，超过阈值 {threshold}ms，请检查实现。"
                    )
            else:
                suggestions.append(f"[{stage}] 平均 {avg_ms}ms，在阈值 {threshold}ms 内，暂无需优化。")

        return suggestions
```

**要点**：
- 使用装饰器模式实现无侵入式插桩，`instrument()` 方法自动包装 Pipeline 的各个阶段方法
- `generate_report()` 提供完整的统计指标（avg/p50/p95/p99/min/max），帮助定位延迟分布
- `get_optimization_suggestions()` 按平均耗时排序取 Top 3，针对不同阶段给出具体优化方向

---

### 练习 2: 实现并行检索器

**思路**：在 `ParallelRetriever` 基础上增加三层容错机制：用 `asyncio.wait_for` 实现单检索器超时控制；用计数器记录连续失败次数实现降级跳过；用文本哈希去重避免重复结果。
...
class RobustParallelRetriever(ParallelRetriever):
    ...
    def __init__(self, retrievers: list[Retriever], merger: ResultMerger,
                 timeout_ms: float = 500, max_consecutive_failures: int = 3):
        super().__init__(retrievers, merger)
    ...
    async def retrieve(self, query: str, top_k: int = 10) -> list[dict]:
        start = asyncio.get_event_loop().time()
        active_retrievers = [
            r for r in self.retrievers
            if type(r).__name__ not in self._disabled
        ]

        tasks = [
            self._retrieve_with_timeout(retriever, query, top_k)
            for retriever in active_retrievers
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        valid_results = []
        for retriever, result in zip(active_retrievers, results):
            source_name = type(retriever).__name__
            if isinstance(result, RetrievalResult):
                valid_results.append(result)
                self._failure_counts[source_name] = 0
            elif isinstance(result, asyncio.TimeoutError):
                self._record_failure(source_name, "timeout")
            elif isinstance(result, Exception):
                self._record_failure(source_name, str(result))

        merged = self.merger.merge(valid_results, top_k=top_k)
        deduplicated = self._deduplicate(merged)

        elapsed = (asyncio.get_event_loop().time() - start) * 1000
        print(f"Robust retrieval: {elapsed:.0f}ms, "
              f"{len(valid_results)} sources, {len(deduplicated)} results")
        return deduplicated

    async def _retrieve_with_timeout(self, retriever, query, top_k) -> RetrievalResult:
        source_name = type(retriever).__name__
        start = asyncio.get_event_loop().time()
        try:
    ...
    def _record_failure(self, source_name: str, reason: str):
        self._failure_counts[source_name] += 1
        count = self._failure_counts[source_name]
        print(f"[{source_name}] 失败 ({reason})，连续失败 {count} 次")
        if count >= self.max_failures:
            self._disabled.add(source_name)
            print(f"[{source_name}] 连续失败 {count} 次，已自动降级跳过")
...
**要点**：
- `asyncio.wait_for` 实现超时控制，单个检索器超过 500ms 自动取消并记录失败
- 连续失败计数器达到阈值后将检索器加入 `_disabled` 集合，后续请求自动跳过
- 基于 MD5 哈希的文本去重比逐对比较高效得多，时间复杂度从 O(n²) 降到 O(n)

---

### 练习 3: 端到端优化

**思路**：将批量 Embedding、并行检索和流式生成三个优化手段组合到一个 Pipeline 中，同时用 `time.perf_counter` 记录优化前后的各阶段延迟，计算加速比。
...
class OptimizedRAGPipeline:
    ...
    def __init__(self, baseline_pipeline):
        ...
        self.batch_embedder = BatchEmbedder(
            client=baseline_pipeline.embedder,
            batch_size=100,
            max_concurrent=5
        )
        self.parallel_retriever = ParallelRetriever(
            retrievers=[
                VectorRetriever(baseline_pipeline.vector_store, baseline_pipeline.embedder),
                KeywordRetriever(baseline_pipeline.search_engine),
            ],
            merger=ResultMerger()
        )
        self._baseline_timings: dict[str, float] = {}
        self._optimized_timings: dict[str, float] = {}

    async def optimized_query(self, question: str) -> AsyncGenerator[str, None]:
        """优化版查询：批量 Embedding + 并行检索 + 流式生成"""
        timings = {}

        # 1. 批量 Embedding
        t0 = time.perf_counter()
        embeddings = await self.batch_embedder.embed_texts([question])
        timings["embedding"] = (time.perf_counter() - t0) * 1000

        # 2. 并行检索（多路召回同时执行）
        t0 = time.perf_counter()
        results = await self.parallel_retriever.retrieve(question, top_k=5)
        timings["retrieval"] = (time.perf_counter() - t0) * 1000

        # 3. 流式生成（首 token 快速返回）
        t0 = time.perf_counter()
        context = "\n".join(r["text"] for r in results)
        async for token in self.baseline.generate_stream(question, context):
            if "first_token" not in timings:
                timings["first_token"] = (time.perf_counter() - t0) * 1000
            yield token
        timings["generation_total"] = (time.perf_counter() - t0) * 1000

        self._optimized_timings = timings

    async def _run_baseline(self, question: str) -> dict:
        """运行 baseline pipeline 并记录耗时"""
        timings = {}
        t0 = time.perf_counter()
        embedding = await self.baseline.embed_query(question)
        timings["embedding"] = (time.perf_counter() - t0) * 1000

        t0 = time.perf_counter()
        results = await self.baseline.search_vectors(embedding, top_k=5)
        timings["retrieval"] = (time.perf_counter() - t0) * 1000

        t0 = time.perf_counter()
        context = "\n".join(r["text"] for r in results)
        answer = await self.baseline.generate_answer(question, context)
        timings["generation"] = (time.perf_counter() - t0) * 1000

        self._baseline_timings = timings
        return {"answer": answer, "sources": results}

    async def compare_performance(self, test_queries: list[str]) -> dict:
        """对比优化前后的性能"""
        baseline_results = []
        optimized_results = []

        for query in test_queries:
            await self._run_baseline(query)
            baseline_results.append(dict(self._baseline_timings))

            chunks = []
            async for token in self.optimized_query(query):
                chunks.append(token)
            optimized_results.append(dict(self._optimized_timings))
        ...
        return {
            "baseline": {
                "avg_embedding_ms": avg_key(baseline_results, "embedding"),
                "avg_retrieval_ms": avg_key(baseline_results, "retrieval"),
                "avg_generation_ms": avg_key(baseline_results, "generation"),
            },
            "optimized": {
                "avg_embedding_ms": avg_key(optimized_results, "embedding"),
                "avg_retrieval_ms": avg_key(optimized_results, "retrieval"),
                "avg_first_token_ms": avg_key(optimized_results, "first_token"),
                "avg_generation_ms": avg_key(optimized_results, "generation_total"),
            },
            "improvement": {
                "embedding_speedup": f"{avg_key(baseline_results, 'embedding') / max(avg_key(optimized_results, 'embedding'), 0.1):.1f}x",
                "retrieval_speedup": f"{avg_key(baseline_results, 'retrieval') / max(avg_key(optimized_results, 'retrieval'), 0.1):.1f}x",
            }
        }
```

**要点**：
- 批量 Embedding 将多条文本合并为一次 API 调用，减少网络往返开销，延迟可降低 50% 以上
- 并行检索同时执行向量检索和关键词检索，总延迟等于最慢的那个而非所有之和，降低约 40%
- 流式生成让用户在首 token 到达（约 500ms）时就能看到内容，感知体验大幅提升
