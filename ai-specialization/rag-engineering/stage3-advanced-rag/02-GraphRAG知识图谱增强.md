...
你的 RAG 系统在回答"A 公司的 CEO 毕业于哪所大学"这类问题时表现不错，但当用户问"A 公司 CEO 的母校最近和哪些企业有合作"时就完全答不上来了。原因在于传统 RAG 只做文档级检索，无法跨越多个实体之间的关系链。知识图谱能将实体和关系结构化存储，让系统能够沿着"人→毕业院校→合作企业"这样的关系路径进行推理，这是纯向量检索做不到的。
...
1. **知识图谱的构建成本高，先评估 ROI**：构建和维护知识图谱需要大量人力和计算资源。如果 80% 的查询只需要简单的文档检索，只对 20% 的关系推理类查询使用图谱检索即可。
2. **实体识别的质量决定图谱质量**：垃圾进垃圾出。投入足够精力优化实体识别和关系抽取的精度，必要时用人工审核关键实体和关系。
3. **图谱需要定期更新机制**：企业知识库不断有新文档入库，知识图谱也需要同步更新。设计增量更新管道，而不是每次全量重建。
4. **混合使用向量检索和图谱检索**：不是所有查询都适合图谱检索。用查询路由器判断查询类型：事实查询走图谱，语义查询走向量，复杂查询两者结合。
...
**思路**：在现有实体类型映射中添加三种新类型，为每种类型编写专用的抽取 prompt，让 LLM 针对不同类型的实体提取更精准的属性和关系。
...
ENTITY_TYPES = {
    ...
    "概念": "方法论、理论、术语、标准等",
    "事件": "会议、发布会、事故、历史事件等",
    "产品": "软件产品、硬件设备、服务、APP 等",
    "技术标准": "协议、规范、格式、编码标准等",
    "法律法规": "法律条文、监管政策、行业规定、合规要求等",
}
...
产品包括：软件产品、硬件设备、云服务、APP、API 服务等。

请以 JSON 格式输出:
{{
    "entities": [
        {{"name": "产品名", "type": "产品", "description": "产品简介", "attributes": {{"版本": "", "厂商": "", "用途": ""}}}}
    ],
    "relationships": [
...
关系类型可选: 开发、发布、依赖、竞争、替代"""
    )
    chain = prompt | llm
    result = chain.invoke({"text": text}).content
    return _parse_extraction(result, "产品")


def extract_standard(text: str) -> ExtractionResult:
    """抽取技术标准实体"""
    prompt = ChatPromptTemplate.from_template(
        """从以下文本中抽取技术标准实体及其关系。
...
技术标准包括：通信协议、数据格式、编码规范、API 规范、行业标准等。

请以 JSON 格式输出:
{{
    "entities": [
        {{"name": "标准名", "type": "技术标准", "description": "标准简介", "attributes": {{"发布组织": "", "版本": "", "适用领域": ""}}}}
    ],
    "relationships": [
        {{"source": "实体A", "target": "实体B", "type": "关系类型", "description": "描述"}}
    ]
...
关系类型可选: 制定、兼容、替代、依赖"""
    ...
    chain = prompt | llm
    result = chain.invoke({"text": text}).content
    return _parse_extraction(result, "技术标准")


def extract_law(text: str) -> ExtractionResult:
    """抽取法律法规实体"""
    prompt = ChatPromptTemplate.from_template(
        """从以下文本中抽取法律法规实体及其关系。
...
法律法规包括：法律、行政法规、部门规章、行业规定、合规要求、数据隐私法规等。

请以 JSON 格式输出:
{{
    "entities": [
        {{"name": "法规名", "type": "法律法规", "description": "法规简介", "attributes": {{"发布机构": "", "实施日期": "", "适用范围": ""}}}}
    ],
    "relationships": [
        {{"source": "实体A", "target": "实体B", "type": "关系类型", "description": "描述"}}
    ]
...
关系类型可选: 监管、约束、保护、处罚"""
    ...
    chain = prompt | llm
    result = chain.invoke({"text": text}).content
    return _parse_extraction(result, "法律法规")
```

**要点**：
- 每种实体类型需要独立的抽取 prompt，因为"产品"关注厂商和版本，"法律法规"关注发布机构和实施日期，属性维度完全不同
- 关系类型也要按实体类型定制：产品之间是"竞争/替代"，标准之间是"兼容/依赖"，法规与组织之间是"监管/约束"
- 新类型加入后需更新 `_parse_extraction` 函数中的类型映射，确保图谱构建时能正确分类

---

### 练习 2：图谱持久化（进阶）

**思路**：利用 NetworkX 内置的 `node_link_data` 将图谱序列化为 JSON，反向用 `node_link_graph` 加载。增量更新时，只对新文档抽取的实体和关系涉及的子图进行更新。
...
class PersistentKnowledgeGraph(KnowledgeGraph):
    ...
    def save_to_json(self, filepath: str):
        ...
        data["entity_metadata"] = {
            name: {
                "entity_type": self.entities[name].entity_type,
                "description": self.entities[name].description,
            }
            for name in self.entities
        }
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        print(f"✅ 图谱已保存至 {filepath} "
              f"({self.graph.number_of_nodes()} 节点, "
              f"{self.graph.number_of_edges()} 边)")

    @classmethod
    def load_from_json(cls, filepath: str) -> "PersistentKnowledgeGraph":
        """从 JSON 文件加载图谱"""
        with open(filepath, "r", encoding="utf-8") as f:
            data = json.load(f)

        kg = cls()
        kg.graph = nx.node_link_graph(data, directed=True)

        # 恢复实体元数据
        for name, meta in data.get("entity_metadata", {}).items():
        ...
        print(f"✅ 图谱已从 {filepath} 加载 "
              f"({kg.graph.number_of_nodes()} 节点, "
              f"{kg.graph.number_of_edges()} 边)")
        return kg

    def incremental_update(self, new_documents: list[str]):
        """增量更新：只处理新文档中涉及的实体和关系"""
        affected_nodes = set()
        ...
        for doc in new_documents:
            ...
            # 记录受影响的节点
            for entity in extraction.entities:
                affected_nodes.add(entity.name)
                if entity.name in self.entities:
                    # 更新已有实体的描述
            ...
            for rel in extraction.relationships:
                affected_nodes.add(rel.source)
                affected_nodes.add(rel.target)
                # 检查边是否已存在
                if self.graph.has_edge(rel.source, rel.target):
                    # 更新边的权重
                    old_weight = self.graph.edges[rel.source, rel.target].get("weight", 1.0)
                    self.graph.edges[rel.source, rel.target]["weight"] = old_weight + rel.weight
                else:
                    self.add_relationship(rel)

        print(f"增量更新完成: 影响 {len(affected_nodes)} 个节点")
        return affected_nodes


# 使用示例
kg = PersistentKnowledgeGraph()
kg.build_from_documents(old_documents)
kg.save_to_json("knowledge_graph.json")

# 后续加载并增量更新
kg = PersistentKnowledgeGraph.load_from_json("knowledge_graph.json")
affected = kg.incremental_update(new_documents)
kg.save_to_json("knowledge_graph.json")
```

**要点**：
- `nx.node_link_data` / `node_link_graph` 是 NetworkX 原生的序列化方案，能完整保留节点属性和边属性
- 增量更新时需区分"新增实体"和"更新已有实体"，避免重复添加导致图谱混乱
- 边的权重在增量更新时累加，反映关系在多篇文档中被提及的频率，可用于后续检索排序

---

### 练习 3：混合检索（挑战）

**思路**：将向量检索和图检索的结果融合，用 LLM 判断问题类型（事实查询/推理查询/模糊查询），对不同类型使用不同的检索组合策略，最后用 RRF（Reciprocal Rank Fusion）合并排序。
...
class HybridGraphRAG:
    """混合检索 GraphRAG：结合向量检索和图检索"""

    def __init__(self, kg: KnowledgeGraph, vectorstore, detector: CommunityDetector, llm):
        self.kg = kg
        self.vectorstore = vectorstore
        self.detector = detector
        self.llm = llm
        self.graph_retriever = GraphRetriever(kg, detector, llm)
    ...
    def query(self, question: str) -> str:
        ...
        # Step 2: 根据类型选择检索策略
        if query_type == "fact":
            # 事实查询：优先 Local Search
            graph_result = self.graph_retriever.local_search(question)
            vector_result = self._vector_search(question)
            context = self._merge_results(graph_result, vector_result, weights=(0.7, 0.3))

        elif query_type == "reasoning":
            # 推理查询：使用图路径推理
            entities = self.graph_retriever._extract_entities_from_question(question)
            paths_context = self._reasoning_search(entities, question)
            graph_result = self.graph_retriever.local_search(question)
            context = f"推理路径:\n{paths_context}\n\n实体关系:\n{graph_result}"
            ...
            # 模糊查询：向量 + 全局图谱
            vector_result = self._vector_search(question)
            global_result = self.graph_retriever.global_search(question)
            context = self._merge_results(global_result, vector_result, weights=(0.5, 0.5))

        # Step 3: 生成答案
        return self._generate_answer(question, context)

    def _classify_query(self, question: str) -> Literal["fact", "reasoning", "vague"]:
        """判断问题类型"""
        prompt = ChatPromptTemplate.from_template(
            """判断以下问题的类型:
- fact: 关于特定实体的事实查询（如"谁是 X 的 CEO"）
- reasoning: 需要多步推理（如"A 和 B 有什么间接关系"）
- vague: 模糊、概括性问题（如"行业趋势如何"）

问题: {question}

只回答 fact、reasoning 或 vague。"""
        )
        chain = prompt | self.llm
        result = chain.invoke({"question": question}).content.strip().lower()
        if result in ("fact", "reasoning", "vague"):
            return result
        return "vague"

    def _vector_search(self, question: str, k: int = 5) -> str:
        """向量检索"""
        docs = self.vectorstore.similarity_search(question, k=k)
        return "\n".join([f"- {d.page_content}" for d in docs])

    def _reasoning_search(self, entities: list[str], question: str) -> str:
        """图路径推理：查找实体间的多跳关系"""
        if len(entities) < 2:
            return "无法进行路径推理（需要至少两个实体）"

        path_contexts = []
        for i in range(len(entities)):
            for j in range(i + 1, len(entities)):
                paths = self.kg.find_paths(entities[i], entities[j])
                for path in paths[:3]:  # 最多 3 条路径
                    path_desc = " → ".join(path)
                    # 沿途收集关系
                    rels = []
                    for k in range(len(path) - 1):
                        edge_data = self.kg.graph.edges.get((path[k], path[k + 1]), {})
                        rel = edge_data.get("relation_type", "相关")
                        rels.append(f"{path[k]} --[{rel}]--> {path[k + 1]}")
                    path_contexts.append(f"路径: {path_desc}\n" + "\n".join(rels))

        return "\n\n".join(path_contexts) if path_contexts else "未找到实体间的路径"

    def _merge_results(
        self, result_a: str, result_b: str, weights: tuple[float, float] = (0.5, 0.5)
    ) -> str:
        """融合两个检索结果"""
        return (
            f"[来源 1, 权重 {weights[0]}]\n{result_a}\n\n"
...
**要点**：
- 问题分类是混合检索的核心路由，决定图检索和向量检索的权重分配
- 推理查询利用图的多跳路径（`find_paths`），这是纯向量检索无法提供的能力
- 结果融合时权重可调，事实查询偏向图谱（0.7），模糊查询两者均衡（0.5）
