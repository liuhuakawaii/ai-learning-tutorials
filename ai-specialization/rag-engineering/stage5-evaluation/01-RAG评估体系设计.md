...
你的 RAG 系统已经上线两个月了，产品团队想知道"系统表现怎么样"。你调出日志发现，除了"请求成功/失败"之外，没有任何关于回答质量的数据。上周用户投诉"回答不准确"，你不知道是检索没找到正确文档，还是 LLM 找到了但没正确引用。没有评估体系，你连"系统是变好了还是变差了"都无法回答，更别说做针对性优化。评估体系是 RAG 系统从"能用"到"好用"的基础设施。
...
1. **评估数据集是最重要的资产**：没有好的评估数据集，所有指标都不可信。投入足够时间和精力构建覆盖各类场景的 ground truth，这是所有优化的基础。
2. **组件级和系统级指标要同时看**：只看端到端指标无法定位问题，只看组件指标可能忽略系统级的交互效应。两者结合才能全面了解系统状态。
3. **自动化评估和人工评估互补**：自动化评估适合高频、低成本的质量监控，人工评估适合定期的深度审核。不要只依赖其中一种。
4. **评估流水线要集成到 CI/CD**：每次模型或配置变更，都要自动跑一轮评估。如果指标下降超过阈值，阻止部署上线。
...
**思路**：使用课中的 `EvalDatasetBuilder` 和 `EvalSample`，按要求覆盖概念类、操作类、故障排查类三个类别，并分配 easy/medium/hard 难度。建议先列出每个类别下用户常问的问题，再补充标准答案和相关文档内容。
...
samples = [
    ...
    EvalSample(
        query="什么是 RAG？",
        ground_truth_answer="RAG (Retrieval-Augmented Generation) 是一种结合外部知识检索与大语言模型生成的技术框架。",
        relevant_doc_contents=["RAG 是一种结合检索和生成的技术..."],
        difficulty="easy",
        category="concept",
    ),
    EvalSample(
        ...
        ground_truth_answer="向量数据库专门存储和检索高维向量，支持相似度搜索；传统数据库基于结构化查询，擅长精确匹配。",
        relevant_doc_contents=["向量数据库使用 ANN 索引进行近似最近邻搜索..."],
        difficulty="medium",
        category="concept",
    ),
    EvalSample(
        query="RAG 中的 Embedding 模型如何选择？",
        ground_truth_answer="选择 Embedding 模型需要考虑：1) 语言支持（中英文场景选不同模型）；2) 维度大小（影响存储和检索速度）；3) 评测基准得分（MTEB 排行榜）；4) 推理速度和部署成本。",
        relevant_doc_contents=["Embedding 模型选型要考虑任务场景、语言、维度和性能..."],
        difficulty="hard",
        category="concept",
    ),
    # ---- 操作类 (operation) ----
    EvalSample(
        query="如何安装 ChromaDB？",
        ground_truth_answer="使用 pip install chromadb 安装，或 pip install chromadb[all] 安装包含所有依赖的完整版本。",
        relevant_doc_contents=["ChromaDB 安装命令: pip install chromadb"],
        difficulty="easy",
        category="operation",
    ),
    EvalSample(
        query="如何对文档进行分块处理？",
        ground_truth_answer="常见的分块策略包括：固定大小分块（按字符数或 token 数切分）、语义分块（按段落或语义边界切分）、递归分块（先按大块切分再递归细分）。推荐使用 LangChain 的 RecursiveCharacterTextSplitter。",
        relevant_doc_contents=["文档分块策略: 固定大小、语义分块、递归分块..."],
        difficulty="medium",
        category="operation",
    ),
    EvalSample(
        query="如何实现混合检索策略？",
        ground_truth_answer="混合检索需要：1) 同时构建 BM25 关键词索引和向量索引；2) 分别执行检索并获取结果；3) 使用 RRF (Reciprocal Rank Fusion) 或加权融合合并两路结果；4) 可选地接入 Reranker 做二次排序。",
        relevant_doc_contents=["混合检索结合 BM25 和向量检索，通过 RRF 融合..."],
        difficulty="hard",
        category="operation",
    ),
    # ---- 故障排查类 (troubleshooting) ----
    EvalSample(
        query="RAG 回答"我不知道"怎么办？",
        ground_truth_answer="可能是检索未命中相关文档。检查：1) 查询是否与文档语义匹配；2) Embedding 模型是否适配；3) top_k 是否太小；4) 相似度阈值是否过高。",
        relevant_doc_contents=["检索未命中时，需要检查查询和文档的匹配度..."],
        difficulty="medium",
        category="troubleshooting",
    ),
    EvalSample(
        query="RAG 出现幻觉怎么解决？",
        ground_truth_answer="解决幻觉问题的策略：1) 在 Prompt 中明确要求"仅基于提供的上下文回答"；2) 添加 Faithfulness 检测环节；3) 使用引用标注，要求模型标注信息来源；4) 降低模型温度参数；5) 后处理阶段验证回答是否包含上下文未提及的信息。",
        relevant_doc_contents=["幻觉解决: 限制 Prompt、引用标注、温度控制..."],
        difficulty="hard",
        category="troubleshooting",
    ),
]

# 使用 EvalDatasetBuilder 构建数据集
class EvalDatasetBuilder:
...
builder = EvalDatasetBuilder("rag_eval_v1", "RAG 系统评估数据集 v1")
...
**要点**：
- 评估数据集至少覆盖 3 个类别（概念、操作、故障排查），每类有不同难度的样本
- `EvalSample` 的 `sample_id` 基于 query 和 answer 的 MD5 自动生成，确保去重
- 保存时同时输出统计信息，便于检查难度和类别的分布是否均衡

---

### 练习 2: 实现自定义指标 (进阶)

**思路**：继承 `BaseMetric`，实现 `AnswerCompleteness` 指标。核心逻辑是将 ground_truth_answer 按句拆分为关键信息点，然后检查每条信息点是否在 answer 中有覆盖。可以先用简单的关键词匹配作为降级方案，再用 LLM 做精确判断。
...
class AnswerCompleteness(BaseMetric):
    """回答完整性指标: 评估 ground_truth 中关键信息在 answer 中的覆盖率"""
    ...
    def compute(
        ...
        **kwargs,
        ...
        scores = []
        for query, answer, gt in zip(queries, answers, ground_truths):
            if self.llm_client:
                score = self._llm_score(query, answer, gt)
            else:
                score = self._keyword_score(answer, gt)
            scores.append(score)
    ...
    def _keyword_score(self, answer: str, ground_truth: str) -> float:
        """基于关键词的简单覆盖计算"""
        gt_sentences = [s.strip() for s in ground_truth.replace("。", ".").split(".") if s.strip()]
        if not gt_sentences:
            return 1.0
        ...
        for sent in gt_sentences:
            keywords = [w for w in sent if len(w) > 1]
            if any(kw in answer_lower for kw in keywords):
                covered += 1

        return covered / len(gt_sentences)

    def _llm_score(self, query: str, answer: str, ground_truth: str) -> float:
        """使用 LLM 评估完整性"""
        prompt = f"""评估回答的完整性。
...
从参考答案中提取关键信息点，检查候选回答覆盖了多少。
返回 0.0 到 1.0 的浮点数:"""
        try:
            response = self.llm_client.generate(prompt)
            return max(0.0, min(1.0, float(response.strip())))
        except (ValueError, AttributeError):
            return self._keyword_score(answer, ground_truth)
    ...
    def compute_all(self, **kwargs):
        ...
        for name, metric in self._metrics.items():
            try:
                ...
                results[name] = EvalResult(metric_name=name, value=0.0, details={"error": str(e)})
        return results


# 测试
registry = MetricRegistry()
registry.register(AnswerCompleteness())

queries = ["什么是 RAG？"]
answers = ["RAG 是检索增强生成技术，结合了信息检索和大语言模型。"]
ground_truths = ["RAG (Retrieval-Augmented Generation) 是一种结合外部知识检索与大语言模型生成的技术框架。"]
...
**要点**：
- `AnswerCompleteness` 继承自定义基类，实现 `compute` 方法，接受 queries、answers、ground_truths 三个参数
- 提供两种评分策略：关键词匹配（无 LLM 时的降级方案）和 LLM 评估（精确但需要 API）
- 使用 `_keyword_score` 时按句拆分 ground_truth，逐句检查关键词覆盖，避免长文本匹配偏差

---

### 练习 3: 端到端评估 (综合)

**思路**：搭建完整流水线需要五步：构建数据集 → 接入 RAG 系统 → 计算指标 → 生成报告 → 提出优化建议。关键是指标要全面（检索指标 + 生成指标 + 系统指标），报告要能定位问题，建议要具体可执行。
...
class FullEvalPipeline:
    def __init__(self, rag_system, metrics, output_dir="eval_results"):
        self.rag = rag_system
        self.metrics = metrics
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)
    ...
    def run(self, eval_dataset, run_name=""):
        ...
        for metric in self.metrics:
            result = metric.compute(
                ...
                answers=[o["answer"] for o in rag_outputs],
                retrieved_ids=[o.get("retrieved_ids", []) for o in rag_outputs],
                relevant_ids=[s.get("relevant_doc_ids", []) for s in eval_dataset],
                ground_truths=[s.get("ground_truth_answer", "") for s in eval_dataset],
            )
        ...
        # Step 4: 生成优化建议
        report.recommendations = self._generate_recommendations(metric_results)
        ...
        print("=" * 50)
        print(f"评估完成: {run_id}")
        print(f"耗时: {duration:.1f}s | 样本: {len(eval_dataset)}")
        print("-" * 50)
        for name, value in report.metrics.items():
            print(f"  {name}: {value:.4f}")
        print("-" * 50)
    ...
    def _generate_recommendations(self, metric_results):
        recs = []
        for name, result in metric_results.items():
            if "Recall" in name and result.value < 0.7:
                recs.append(f"检索召回率偏低 ({result.value:.2f})：建议优化 Embedding 模型或增加 top_k 值，同时考虑添加混合检索策略")
            if "Precision" in name and result.value < 0.5:
                recs.append(f"检索精确度偏低 ({result.value:.2f})：建议引入 Reranker 对检索结果重排序，或缩小分块粒度减少噪音")
            if "Faithfulness" in name and result.value < 0.7:
                recs.append(f"忠实度偏低 ({result.value:.2f})：建议在 Prompt 中强化"仅基于上下文回答"的约束，并添加引用标注机制")
            if "Relevancy" in name and result.value < 0.6:
                recs.append(f"回答相关性偏低 ({result.value:.2f})：建议优化 Prompt 模板，明确要求直接回答用户问题，避免生成无关信息")
        if not recs:
            recs.append("所有指标表现良好，建议定期运行评估以监控系统稳定性")
        return recs
...
class MockRAG:
    def query(self, q):
        return {"answer": f"关于'{q}'的回答", "contexts": ["..."], "retrieved_ids": ["d1"]}

class RecallMetric:
    name = "Recall@5"
    def compute(self, retrieved_ids, relevant_ids, **kw):
        vals = []
        for ret, rel in zip(retrieved_ids, relevant_ids):
            vals.append(len(set(ret) & set(rel)) / max(len(set(rel)), 1))
        return type("R", (), {"value": np.mean(vals), "details": {}})()

class FaithfulnessMetric:
    name = "Faithfulness"
    def compute(self, answers, ground_truths, **kw):
        vals = []
        for a, g in zip(answers, ground_truths):
            overlap = len(set(a.split()) & set(g.split())) / max(len(set(g.split())), 1)
            vals.append(min(overlap, 1.0))
        return type("R", (), {"value": np.mean(vals), "details": {}})()

pipeline = FullEvalPipeline(
    rag_system=MockRAG(),
    metrics=[RecallMetric(), FaithfulnessMetric()],
)

eval_data = [
    {"query": "什么是 RAG？", "ground_truth_answer": "RAG 是检索增强生成", "relevant_doc_ids": ["d1"]},
    {"query": "如何优化检索？", "ground_truth_answer": "混合检索和重排序", "relevant_doc_ids": ["d2"]},
]
report = pipeline.run(eval_data, run_name="baseline")
```

**要点**：
- 完整 Pipeline 包含五个步骤：数据加载 → RAG 执行 → 指标计算 → 报告生成 → 建议输出，每步解耦便于独立优化
- `_generate_recommendations` 根据各指标的阈值自动产出可执行的优化建议，而不是笼统的"需要改进"
- 报告同时保存 JSON（便于程序化处理）和控制台输出（便于人工阅读），支持历史对比
