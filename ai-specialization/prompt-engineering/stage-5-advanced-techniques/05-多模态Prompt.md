...
你收到一张包含复杂数据图表的截图，需要从中提取数据并分析趋势。用纯文本 Prompt 无法描述这张图片的内容。或者你的电商平台有上万张商品图片需要自动生成描述文字和 alt text。在这些场景中，纯文本 Prompt 已经不够用了——我们需要让 LLM 能够"看懂"图片，结合视觉信息进行推理和生成。
...
1. 图片分辨率至少 512x512，过低的分辨率会严重影响识别准确率
2. 对于批量处理场景，使用异步调用和限速控制，避免触发 API 速率限制
3. 图片预处理（裁剪、增强、压缩）能显著提升分析效果，不要直接使用原始大图
4. 为不同类型的视觉任务设计专用 Prompt 模板，避免使用过于通用的描述
...
**思路**：使用 GPT-4 Vision 进行 OCR 的关键是设计好 Prompt——要明确指出识别语言、输出格式、以及对不确定字符的处理方式。对于包含表格或结构化文字的图片，应要求模型以 Markdown 格式输出以保留排版信息。
...
if __name__ == "__main__":
    result = extract_text_from_image(
        image_path="path/to/document_with_text.jpg",
        languages = "中文和英文"
    )
    print(result)
```
...
**要点**：
- `detail: "high"` 会消耗更多 Token 但识别精度显著提升，对于文字密集的图片必须使用
- Prompt 中要求标注不确定字符可以提高结果的可信度，让用户知道哪些需要人工复核
- 对于表格类内容，明确要求 Markdown 格式输出比纯文本描述更实用

---

### 练习 2

**思路**：多图对比的关键是在一个请求中同时传入多张图片，并设计 Prompt 从多个维度（颜色、构图、内容、风格等）进行结构化对比。需要注意 API 对单次请求图片数量和总 Token 的限制。
...
def compare_two_images(
    ...
    if comparison_aspects is None:
        comparison_aspects = ["内容主题", "颜色色调", "构图布局", "风格特点", "细节差异"]
    ...
    img1_data, img1_mime = load_image(image_path_1)
    img2_data, img2_mime = load_image(image_path_2)
...
[哪张图片在哪些方面更优，给出简要评价]"""
    ...
    response = client.chat.completions.create(
        ...
        messages=[{
            ...
            "content": [
                {"type": "text", "text": prompt},
                {"type": "image_url", "image_url": {"url": f"data:{img1_mime};base64,{img1_data}", "detail": "high"}},
                {"type": "image_url", "image_url": {"url": f"data:{img2_mime};base64,{img2_data}", "detail": "high"}}
            ]
        }],
        max_tokens=1500
    )
...
if __name__ == "__main__":
    result = compare_two_images(
        image_path_1="path/to/image1.jpg",
        image_path_2="path/to/image2.jpg",
        comparison_aspects=["内容主题", "颜色色调", "构图布局"]
    )
    print(result)
```

**要点**：
- 多图请求中每张图片都会消耗 Token，两张 high detail 图片可能占用数千 Token，需注意成本
- 对比维度应根据具体场景定制——产品图片侧重外观差异，设计稿侧重布局和配色
- 如果图片超过 2 张，建议分批对比再汇总，避免单次请求 Token 超限

---

### 练习 3

**思路**：完整的文档解析 Pipeline 需要三个阶段：先用通用描述 Prompt 识别文档类型（发票、合同、简历等），再根据类型选择专用的结构化提取 Prompt，最后输出标准化的 JSON 结果。关键是设计一个路由机制，根据第一阶段的识别结果动态选择第二阶段的处理策略。
...
def load_image_base64(image_path: str) -> tuple:
    """加载图片并返回 base64 和 MIME 类型"""
    with open(image_path, "rb") as f:
        data = base64.b64encode(f.read()).decode("utf-8")
    ext = Path(image_path).suffix.lower()
    mime_map = {".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png", ".webp": "image/webp"}
...
def call_vision(image_path: str, prompt: str, model: str = "gpt-4o") -> str:
    """调用 Vision 模型"""
    img_data, img_mime = load_image_base64(image_path)
    response = client.chat.completions.create(
        model=model,
        messages=[{
            "role": "user",
            "content": [
                {"type": "text", "text": prompt},
                {"type": "image_url", "image_url": {"url": f"data:{img_mime};base64,{img_data}", "detail": "high"}}
            ]
        }],
        max_tokens=2000
    )
...
class DocumentParserPipeline:
    ...
    # 不同文档类型的专用 Prompt
    TYPE_PROMPTS = {
        "发票": """请从这张发票图片中提取以下信息，以 JSON 格式输出：
{
    "发票号码": "",
    "开票日期": "",
    "销售方": "",
    "购买方": "",
    "商品明细": [{"名称": "", "数量": 0, "单价": 0, "金额": 0}],
    "合计金额": 0,
    "税额": 0,
    "价税合计": 0
}""",

        "简历": """请从这份简历图片中提取以下信息，以 JSON 格式输出：
{
    "姓名": "",
    "联系方式": {"电话": "", "邮箱": ""},
    "教育背景": [{"学校": "", "专业": "", "学历": "", "时间": ""}],
    ...
    "项目经验": [{"名称": "", "描述": ""}]
        ...
        "合同": """请从这份合同图片中提取以下关键信息，以 JSON 格式输出：
{
    "合同名称": "",
    "甲方": "",
    "乙方": "",
    ...
    "违约责任": ""
        ...
        "表格": """请将图片中的表格转换为 JSON 格式：
{
    "表头": [],
    "数据行": []
}""",

        "通用文档": """请提取文档中的所有文字内容，保持原始结构，以 Markdown 格式输出。"""
    }

    def classify_document(self, image_path: str) -> str:
        """第一步：识别文档类型"""
        prompt = """请判断这张图片属于以下哪种文档类型，只输出类型名称：
- 发票
- 简历
- 合同
- 表格
...
只输出一个类型名称，不要解释。"""
        ...
        result = call_vision(image_path, prompt, temperature=0)
    ...
    def extract_structured_info(self, image_path: str, doc_type: str) -> str:
        """第二步：根据类型提取结构化信息"""
        prompt = self.TYPE_PROMPTS[doc_type]
        return call_vision(image_path, prompt, temperature=0)

    def validate_and_format(self, raw_output: str) -> Dict:
        """第三步：验证并格式化输出"""
        # 尝试解析 JSON
        try:
            # 提取 JSON 部分（模型有时会附带说明文字）
        ...
        # JSON 解析失败，返回原始文本
        return {"status": "text_only", "data": raw_output}

    def parse(self, image_path: str) -> Dict:
        """完整解析流程"""
        print(f"解析文档: {image_path}")
        ...
        # 第二步：提取
        raw_output = self.extract_structured_info(image_path, doc_type)
        print(f"  提取完成")

        # 第三步：验证
        result = self.validate_and_format(raw_output)
    ...
    def batch_parse(self, image_paths: List[str]) -> List[Dict]:
        """批量解析"""
        results = []
...
if __name__ == "__main__":
    ...
    # 单个文档解析
    result = pipeline.parse("path/to/invoice.jpg")
    print(json.dumps(result, ensure_ascii=False, indent=2))

    # 批量解析
    paths = ["invoice1.jpg", "resume1.png", "contract1.jpg"]
    # results = pipeline.batch_parse(paths)
```

**要点**：
- 三阶段 Pipeline（分类 → 提取 → 验证）比单次调用更可靠——分类错误可以 fallback 到通用文档
- JSON 输出时模型经常在 JSON 前后附加说明文字，解析时需要用 `{` 和 `}` 定位 JSON 部分
- 对于生产环境，应在验证阶段添加字段完整性检查和数据类型校验
