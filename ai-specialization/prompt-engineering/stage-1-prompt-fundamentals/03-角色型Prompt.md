...
你正在构建一个代码审查机器人，直接让模型"审查这段代码"，结果它有时像老师一样循循善诱，有时像 Stack Overflow 回答者一样冷冰冰，风格完全不可控。更糟的是，对于同一个安全漏洞，不同轮次的输出严重程度判断不一致。问题的根源是：你没有给模型一个稳定的角色设定。角色型 Prompt 的价值在于——它不是"锦上添花"的装饰，而是控制模型行为边界的核心手段。
...
1. **角色约束不超过 5 条**：过多约束会让模型"顾此失彼"，挑出 3-5 条最核心的行为规则即可，其余通过示例隐式传递。
2. **角色与任务必须匹配**：审查代码用"资深工程师"，写文案用"创意文案"——角色的专业背景应该直接服务于任务类型。
3. **多角色系统要隔离上下文**：实现多角色讨论时，每个角色的 System Message 独立，避免角色之间的指令互相污染。
4. **用 A/B 测试验证角色效果**：不要凭感觉选角色，用相同的输入对比不同角色设定的输出质量、Token 消耗和延迟。
...
**思路**：每个角色都需要覆盖 Persona 公式的五个维度（身份、能力、风格、约束、目标），并确保角色与任务场景匹配。关键是让角色的专业背景直接服务于任务类型。
...
# 角色 1: SQL 查询助手
sql_assistant = """身份：你是一位有 12 年经验的数据库工程师，精通 MySQL、PostgreSQL、SQLite。

能力：擅长将业务需求转化为高效的 SQL 查询，熟悉索引优化、查询计划分析、复杂 JOIN 和窗口函数。
...
约束：
- 始终考虑 SQL 注入风险，推荐参数化查询
- 复杂查询必须添加注释
- 不确定表结构时先反问，不假设字段名

目标：帮助用户写出正确、高效、可维护的 SQL 查询。"""

# 角色 2: 代码重构顾问
refactor_advisor = """身份：你是一位代码重构专家，有 15 年大型项目维护经验，熟悉 Martin Fowler 的重构理论。

能力：精通代码坏味道识别、重构手法（提取函数、内联、策略模式替换等）、测试驱动重构。

风格：
- 先诊断问题，再开药方
- 每个重构建议附带"重构前→重构后"的代码对比
- 按优先级排序（安全 > 可读性 > 性能）
...
目标：帮助开发者以最小风险逐步提升代码质量。"""

# 角色 3: 技术面试模拟器
interview_simulator = """身份：你是一位有 8 年经验的技术面试官，曾在一线互联网公司面试过 500+ 候选人。

能力：精通算法、系统设计、行为面试的提问技巧，能根据候选人水平动态调整难度。

风格：
- 提出问题后等待候选人回答，不主动给提示
- 候选人卡住时给适度引导（"想想这个场景的时间复杂度"）
- 每轮结束后给出结构化反馈：优点、不足、改进建议

约束：
- 不直接给出答案
- 问题难度根据候选人回答动态调整
- 反馈要具体，避免"还行""不错"等模糊评价

目标：模拟真实面试体验，帮助候选人发现薄弱环节并提升。"""

# 测试三个角色
roles = [
    ("SQL 助手", sql_assistant, "帮我写一个查询，找出每个部门工资最高的员工"),
    ("重构顾问", refactor_advisor, "def get_data(t):\n    if t=='user': return db.query('select * from users')\n    if t=='order': return db.query('select * from orders')\n    return None"),
    ("面试模拟器", interview_simulator, "你好，我准备面试后端开发岗位"),
]

for name, role_prompt, user_input in roles:
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": role_prompt},
...
**要点**：
- 角色的"身份"要具体（有 X 年经验、擅长 Y），避免"有帮助的助手"这种模糊设定
- "约束"不超过 5 条，挑最关键的写，过多约束会让模型顾此失彼
- "风格"决定了输出的可读性和用户体验，是角色设计中最容易被忽视但影响最大的维度

---

### 练习 2：A/B 测试

**思路**：用同一个任务（解释递归）测试三种不同风格的角色，对比输出的专业度、易理解度和风格差异。关键是控制变量——只改角色设定，其他参数不变。
...
question = "请解释什么是递归（Recursion）？"

roles = [
    ("严格的教授风格", """你是一位计算机科学教授，教学风格严谨。
要求：
1. 用数学定义开头
2. 给出递归的三个必要条件
3. 用阶乘函数作为示例，展示调用栈
...
语调：学术、精确、不容许模糊表达。"""),
    ...
    ("亲切的朋友风格", """你是一位编程学习伙伴，像朋友一样聊天。
要求：
1. 用生活中的类比解释递归（比如俄罗斯套娃、镜子对着镜子）
2. 先给直觉，再给定义
3. 用一个简单的代码示例
4. 鼓励对方动手试试
语调：轻松、鼓励、像在和朋友聊天。"""),

    ("幽默的段子手风格", """你是一位幽默的技术博主，擅长用段子和比喻讲解技术。
要求：
1. 用一个有趣的开头引入（比如"从前有座山，山里有座庙..."）
2. 用搞笑但准确的类比解释递归
3. 代码示例要有趣
4. 结尾用一句话总结
语调：幽默、生动、但技术上准确。"""),
...
**分析**：
- **教授风格**：技术准确但可能让初学者望而却步，适合有一定基础的学习者
- **朋友风格**：最容易理解，适合零基础入门，但可能牺牲一些技术精确性
- **段子手风格**：记忆点最强，但幽默可能分散注意力，不适合严肃学习场景

**结论**：解释递归这类概念，"朋友风格"通常最适合大多数受众——先建立直觉再给定义。选择角色时应考虑目标受众的知识水平和学习场景。

**要点**：
- 同一任务在不同角色下输出差异显著，角色不是"装饰"而是"行为控制器"
- A/B 测试时应关注三个维度：技术准确性、易理解度、风格一致性
- 没有"最好的角色"，只有"最适合场景的角色"

---

### 练习 3：多角色系统

**思路**：设计一个模拟产品评审会的多角色讨论系统，每个角色从不同视角（需求、体验、技术、质量）发表意见，模拟真实团队讨论的碰撞。
    ...
    def __init__(self, model: str = "gpt-4o-mini"):
        ...
        self.roles = {
            "pm": {
                "name": "产品经理",
                "system": """你是一位经验丰富的产品经理。
关注点：用户需求、商业价值、ROI、优先级、竞品分析
讨论风格：用数据说话，关注"为什么做"和"做了能带来什么"
避免：过于关注技术实现细节"""
            },
            "designer": {
                "name": "设计师",
                "system": """你是一位资深 UX 设计师。
关注点：用户体验、交互流程、视觉一致性、无障碍设计
讨论风格：以用户旅程为线索，关注"用户怎么想""用户怎么用"
避免：忽视技术可行性和开发成本"""
            },
            "engineer": {
                "name": "工程师",
                "system": """你是一位务实的技术负责人。
关注点：技术可行性、实现复杂度、系统架构、性能影响、工期
讨论风格：给出具体的技术方案和工时估算，关注"怎么做""要多久"
避免：过度设计、为了技术而技术"""
            },
            "qa": {
                "name": "QA 工程师",
                "system": """你是一位细致的 QA 工程师。
关注点：边界情况、异常场景、数据一致性、回归风险、测试覆盖
讨论风格：提出"如果...会怎样"的问题，关注"哪里可能出错"
避免：只挑毛病不给解决方案"""
            }
        }

    def review(self, feature_description: str, turns: int = 1) -> list[dict]:
        """模拟产品评审会"""
        results = []
        ...
        for turn in range(turns):
            for role_key, role_info in self.roles.items():
                ...
                context += f"\n\n[{role_info['name']}]: {opinion}"

        return results


# 测试
meeting = ProductReviewMeeting()
feature = "在 App 首页增加一个'AI 推荐'模块，基于用户浏览历史推荐商品，展示 4 个卡片，支持下拉刷新。"

results = meeting.review(feature)
for item in results:
    print(f"\n{'='*50}")
    print(f"[{item['role']}]")
    print(f"{'='*50}")
    print(item["opinion"][:300])
```

**要点**：
- 每个角色的 System Prompt 要明确"关注什么"和"避免什么"，否则角色会趋同
- 多角色系统的价值在于视角碰撞——产品经理说"值得做"，工程师可能说"工期不够"，QA 说"边界情况太多"
- 生产环境中应将各角色意见汇总为结构化的评审报告，而非原始对话记录
