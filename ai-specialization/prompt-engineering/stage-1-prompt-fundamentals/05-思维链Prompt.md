...
**思路**：选择 5 道需要多步推理的数学应用题，分别用直接回答和 Zero-shot CoT 测试，对比准确率和 Token 消耗。预期 CoT 在多步推理题上准确率更高，但 Token 消耗也更大。
...
problems = [
    {"question": "一个水池有两个水管，A 管每小时注水 3 吨，B 管每小时排水 1 吨。水池容量 20 吨，从空开始多久能注满？", "answer": "10"},
    {"question": "小明买了 3 本书和 2 支笔，共花了 55 元。已知一本书比一支笔贵 10 元，请问书多少钱一本？", "answer": "15"},
    {"question": "一列火车长 200 米，以 72km/h 的速度通过一座 800 米长的桥，需要多少秒？", "answer": "50"},
    {"question": "鸡兔同笼，共有 35 个头，94 只脚，鸡有多少只？", "answer": "23"},
    {"question": "甲乙两人从相距 100 公里的两地同时出发相向而行，甲每小时走 6 公里，乙每小时走 4 公里，几小时后相遇？", "answer": "10"},
]
...
def cot_answer(question: str) -> dict:
    response = client.chat.completions.create(
        model="gpt-4o-mini",
...
for i, p in enumerate(problems):
    d = direct_answer(p["question"])
    c = cot_answer(p["question"])

    d_match = p["answer"] in d["answer"]
    c_match = p["answer"] in c["answer"]

    direct_correct += int(d_match)
    cot_correct += int(c_match)
    direct_tokens += d["tokens"]
    cot_tokens += c["tokens"]

    print(f"问题 {i+1}: {p['question'][:40]}...")
    print(f"  直接回答: {d['answer'][:60]} {'✓' if d_match else '✗'} ({d['tokens']} tokens)")
    print(f"  CoT 回答: {c['answer'][:60]} {'✓' if c_match else '✗'} ({c['tokens']} tokens)")
    print()

n = len(problems)
print(f"总结:")
print(f"  直接回答 准确率: {direct_correct}/{n} = {direct_correct/n:.0%}, 总 Token: {direct_tokens}")
print(f"  CoT 回答 准确率: {cot_correct}/{n} = {cot_correct/n:.0%}, 总 Token: {cot_tokens}")
print(f"  Token 增幅: {(cot_tokens/direct_tokens - 1)*100:.0f}%")
```

**要点**：
- CoT 在多步数学推理上准确率通常高于直接回答，特别是行程问题、鸡兔同笼等经典题型
- CoT 的 Token 消耗约为直接回答的 2-4 倍，需要在准确率和成本间权衡
- 即使 CoT 最终答案正确，也应抽查推理过程是否逻辑一致——可能存在"答案对但推理错"的假阳性
- 简单算术题（如 1+1）用 CoT 反而是浪费

---

### 练习 2：Manual CoT 示例设计

**思路**：为逻辑推理任务设计 3 个难度递进的 CoT 示例，每个示例的推理步骤清晰、每步一个结论。覆盖不同类型的逻辑问题（条件推理、集合推理、反证推理）。
...
class ManualCoT:
    ...
    def add_example(self, question: str, steps: list[str], answer: str):
        self.examples.append({"question": question, "steps": steps, "answer": answer})

    def solve(self, question: str) -> dict:
        messages = [
            {"role": "system", "content": "你是一个善于逻辑推理的助手。对每个问题，展示清晰的推理步骤，每步一个结论。最终答案用"最终答案:"开头。"}
        ]
        for ex in self.examples:
            reasoning = "\n".join(f"  {i+1}. {s}" for i, s in enumerate(ex["steps"]))
            messages.append({"role": "user", "content": f"问题: {ex['question']}"})
            messages.append({"role": "assistant", "content": f"推理过程:\n{reasoning}\n\n最终答案: {ex['answer']}"})
        messages.append({"role": "user", "content": f"问题: {question}"})
        ...
        output = response.choices[0].message.content
        answer = output.split("最终答案:")[-1].strip() if "最终答案:" in output else ""
        return {"reasoning": output, "answer": answer, "tokens": response.usage.total_tokens}
...
# 示例 2: 集合推理
cot.add_example(
    question="班级里 30 人学数学，20 人学物理，10 人既学数学又学物理。班级至少有多少人？",
    steps=[
        "学数学的人数: |M| = 30",
        "学物理的人数: |P| = 20",
        "既学数学又学物理的人数: |M∩P| = 10",
...
cot.add_example(
    ...
    steps=[
        ...
        "甲在说谎 → 乙没有说谎 → 乙说真话",
        "乙说真话 → 丙在说谎",
        "验证：丙在说谎 → "甲和乙都在说谎"为假 → 至少一人说真话 → 乙说真话，一致"
    ],
    answer="乙说真话"
)
...
# 测试新问题
test_questions = [
    "如果 A 比 B 高，B 比 C 高，C 比 D 矮，那么谁最矮？",
    "一个盒子里有红球和蓝球共 10 个。如果拿出 2 个红球后，红球比蓝球少 2 个。盒子里原来有几个红球？",
    "甲乙丙三人分别是教师、医生、律师。已知：(1) 甲不是教师 (2) 乙不是医生 (3) 教师不知道丙的名字。三人各是什么职业？",
]

print("=== Manual CoT 逻辑推理 ===\n")
...
**要点**：
- 示例难度应递进：从简单的单步推理到复杂的多步推理
- 每个推理步骤只包含一个结论，避免在单步中做多个判断
- 示例应覆盖不同推理类型（条件推理、集合推理、反证推理），让模型学会通用的推理模式
- 最终答案用固定格式（如"最终答案:"）标记，方便程序提取

---

### 练习 3：Self-Consistency 实验

**思路**：实现 Self-Consistency 系统，测试采样次数和 Temperature 两个参数对准确率的影响。预期：采样次数越多、Temperature 适中时准确率最高。
...
class SelfConsistencyTester:
    ...
    def solve_once(self, question: str, temperature: float) -> str:
        response = client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "你是一个善于推理的助手。逐步推理后用"答案: XXX"给出最终答案。"},
                {"role": "user", "content": f"{question}\n\n让我们一步步推理:"}
            ],
        ...
        output = response.choices[0].message.content
        for marker in ["答案:", "答案：", "Answer:"]:
            if marker in output:
                return output.split(marker)[-1].strip().split("\n")[0].strip()
        return output.strip().split("\n")[-1].strip()

    def solve_consistent(self, question: str, n_samples: int, temperature: float) -> dict:
        answers = [self.solve_once(question, temperature) for _ in range(n_samples)]
...
# 测试题
test_problems = [
    {"question": "一个班级有 45 人，男生比女生多 5 人，男生有多少人？", "answer": "25"},
    {"question": "鸡兔同笼，35 个头 94 只脚，鸡有多少只？", "answer": "23"},
    {"question": "小明从家到学校，去时速度 4km/h，回时速度 6km/h，往返平均速度是多少？", "answer": "4.8"},
]

tester = SelfConsistencyTester()

# 实验 1: 采样次数 vs 准确率 (固定 Temperature=0.5)
print("=== 实验 1: 采样次数 vs 准确率 (Temperature=0.5) ===\n")
for n in [1, 3, 5, 7]:
    correct = 0
    for p in test_problems:
        result = tester.solve_consistent(p["question"], n_samples=n, temperature=0.5)
        if p["answer"] in result["answer"]:
            correct += 1
    print(f"采样 {n} 次: 准确率 {correct}/{len(test_problems)} = {correct/len(test_problems):.0%}")

# 实验 2: Temperature vs 准确率 (固定采样 5 次)
print("\n=== 实验 2: Temperature vs 准确率 (采样 5 次) ===\n")
for temp in [0.3, 0.5, 0.7, 1.0]:
    correct = 0
    for p in test_problems:
        result = tester.solve_consistent(p["question"], n_samples=5, temperature=temp)
        if p["answer"] in result["answer"]:
            correct += 1
    print(f"Temperature={temp}: 准确率 {correct}/{len(test_problems)} = {correct/len(test_problems):.0%}")
```

**预期结论**：
- 采样次数从 1 增加到 5 时准确率提升明显，5 到 7 时边际收益递减
- Temperature 过低（0.3）时采样多样性不足，投票效果有限；过高（1.0）时噪声太大
- Temperature=0.5-0.7 通常是 Self-Consistency 的最佳区间
- 生产环境中需在准确率提升和 API 成本增加之间找平衡点

**要点**：
- Self-Consistency 的本质是"用多次采样的多样性换取准确率"
- Temperature 太低则采样结果趋同（投票无意义），太高则噪声过大（多数票也不可靠）
- 采样次数应为奇数（3、5、7），避免投票出现平票
- 成本线性增长，5 次采样意味着 5 倍 API 调用，只在高价值场景使用
