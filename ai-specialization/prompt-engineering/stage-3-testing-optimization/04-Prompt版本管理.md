...
**思路**：语义化版本号的核心规则是 PATCH 用于小修正（不影响行为），MINOR 用于新增功能（向后兼容），MAJOR 用于重大变更（可能不兼容）。判断版本号的关键是看变更是否影响 Prompt 的整体行为。

**答案**：

1. **修正了一个错别字** → `1.0.0` → `1.0.1`（PATCH）
   - 理由：错别字修正不影响 Prompt 的核心行为，属于小幅修正。例如将"请判断以下文本的情感倾向向"修正为"请判断以下文本的情感倾向"。

2. **添加了输出格式的 JSON 约束** → `1.0.0` → `1.1.0`（MINOR）
   - 理由：添加 JSON 格式约束是新增功能，Prompt 的核心任务（情感分析）未变，但输出格式发生了变化。下游系统需要适配新格式，因此不是 PATCH 而是 MINOR。

3. **完全重写了 Prompt 的任务定义** → `1.0.0` → `2.0.0`（MAJOR）
   - 理由：任务定义是 Prompt 的核心逻辑，重写意味着 Prompt 的行为可能发生根本性变化。例如从"情感分类"改为"文本摘要"，这是不兼容的重大变更。

4. **从支持中文改为支持多语言** → `1.0.0` → `2.0.0`（MAJOR）
   - 理由：虽然表面上是"扩展"能力，但支持多语言意味着 Prompt 的系统提示、输出格式、处理逻辑都可能发生根本变化。例如输出语言从固定中文变为动态匹配输入语言，这是不兼容的变更。

**要点**：
- 版本号变更的关键判断标准是"是否影响已有使用者的行为"——影响越大，版本号升级越多
- MINOR 和 PATCH 的区别在于：PATCH 不改变输出行为，MINOR 改变输出但不破坏向后兼容
- 如果不确定该升 MINOR 还是 MAJOR，通常选 MAJOR 更安全——宁可版本号升得快，也不要让使用者踩坑

---

### 练习2：Diff 分析

**思路**：使用课程中的 PromptDiffer 类比较两个版本，识别文本级差异，然后结合变更内容分析语义影响。关键是要区分"表面修改"和"行为变更"。
...
class PromptDiffer:
    def diff(self, old_text: str, new_text: str) -> list[DiffLine]:
        ...
        for op, i1, i2, j1, j2 in matcher.get_opcodes():
            if op == 'equal':
                for k in range(i2 - i1):
                    diff_lines.append(DiffLine(old_num + k, new_num + k,
                                               old_lines[i1 + k].rstrip(), 'equal'))
                old_num += i2 - i1
                new_num += j2 - j1
            elif op == 'insert':
                for k in range(j2 - j1):
                    diff_lines.append(DiffLine(None, new_num + k,
                                               new_lines[j1 + k].rstrip(), 'insert'))
                new_num += j2 - j1
            elif op == 'delete':
                for k in range(i2 - i1):
                    diff_lines.append(DiffLine(old_num + k, None,
                                               old_lines[i1 + k].rstrip(), 'delete'))
                old_num += i2 - i1
            elif op == 'replace':
                max_len = max(i2 - i1, j2 - j1)
                for k in range(max_len):
                    old_c = old_lines[i1 + k].rstrip() if k < (i2 - i1) else ""
                    new_c = new_lines[j1 + k].rstrip() if k < (j2 - j1) else ""
                    diff_lines.append(DiffLine(
                        old_num + k if k < (i2 - i1) else None,
                        new_num + k if k < (j2 - j1) else None,
                        f"{old_c} → {new_c}" if old_c and new_c else new_c or old_c,
                        'replace'
                    ))
                old_num += i2 - i1
                new_num += j2 - j1
    ...
    def format_markdown(self, old_version: str, new_version: str,
                        diff_lines: list[DiffLine]) -> str:
        output = [f"## Prompt Diff: {old_version} → {new_version}", "", "```diff"]
        stats = {"equal": 0, "insert": 0, "delete": 0, "replace": 0}
        ...
        for line in diff_lines:
            ...
            if line.type == 'equal':
                ...
                if len(parts) > 1:
                    output.append(f"+ {parts[1]}")

        output.append("```")
        output.append("")
        output.append("| 变更类型 | 数量 |")
        output.append("|---------|------|")
...
# 使用示例
old_prompt = """你是一个情感分析专家。
请判断以下文本的情感倾向。
输出格式：正面/负面/中性"""

new_prompt = """你是一个专业的情感分析专家。
请根据以下文本判断其情感倾向。
...
differ = PromptDiffer()
diff_lines = differ.diff(old_prompt, new_prompt)
report = differ.format_markdown("v1.0.0", "v1.1.0", diff_lines)
print(report)
```

**语义影响分析：**
- 新增了"规则"部分（JSON 输出约束、混合情感处理规则）→ 行为变更：下游系统需适配 JSON 格式
- "判断"改为"根据...判断" → 表面修改：措辞更精确，不影响行为
- 输出从纯文本改为 JSON 对象 → 重大变更：所有消费 Prompt 输出的代码都需要修改

**要点**：
- 文本级 diff 只能看到"改了什么"，语义级分析才能判断"影响多大"——两者缺一不可
- 输出格式变更是最容易被忽视的破坏性变更——改一个字段名就可能让下游系统崩溃
- Diff 报告应同时展示给开发者和产品经理，让技术和业务双方都了解变更影响

---

### 练习3：版本管理系统

**思路**：在课程 PromptVersionManager 基础上，增加回滚操作、Git 提交集成和 Changelog 自动生成。核心设计是每次变更都创建新版本记录（包括回滚），保持版本历史的完整性。
...
class PromptVersionManager:
    ...
    def _save_metadata(self, prompt_name: str, version: PromptVersion):
        """自动生成并保存 metadata.yaml"""
        prompt_dir = self.base_dir / prompt_name
        prompt_dir.mkdir(parents=True, exist_ok=True)
        import yaml
        metadata = {
        ...
        with open(prompt_dir / "metadata.yaml", "w", encoding="utf-8") as f:
    ...
    def create(self, prompt_name: str, content: str,
               author: str, metadata: Optional[dict] = None) -> PromptVersion:
        version = PromptVersion(
            version="1.0.0", content=content,
            metadata=metadata or {}, created_at=datetime.now().isoformat(),
            author=author, status="draft", change_description="初始版本"
        )
        self.versions[prompt_name] = [version]
        self._save_versions(prompt_name)
        self._save_current(prompt_name, content)
        self._save_metadata(prompt_name, version)
        return version

    def update(self, prompt_name: str, content: str, author: str,
        ...
        versions = self.versions[prompt_name]
        latest = versions[-1]
        new_ver = self._bump_version(latest.version, version_type)

        new_metadata = {**latest.metadata, **(metadata or {})}
        version = PromptVersion(
            version=new_ver, content=content, metadata=new_metadata,
            created_at=datetime.now().isoformat(), author=author,
            status="draft", change_description=change_description
        )
        versions.append(version)
        self._save_versions(prompt_name)
        self._save_current(prompt_name, content)
    ...
    def rollback(self, prompt_name: str, target_version: str,
                 author: str, reason: str = "") -> PromptVersion:
        """回滚到指定版本（创建新版本记录）"""
        target = self.get_version(prompt_name, target_version)
        if not target:
            raise ValueError(f"版本 {target_version} 不存在")

        current_latest = self.versions[prompt_name][-1]
        new_ver = self._bump_version(current_latest.version, "patch")

        rollback_desc = f"回滚到 {target_version}"
        if reason:
            rollback_desc += f"，原因：{reason}"

        version = PromptVersion(
            version=new_ver, content=target.content,
            metadata={**target.metadata, "rollback_from": current_latest.version,
                      "rollback_to": target_version},
            created_at=datetime.now().isoformat(), author=author,
            status="active", change_description=rollback_desc
        )
        self.versions[prompt_name].append(version)
        self._save_versions(prompt_name)
        self._save_current(prompt_name, target.content)
        self._save_metadata(prompt_name, version)
        return version
    ...
    def list_versions(self, prompt_name: str) -> list[dict]:
        return [
            {"version": v.version, "author": v.author,
             ...
             "change_description": v.change_description, "hash": v.hash}
            for v in self.versions.get(prompt_name, [])
        ]
    ...
    def changelog(self, prompt_name: str) -> str:
        ...
        for v in reversed(versions):
            ...
            if v.change_description:
                lines.append(f"- 变更: {v.change_description}")
            lines.append("")
        return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    v1 = manager.create("classifier", "你是一个情感分析专家。判断情感倾向。",
                        author="zhangwei@example.com",
                        metadata={"model": "gpt-4o-mini"})
    print(f"创建: v{v1.version}")

    v2 = manager.update("classifier",
                        "你是一个专业的情感分析专家。判断情感倾向。输出JSON格式。",
                        author="zhangwei@example.com",
                        version_type="minor",
                        change_description="添加JSON输出约束")
    print(f"更新: v{v2.version}")

    v3 = manager.rollback("classifier", "1.0.0",
                          author="lina@example.com",
                          reason="v1.1.0的JSON格式导致下游系统报错")
    print(f"回滚: v{v3.version}")

    print("\n版本历史:")
    for v in manager.list_versions("classifier"):
        print(f"  {v['version']} ({v['status']}) - {v['change_description']}")

    print("\nChangelog:")
    print(manager.changelog("classifier"))
```

**要点**：
- 回滚时必须创建新版本记录而不是直接覆盖——这样版本历史才能完整追溯"谁在什么时候为什么回滚"
- metadata.yaml 应在每次版本变更时自动生成，包含版本号、作者、时间、变更描述等关键信息
- Git 提交信息应与版本变更关联——建议在 commit message 中引用 Prompt 版本号，便于追溯
