...
**思路**：从历史日志中提取高频查询（按查询文本去重计数），按频率排序取 Top-N，批量调用 LLM 获取响应后预填充到缓存中。支持增量预热和全量预热两种模式。
...
class CacheWarmer:
    ...
    def __init__(self, cache, llm_call_fn: Callable,
                 min_frequency: int = 3, top_n: int = 100):
        """
        Args:
            cache: 缓存对象，需支持 put(messages, model, content, ...) 方法
            llm_call_fn: LLM 调用函数，签名为 (messages, model) -> str
            min_frequency: 最低频率阈值，低于此频率的查询不预热
            top_n: 预热查询数量上限
        """
        self.cache = cache
    ...
    def extract_high_frequency_queries(
        self, logs: List[QueryRecord]
    ) -> List[Dict]:
        """从日志中提取高频查询"""
        ...
        high_freq = [
            ...
            for key, count in query_counter.most_common(self.top_n)
            if count >= self.min_frequency
        ]

        return high_freq

    def warm(self, logs: List[QueryRecord],
             dry_run: bool = False) -> Dict:
        """执行缓存预热"""
        high_freq_queries = self.extract_high_frequency_queries(logs)

        if not high_freq_queries:
            return {"status": "no_queries", "warmed": 0}
        ...
        for item in high_freq_queries:
            ...
            if dry_run:
                print(f"[DRY RUN] 预热: {item['query_text'][:50]}... (频率: {item['frequency']})")
                warmed += 1
                continue

            cached = self.cache.get(messages, item["model"])
            if cached:
                skipped += 1
                continue

            try:
                response = self.llm_call_fn(messages, item["model"])
                tokens = len(response) // 3
                self.cache.put(
                    messages, item["model"], response,
                    tokens=tokens, cost=0.001,
                )
                warmed += 1
                total_tokens += tokens
            except Exception as e:
                errors += 1
                print(f"预热失败: {item['query_text'][:30]}... -> {e}")

        return {
            "status": "completed",
            "total_candidates": len(high_freq_queries),
            "warmed": warmed,
            "skipped": skipped,
            "errors": errors,
    ...
    def generate_report(self, logs: List[QueryRecord]) -> str:
        """生成预热分析报告"""
        queries = self.extract_high_frequency_queries(logs)
        lines = ["=" * 50, "缓存预热分析报告", "=" * 50]
        lines.append(f"总查询数: {len(logs)}")
        lines.append(f"高频查询数 (>= {self.min_frequency}次): {len(queries)}")
        lines.append(f"预热上限: {self.top_n}")
        lines.append("\nTop 10 高频查询:")
        for i, q in enumerate(queries[:10]):
            lines.append(f"  {i+1}. [{q['frequency']}次] {q['query_text'][:50]}...")
        return "\n".join(lines)


if __name__ == "__main__":
    mock_logs = [
        QueryRecord(query_text="怎么退款", model="gpt-4o-mini", timestamp=time.time() - i)
        for i in range(20)
    ] + [
        QueryRecord(query_text="物流单号查一下", model="gpt-4o-mini", timestamp=time.time() - i)
        for i in range(15)
    ] + [
        QueryRecord(query_text="密码忘了怎么办", model="gpt-4o-mini", timestamp=time.time() - i)
        for i in range(10)
    ] + [
        QueryRecord(query_text="你好", model="gpt-4o-mini", timestamp=time.time() - i)
        for i in range(2)
    ]
    ...
    class MockCache:
        ...
        def get(self, messages, model):
            return self._data.get(str(messages))
        def put(self, messages, model, content, **kwargs):
            self._data[str(messages)] = content

    def mock_llm(messages, model):
        return f"这是对 '{messages[-1]['content']}' 的回答"
    ...
    print(warmer.generate_report(mock_logs))
    print()

    result = warmer.warm(mock_logs, dry_run=True)
    print(f"\n预热结果: {json.dumps(result, indent=2, ensure_ascii=False)}")
```

**要点**：
- 预热的核心是"先分析频率，再批量填充"，不要盲目预热所有历史查询
- `min_frequency` 阈值很关键：太低会浪费 API 调用，太高会遗漏常见查询
- 预热过程需要跳过已有缓存条目，避免重复调用 API

---

### 练习 2：缓存分析仪表盘

**思路**：构建一个 `CacheDashboard`，收集各缓存层的统计数据，计算命中率、节省成本、热门查询等指标，并提供格式化的展示接口。
...
class CacheDashboard:
    ...
    def __init__(self, l1_cache=None, l2_cache=None):
        ...
        self._hourly_stats: Dict[str, Dict] = defaultdict(lambda: {
            "hits": 0, "misses": 0, "cost_saved": 0.0, "tokens_saved": 0,
        })

    def record_query(self, query: str, cache_hit: bool, layer: str,
                     latency_ms: float, tokens_saved: int = 0,
    ...
    def get_hit_rate_report(self) -> Dict:
        """获取命中率报告"""
        total = len(self._query_log)
        if total == 0:
            return {"total_queries": 0, "hit_rate": "N/A"}

        hits = sum(1 for q in self._query_log if q["hit"])
        by_layer = defaultdict(lambda: {"hits": 0, "total": 0})

        for q in self._query_log:
            by_layer[q["layer"]]["total"] += 1
            if q["hit"]:
                by_layer[q["layer"]]["hits"] += 1

        return {
            "total_queries": total,
            "overall_hit_rate": f"{hits / total:.1%}",
            "by_layer": {
                layer: {
                    "hit_rate": f"{data['hits'] / data['total']:.1%}",
                    "hits": data["hits"],
                    "total": data["total"],
                }
    ...
    def get_savings_report(self) -> Dict:
        ...
        total_cost_saved = sum(
            s["cost_saved"] for s in self._hourly_stats.values()
        )
        total_tokens_saved = sum(
            s["tokens_saved"] for s in self._hourly_stats.values()
        )
        ...
        return {
            ...
            "hourly_breakdown": {
                hour: {
                    "cost_saved": f"${s['cost_saved']:.4f}",
                    "tokens_saved": s["tokens_saved"],
                }
                for hour, s in sorted(self._hourly_stats.items())
            },
        }

    def get_popular_queries(self, top_n: int = 10) -> List[Dict]:
        """获取热门查询"""
        query_counter = defaultdict(lambda: {"count": 0, "hits": 0})
        for q in self._query_log:
            key = q["query"]
            query_counter[key]["count"] += 1
            if q["hit"]:
                query_counter[key]["hits"] += 1
        ...
        return [
            {
                "query": query,
                "count": data["count"],
                "hit_rate": f"{data['hits'] / data['count']:.1%}",
            }
    ...
    def get_latency_report(self) -> Dict:
        """获取延迟报告"""
        hit_latencies = [q["latency_ms"] for q in self._query_log if q["hit"]]
        miss_latencies = [q["latency_ms"] for q in self._query_log if not q["hit"]]
        ...
        return {
            "cache_hit": {
                "avg_ms": f"{avg(hit_latencies):.1f}",
                "count": len(hit_latencies),
            },
            "cache_miss": {
                "avg_ms": f"{avg(miss_latencies):.1f}",
                "count": len(miss_latencies),
            },
            "latency_reduction": (
                f"{(1 - avg(hit_latencies) / avg(miss_latencies)):.1%}"
                if avg(miss_latencies) > 0 else "N/A"
            ),
        }
    ...
    def print_dashboard(self):
        ...
        hit_report = self.get_hit_rate_report()
        print(f"\n📊 命中率: {hit_report.get('overall_hit_rate', 'N/A')}")
        print(f"   总查询: {hit_report.get('total_queries', 0)}")

        savings = self.get_savings_report()
        print(f"\n💰 成本节省: {savings['total_cost_saved']}")
        print(f"   Token 节省: {savings['total_tokens_saved']}")

        latency = self.get_latency_report()
        print(f"\n⚡ 缓存命中延迟: {latency['cache_hit']['avg_ms']}ms")
        print(f"   缓存未命中延迟: {latency['cache_miss']['avg_ms']}ms")
        print(f"   延迟降低: {latency['latency_reduction']}")

        popular = self.get_popular_queries(5)
        if popular:
            print(f"\n🔥 热门查询 Top 5:")
            for i, q in enumerate(popular):
                print(f"   {i+1}. [{q['count']}次, 命中率{q['hit_rate']}] {q['query'][:40]}")
...
if __name__ == "__main__":
    ...
    queries = ["怎么退款", "物流查询", "密码重置", "怎么退款", "物流查询",
               "怎么退款", "会员权益", "物流查询", "怎么退款", "密码重置"]
    ...
    for q in queries:
        ...
        dashboard.record_query(
            query=q,
            cache_hit=hit,
            layer="L1-exact" if hit else "miss",
            latency_ms=random.uniform(0.5, 2) if hit else random.uniform(500, 2000),
            tokens_saved=150 if hit else 0,
            cost_saved=0.002 if hit else 0,
        )
...
**要点**：
- 仪表盘需要多维度数据：命中率、成本节省、延迟对比、热门查询
- 按小时统计便于发现缓存命中率的时间规律（如高峰期命中率下降）
- 热门查询分析可以指导缓存预热策略
...
**思路**：为每个缓存条目记录历史更新频率（命中次数、上次更新时间），频率高的查询说明内容变化快，设置短 TTL；频率低的查询说明内容稳定，设置长 TTL。使用滑动窗口计算更新频率。

**答案**：

```python
"""
adaptive_ttl_cache.py
自适应 TTL 缓存 - 根据查询更新频率动态调整 TTL
"""
...
class AdaptiveEntry:
    ...
    def access_frequency(self) -> float:
        ...
        if time_span <= 0:
            return 0.0
        return len(self.access_times) / (time_span / 3600)
...
class AdaptiveTTLCache:
    ...
    def __init__(
        self,
        max_size: int = 1000,
        min_ttl: float = 300,       # 5 分钟
        max_ttl: float = 86400,     # 24 小时
        default_ttl: float = 3600,  # 1 小时
        frequency_window: int = 50,  # 最近 N 次访问用于计算频率
    ):
        self.max_size = max_size
        ...
        self._cache: OrderedDict[str, AdaptiveEntry] = OrderedDict()
    ...
    def _calculate_adaptive_ttl(self, entry: AdaptiveEntry) -> float:
        """根据访问频率计算自适应 TTL"""
    ...
    def get(self, messages: List[Dict], model: str) -> Optional[Dict]:
        ...
        if len(entry.access_times) > self.frequency_window:
            entry.access_times = entry.access_times[-self.frequency_window:]
        ...
        return {
            "content": entry.content,
            "from_cache": True,
            "ttl_remaining": entry.ttl - (time.time() - entry.created_at),
            "hit_count": entry.hit_count,
            "access_frequency": f"{entry.access_frequency:.1f}/h",
        }
    ...
    def put(self, messages: List[Dict], model: str, content: str,
        ...
        if key in self._cache:
            ...
            old_entry.refresh_count += 1
            new_ttl = self._calculate_adaptive_ttl(old_entry)
            old_entry.content = content
            old_entry.created_at = time.time()
            old_entry.ttl = new_ttl
            self._cache.move_to_end(key)
            return
    ...
    def get_stats(self) -> Dict:
        ...
        ttl_buckets = {"short (<10min)": 0, "medium (10min-2h)": 0, "long (>2h)": 0}

        for e in entries:
            if e.ttl < 600:
                ttl_buckets["short (<10min)"] += 1
            elif e.ttl < 7200:
                ttl_buckets["medium (10min-2h)"] += 1
            else:
                ttl_buckets["long (>2h)"] += 1

        return {
            "size": len(self._cache),
            "ttl_distribution": ttl_buckets,
            "avg_hit_count": (
                sum(e.hit_count for e in entries) / len(entries)
                if entries else 0
            ),
            "avg_ttl_seconds": (
                sum(e.ttl for e in entries) / len(entries)
                if entries else 0
            ),
        }
...
if __name__ == "__main__":
    cache = AdaptiveTTLCache(min_ttl=60, max_ttl=3600, default_ttl=300)

    messages = [{"role": "user", "content": "今天天气怎么样？"}]
    cache.put(messages, "gpt-4o-mini", "今天晴天，气温 25°C")

    for i in range(15):
        result = cache.get(messages, "gpt-4o-mini")

    cache.put(messages, "gpt-4o-mini", "今天多云，气温 23°C")
    ...
    if result:
        ...
        print(f"  剩余 TTL: {result['ttl_remaining']:.0f}s")
        print(f"  访问频率: {result['access_frequency']}")
        print(f"  命中次数: {result['hit_count']}")
```

**要点**：
- 自适应 TTL 的核心是"高频查询短 TTL，低频查询长 TTL"
- 使用滑动窗口计算访问频率，避免历史数据对频率判断的干扰
- 缓存更新时重新计算 TTL，确保 TTL 跟随访问模式变化
