...
你刚用 Vue 写好了前端页面，准备对接后端接口。第一个问题来了：用什么框架写后端？Flask 太轻量缺少类型校验，Django 太重异步支持弱。你需要一个能自动生成 API 文档、原生支持异步、自带请求参数校验的框架。更现实的问题是：路由怎么设计才规范？错误响应格式怎么统一？数据库连接池怎么配置？这些问题不解决，前后端联调时会反复扯皮。
...
**思路**：围绕用户认证和对话两个业务域设计 API，用 Pydantic Schema 定义请求和响应模型，用 `tags` 分组路由，用标准 HTTP 状态码。
...
class RegisterRequest(BaseModel):
    username: str = Field(..., min_length=2, max_length=50, description="用户名", examples=["zhangsan"])
...
class ChatRequest(BaseModel):
    message: str = Field(..., min_length=1, max_length=10000, description="用户消息", examples=["帮我分析销售数据"])
    session_id: str | None = Field(None, description="会话 ID，为空则创建新会话")
...
async def register(request: RegisterRequest):
    """用户注册"""
    # 实际实现中需要哈希密码、写入数据库
    return UserResponse(id="new-user-id", username=request.username, email=request.email, is_active=True)

@router.post("/login", response_model=TokenResponse)
async def login(request: LoginRequest):
    """用户登录，返回 JWT token"""
    return TokenResponse(access_token="eyJhbGciOiJIUzI1NiIs...")

@router.get("/me", response_model=UserResponse)
async def get_current_user():
    """获取当前登录用户信息"""
    return UserResponse(id="user-1", username="zhangsan", email="zhangsan@example.com", is_active=True)
```
...
async def list_sessions(page: int = 1, size: int = 20):
    """获取会话列表"""
    return SessionListResponse(items=[], total=0, page=page, size=size)
...
async def create_session(request: ChatRequest):
    """创建新会话"""
    return ChatResponse(session_id="new-session", message=MessageResponse(id="msg-1", role="assistant", content="你好", created_at=datetime.now(), token_usage=10))
...
**要点**：
- 请求模型和响应模型要分开定义，响应中不能暴露 `password` 等敏感字段
- `Field` 的 `description` 和 `examples` 会自动显示在 Swagger 文档中
- 常见错误：把所有路由放在一个文件里，路由多了以后难以维护，应该按业务域拆分到不同文件

### 练习 2

**思路**：实现三个中间件——请求日志记录耗时、全局异常捕获统一错误格式、CORS 允许前端跨域访问。
...
**要点**：
- 中间件注册顺序很重要：日志中间件放最前面才能记录所有请求的耗时
- CORS 的 `allow_origins` 不能设为 `["*"]`（当 `allow_credentials=True` 时），必须指定具体域名
- 常见错误：全局异常处理返回了 Python 默认的 traceback 给前端，生产环境应只返回通用错误信息，避免泄露代码细节

### 练习 3

**思路**：用 `Query` 参数定义分页依赖函数，在列表路由中通过 `Depends` 注入使用。
...
async def list_sessions(
    ...
    # 实际实现：SELECT * FROM chat_sessions LIMIT size OFFSET offset
    return SessionListResponse(items=[], total=0, page=page, size=size)
```

**要点**：
- `Depends(get_pagination)` 不加括号，传的是函数引用而非调用结果
- `Query` 的 `ge`、`le` 参数自动校验范围，超出范围返回 422 错误
- 常见错误：在依赖函数里写业务逻辑，依赖函数应该只负责参数解析和校验，业务逻辑放在 Service 层
...
- API 版号放 URL 路径（`/api/v1/...`），不要放 Header，方便浏览器直接测试和未来平滑升级
- 接口文档要写清楚每个字段的 `description` 和 `example`，这是前后端协作的契约，比口头沟通可靠
- 生产环境关闭 `debug=True` 和 Swagger 文档暴露，避免信息泄露
- 数据库连接池大小要根据实际并发量调整，初始值建议 `pool_size=20, max_overflow=10`
