...
**思路**：三种切分策略的核心区别在于是否尊重语义边界以及是否有重叠。固定大小无重叠最简单但容易切断语义，加重叠能缓解边界问题，按段落切分则天然尊重语义边界。
...
// 对比
console.log('=== 策略 1：固定 50 字符，无重叠 ===')
const r1 = fixedChunk(testDoc, 50)
r1.forEach((c, i) => console.log(`Chunk ${i}: "${c}"`))
console.log('\n=== 策略 2：固定 50 字符，10 字符重叠 ===')
const r2 = fixedChunkWithOverlap(testDoc, 50, 10)
r2.forEach((c, i) => console.log(`Chunk ${i}: "${c}"`))
console.log('\n=== 策略 3：按段落切分 ===')
const r3 = paragraphChunk(testDoc)
r3.forEach((c, i) => console.log(`Chunk ${i}: "${c}"`))
```

预期对比结果：
- 策略 1：可能把"退款需在购买后30天内"和"申请，且商品需保持原包装完好"切到两个 chunk，语义断裂
- 策略 2：重叠部分保证"30天内申请"这样的关键信息不会丢失
- 策略 3：每个段落是完整语义单元，但段落长度不均匀，可能有特别长的段落

**要点**：
- 固定大小无重叠最简单但质量最差，容易在句子中间切断
- 重叠能缓解边界问题，但过大的重叠会导致重复存储
- 常见错误：只用一种策略处理所有文档，不根据文档特点调整

### 练习二

**思路**：Markdown 切分器的核心是识别标题层级，维护标题栈来追踪 parentHeadings。遇到新标题时保存之前的内容，同时更新标题栈（移除同级及更低级别的标题）。
...
### PDF 解析
PDF 是最复杂的格式，需要处理坐标重建文本顺序。

### Markdown 解析
Markdown 结构清晰，按标题切分即可。

## 向量检索

### Embedding 模型
推荐使用 text-embedding-3-small。

### 相似度计算
使用余弦相似度衡量语义接近程度。
`

const result = chunkMarkdown(testMd)
result.forEach((chunk, i) => {
  console.log(`\n--- Chunk ${i} ---`)
  console.log(`heading: "${chunk.heading}"`)
  console.log(`headingLevel: ${chunk.headingLevel}`)
  console.log(`parentHeadings: [${chunk.parentHeadings.join(', ')}]`)
  console.log(`content: "${chunk.content.slice(0, 50)}..."`)
})
```

预期输出：每个 chunk 的 heading 和 parentHeadings 正确反映层级关系，如"PDF 解析"的 parentHeadings 应为 ["RAG 系统设计", "文档解析"]。

**要点**：
- 标题栈的维护是关键：遇到新标题时要移除同级及更低级别的标题
- parentHeadings 让每个 chunk 知道自己在文档结构中的位置
- 常见错误：没有正确维护标题栈，导致 parentHeadings 包含已过时的标题

### 练习三

**思路**：元数据设计要同时服务于三个用途：引用溯源（告诉用户答案来自哪里）、过滤检索（缩小搜索范围）、排序优化（优先返回更精确的结果）。
...
interface ChunkMetadata {
  // === 来源信息（用于引用溯源） ===
  source: {
    filename: string        // 文件名，如 "退款政策.pdf"
    type: 'markdown' | 'pdf' | 'txt'
    url?: string            // 可选：原始文档的 URL，支持点击跳转
    page?: number           // PDF 页码
    lastUpdated: string     // 文档最后更新时间，避免引用过时内容
  }
  ...
  content: {
    type: 'paragraph' | 'list' | 'code' | 'table' | 'heading'
    charCount: number
    language?: string       // 代码语言标签
    tags?: string[]         // 自定义标签，如 ["退款", "客服"]
  }
}
```

**如何用于引用溯源：**
- 用户看到回答中的 [1] 引用，点击后展示：`退款政策.pdf > 退款流程 > 第 2.1 节`
- 前 200 字符作为 snippet 预览，用户无需打开原文即可判断引用是否准确
- `lastUpdated` 字段帮助用户判断信息是否过时

**如何用于过滤检索：**
- 按文件过滤：`{ source: '退款政策.pdf' }` — 只在特定文档中搜索
- 按类型过滤：`{ type: 'code' }` — 只搜索代码片段
- 按标签过滤：`{ tags: '退款' }` — 只搜索退款相关内容
- 组合过滤：`{ source: '退款政策.pdf', type: 'paragraph' }`

**要点**：
- 元数据要在切分阶段就生成，后面很难补回来
- `parentHeadings` 是引用溯源的核心，没有它就无法展示完整的位置路径
- 常见错误：只存 chunk 内容不存元数据，等到需要引用时才发现没有来源信息
