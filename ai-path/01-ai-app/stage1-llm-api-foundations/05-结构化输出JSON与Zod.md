...
你在做一个"工单自动分类"功能，让 AI 分析用户提交的工单并返回分类结果。第一次你让模型直接返回文本，它说"这是一个关于登录问题的 bug 报告，优先级较高"。你用正则去提取"bug"和"较高"，跑了 100 条测试，30 条解析失败——模型有时候说"这是一个 Bug"，有时候说"属于缺陷类"，有时候说"建议标记为 bug"。你需要模型返回程序可以直接使用的结构化数据，而不是需要猜谜的自然语言。
...
**思路**：定义 Zod Schema 时，要根据业务需求选择合适的类型和约束。枚举类型（z.enum）适合有限的选项集，可选字段（.optional()）用于非必填项，数字约束（.min/.max）用于限定范围。
...
const ArticleMetadataSchema = z.object({
  ...
  publishDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '日期格式：YYYY-MM-DD'),
...
**要点**：
- Schema 设计要平衡严格性和灵活性：业务必需的字段（如 title、rating）加约束，非必需的字段（如 description、dueDate）用 optional()
- 常见错误：给所有字段都加 `.min(1)`，导致模型返回空字符串绕过校验，或者校验过于频繁失败

### 练习二

**思路**：信息提取是结构化输出的经典应用场景。关键是定义一个合理的 Schema，然后用 `responses.parse` + `zodTextFormat` 让模型按 Schema 输出。测试时用多样化的文本验证 Schema 的通用性。
...
async function extractInfo(text: string): Promise<ExtractedInfo | null> {
  try {
    const response = await openai.responses.parse({
      model: 'gpt-5.5',
      input: [
        {
          role: 'system',
          content: '从用户提供的文本中提取实体信息。包括人名（及角色）、组织名、日期、地点和关键事件。'
        },
        { role: 'user', content: text }
      ],
      text: { format: zodTextFormat(ExtractedInfoSchema, 'extracted_info') },
    })
...
async function main() {
  const texts = [
    '张三（项目经理）和李四（前端开发）将于下周三在北京开会，讨论新版本发布计划。',
    '2024 年 1 月 15 日，Apple 公司在加州库比蒂诺发布了 Vision Pro，售价 3499 美元。',
    '王医生建议患者在 3 月 20 日去北京协和医院做进一步检查。',
  ]

  for (const text of texts) {
    console.log(`\n输入: ${text}`)
    const result = await extractInfo(text)
    if (result) {
...
**要点**：
- Schema 中数组字段的类型要定义清楚——`people` 是对象数组（含 name 和 role），而 `organizations` 是字符串数组
- 常见错误：用正则从自由文本里提取信息——看似省事，实际上极其脆弱，模型的措辞每次都不同，正则根本覆盖不了所有情况

### 练习三

**思路**：三种输出方式的对比可以直观展示结构化输出的价值。自由文本无法程序化处理，JSON Mode 保证格式但不保证结构，Structured Outputs 保证格式和结构。
...
async function compareOutputs(comment: string) {
  console.log(`\n输入: ${comment}`)
  console.log('='.repeat(60))

  // 方式 1：自由文本
  const freeResponse = await openai.responses.create({
    model: 'gpt-5.5',
    input: [
  ...
  console.log('\n方式 1 - 自由文本:')
  console.log(freeResponse.output_text)
  console.log('可解析性: ❌ 需要用正则提取，脆弱不可靠')
  ...
  const jsonResponse = await openai.responses.create({
    ...
    input: [
      {
        role: 'system',
        content: '分析用户评论的情感，返回 JSON 格式，包含 sentiment（positive/negative/neutral）、confidence（0-1）、keywords（数组）字段。'
      },
      { role: 'user', content: comment }
    ],
    text: { format: { type: 'json_object' } },
  ...
  const jsonResult = JSON.parse(jsonResponse.output_text)
  console.log('\n方式 2 - JSON Mode:')
  console.log(JSON.stringify(jsonResult, null, 2))
  console.log('可解析性: ⚠️ 是合法 JSON，但字段名和类型可能不符合预期')

  // 方式 3：Structured Outputs (Zod)
  const structuredResponse = await openai.responses.parse({
    model: 'gpt-5.5',
    input: [
      { role: 'user', content: `分析情感：${comment}` }
    ],
    text: { format: zodTextFormat(SentimentSchema, 'sentiment') },
    temperature: 0.7,
  })
  console.log('\n方式 3 - Structured Outputs:')
  console.log(JSON.stringify(structuredResponse.output_parsed, null, 2))
  console.log('可解析性: ✅ 格式和类型完全符合 Schema 定义')
}

async function main() {
  await compareOutputs('这个产品用了一周，真的很失望，功能不全还经常崩溃。')
  await compareOutputs('还行吧，中规中矩，没什么惊喜也没什么大问题。')
}
...
**要点**：
- 三种方式的可靠性递增：自由文本 < JSON Mode < Structured Outputs
- JSON Mode 只保证是合法 JSON，不保证结构——模型可能返回 `{ "result": "positive" }` 而你期望的是 `{ "sentiment": "positive", "confidence": 0.95 }`
- 常见错误：开了 JSON Mode 就认为万事大吉，不做 Schema 校验，结果生产环境中模型偶尔返回不符合预期的结构导致程序崩溃

---

## 常见误区

1. **"开了 JSON Mode 就不用校验了"**：JSON Mode 只保证输出是合法的 JSON，不保证结构符合你的预期。模型可能返回 `{ "result": "positive" }` 而你期望的是 `{ "sentiment": "positive", "confidence": 0.95 }`。JSON Mode + Schema 校验才是完整的方案。

2. **"Schema 定义得越严格越好"**：给所有字段都加上 `z.string().min(1).max(100).regex(/xxx/)`，结果模型经常无法满足所有约束，校验频繁失败。Schema 应该只约束业务必需的字段和格式，给模型适当的输出自由度。

3. **"用正则从自由文本里提取 JSON 就行"**：在模型输出里找 `JSON.parse(...)` 的内容，看似省事，实际上极其脆弱。模型可能在 JSON 前后加说明文字，可能用不同的格式，可能输出不合法的 JSON。直接用 Structured Outputs 是正道。

4. **"校验失败就报错给用户"**：Zod 校验失败时直接 `throw Error`，用户看到"校验失败"完全不知道怎么回事。应该重试一次（在 Prompt 里加入修正提示），或者降级返回默认值，给用户一个可理解的提示。

---

## 工程建议

1. **用 Zod Schema 同时定义类型和校验逻辑**：`z.infer<typeof Schema>` 让你只维护一份 Schema，同时得到 TypeScript 类型和运行时校验。不要手动写 `interface` 再单独写校验函数——两份定义迟早会不同步。

2. **Prompt 里描述期望的 JSON 结构**：即使用了 Structured Outputs，也在 Prompt 里说清楚每个字段的含义和期望值。模型生成的内容质量取决于它是否理解字段的语义，而 Schema 只能约束格式。

3. **为校验失败设计降级策略**：不是所有校验失败都需要报错。对于非关键字段（如 `summary`），可以用默认值降级；对于关键字段（如 `sentiment`），重试一次再报错。区分"必须正确"和"大致正确就行"的字段。

4. **在开发阶段打印校验错误详情**：`ZodError.issues` 包含每个字段的具体错误信息，开发时打印出来能快速定位问题。上线后把这些错误记录到日志，用于优化 Schema 和 Prompt。
