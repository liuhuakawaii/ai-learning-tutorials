...
**思路**：按照课程中的步骤逐一完成。关键是从第一天就养成安全习惯——创建 Key 后立即配置环境变量，而不是先把 Key 写在代码里"方便调试"。

**答案**：

1. 登录 [OpenAI Platform](https://platform.openai.com)，进入 API Keys 页面
2. 点击 "Create new secret key"，给 Key 加上标签（如 `dev-learning`），复制保存
3. 在项目根目录创建 `.env` 文件：
   ```bash
   OPENAI_API_KEY=sk-proj-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   ```
4. 确认 `.gitignore` 包含以下内容：
   ```
   .env
   .env.local
   .env.*.local
...
5. 创建并运行第一次调用：
   ...
   async function main() {
     const response = await openai.responses.create({
       model: 'gpt-5.5',
       input: [{ role: 'user', content: '用一句话解释什么是 API' }]
     })
     console.log(response.output_text)
   }
   ...
   main()
   ```
   运行命令：`npx tsx src/examples/first-call.ts`

**要点**：
- 创建 Key 后立即设置用量上限（在 OpenAI 后台），防止泄露导致账单爆炸
- 常见错误：先把 Key 写在代码里"方便调试，上线再改"——结果一忙就忘了，Key 被提交到 Git 历史

### 练习二

**思路**：通过打印完整响应对象来理解 API 返回的数据结构。这是学习任何 API 的标准方法——先看完整响应，再按需取用字段。
...
1. **`status` 有哪些可能的值？**
   - `completed`：正常完成，输出完整
   - `incomplete`：输出被截断（通常因为达到 max_output_tokens 或触发安全策略）
   - `failed`：请求失败

2. **`input_tokens` 和 `output_tokens` 分别代表什么？**
   - `input_tokens`：你发送给模型的所有内容的 token 数（system prompt + 历史消息 + 当前用户输入）
   - `output_tokens`：模型生成的回答的 token 数
   - 成本 = input_tokens × 输入价格 + output_tokens × 输出价格（输出价格通常是输入的 4 倍）

3. **请求的 `id` 有什么用？**
   - 用于排查问题：当请求出错时，把 `id` 提供给 OpenAI 支持团队，他们可以定位到具体的请求日志
   - 用于日志记录：在你的应用中记录每个请求的 `id`，方便后续分析和审计

**要点**：
- 打印完整响应是学习 API 的第一步，不要只看 `output_text`
- 常见错误：忽略 `usage` 字段，导致无法监控成本

### 练习三

**思路**：故意触发错误是学习错误处理的最佳方式。每种错误对应不同的处理策略——有的需要重试，有的需要提示用户，有的需要立即停止。
...
async function testErrors() {
  // 测试 1：无效的 API Key
  try {
    const badClient = new OpenAI({ apiKey: 'sk-invalid-key' })
    await badClient.responses.create({
      model: 'gpt-5.5',
      input: [{ role: 'user', content: '你好' }]
    })
  } catch (error) {
    console.log('测试 1 - 无效 Key:', error.constructor.name, error.message)
    // 错误类型：AuthenticationError（401）
    // 处理方式：提示用户检查配置，不重试
  }
  ...
  try {
    ...
    console.log('测试 2 - 不存在的模型:', error.constructor.name, error.message)
    // 错误类型：NotFoundError（404）
    // 处理方式：检查模型名拼写
  }

  // 测试 3：超长输入
  try {
    const longMessage = '你好'.repeat(50000)  // 超长输入
    await openai.responses.create({
      model: 'gpt-5.5',
      input: [{ role: 'user', content: longMessage }]
    })
  } catch (error) {
    console.log('测试 3 - 超长输入:', error.constructor.name, error.message)
    // 错误类型：BadRequestError（400）- 上下文过长
    // 处理方式：缩短输入或分段处理
  }
}
...
**要点**：
- 每种错误的处理策略不同：401 不重试（Key 无效重试也没用），429 等待后重试（限流），400 检查输入（参数问题），500 重试 1-2 次（服务端临时问题）
- 常见错误：用一个 `catch (error) { console.log(error) }` 捕获所有错误，不区分类型，导致无法针对性处理
