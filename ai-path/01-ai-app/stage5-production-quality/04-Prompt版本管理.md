...
**思路**：Prompt 版本管理的核心是用数据库存储每个版本的内容、变更说明、创建人、评估分数。通过 name + version 唯一标识每个版本，isActive 标记当前使用的版本。
...
model PromptVersion {
  ...
  evalScore   Float?
  evalPassed  Boolean?
  trafficPercentage Int @default(0)
  createdAt   DateTime  @default(now())
  createdBy   String?
...
export class PromptManager {
  ...
  async createVersion(
    ...
    return prisma.promptVersion.create({
      data: { name, version, content, description, createdBy },
    })
  }
...
**要点**：
- Prompt 应该像配置一样独立管理，支持热更新，不能写死在代码里
- 每个版本记录内容、变更说明、创建人、创建时间，支持追溯
- 常见错误：把 Prompt 写死在代码里，改 Prompt 就要改代码、发版、部署；或者只存最新版本不存历史，出了问题无法回滚

### 练习二

**思路**：回滚的本质是切换 isActive 标记——找到当前版本的前一个版本，将其激活。关键是保留所有历史版本，回滚是秒级操作而非重新部署。

**答案**：

```typescript
// lib/prompt-manager.ts（续）
export class PromptManager {
  // 回滚到上一个版本
  async rollback(name: string) {
    const current = await prisma.promptVersion.findFirst({
  ...
  // 回滚到指定版本
  async rollbackTo(name: string, targetVersion: number) {
    const target = await prisma.promptVersion.findUnique({
      where: { name_version: { name, version: targetVersion } },
    })

    if (!target) throw new Error(`Version ${targetVersion} not found`)

    await this.activate(name, targetVersion)
    return target
  }
}
...
async function testRollback() {
  // 1. 记录当前版本
  const before = await promptManager.getActive('chat-system')
  console.log('当前版本:', before.slice(0, 50))

  // 2. 创建新版本并激活
  await promptManager.createVersion('chat-system', '新 Prompt 内容', '测试变更')
  const after = await promptManager.getActive('chat-system')
  console.log('新版本:', after.slice(0, 50))

  // 3. 回滚
  const rolledBack = await promptManager.rollback('chat-system')
  console.log('回滚到:', rolledBack.content.slice(0, 50))

  // 4. 验证回滚后的内容和之前一致
  const current = await promptManager.getActive('chat-system')
  console.log('验证一致:', current === before)
}
```

**要点**：
- 回滚是切换版本开关，秒级生效，不是"改回代码再部署"
- 保留旧版本至少 7 天，全量上线后不要立即删除
- 常见错误：每次修改 Prompt 直接覆盖旧版本，出了问题想回滚发现旧版本已经丢了

### 练习三

**思路**：灰度路由基于用户 ID 做哈希，保证同一用户始终使用同一版本。流量分配按百分比配置，支持 5% → 20% → 50% → 100% 的渐进式发布。
...
// 在聊天中使用灰度
export async function chat(userId: string, input: any[]) {
  const version = await trafficRouter.getVersion('chat-system', userId)
...
// 验证不同用户看到不同版本
async function verifyTrafficSplit() {
  const users = ['user-1', 'user-2', 'user-3', 'user-4', 'user-5']
  for (const userId of users) {
    const version = await trafficRouter.getVersion('chat-system', userId)
    console.log(`${userId} → version ${version}`)
  }
}
```

**要点**：
- 灰度路由必须基于用户 ID 做哈希，保证同一用户始终使用同一版本
- 灰度期间必须对比新旧版本的评估指标，不能只看"有没有报错"
- 常见错误：用随机数分配流量，同一个用户这次用新版本、下次用旧版本，体验不一致
